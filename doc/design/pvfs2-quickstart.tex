...
This document only applies to alpha snapshots of PVFS2 as of August
2003.
...
\item Linux kernel version 2.6.0-test3 [ or later ]
\end{itemize}
...
the PVFS2 Linux kernel driver [ Requires Linux Kernel 2.6.0-test3 or later ]
...
something we will try to address in future releases, but for now, push them out
by hand, renaming them to pvfs2-server.conf.  Here's one idea:
...
exist.  Do a command like this on every metadata or IO node in the cluster:
...
run a Linux 2.6.0-test3 kernel or later.

The basic steps are as follows:
\begin{itemize}
\item Make sure you are running Linux Kernel version 2.6.0-test3 or
  later
\item Compile the PVFS2 Kernel Module against your running kernel
\end{itemize}

Check to make sure that you are running a Linux 2.6.0-test3 Kernel or
later.  You can do this in the following manner:

\begin{verbatim}
gil:~# cat /proc/version 
Linux version 2.6.0-test3 (root@gil) (gcc version 3.3.1 20030722
...
\begin{verbatim}
gil:/usr/src/pvfs2# ./configure --with-kernel=/usr/src/linux-2.6.0-test3
\end{verbatim}

This example assumes that you've already downloaded, compiled, and are
now running the kernel located in the /usr/src/linux-2.6.0-test3
...
the where the code is located, and then run make.  You should see
...
modules
make[1]: Entering directory `/usr/src/linux-2.6.0-test3'
...
make[1]: Leaving directory `/usr/src/linux-2.6.0-test3'
\end{verbatim}
...
exist.  For the sake of example, our chosen mount point is /tmp/mnt.
...
\begin{verbatim}
gil:~# mount -t pvfs2 pvfs2 /tmp/mnt
gil:~# mount
/dev/hda3 on / type ext3 (rw,errors=remount-ro)
proc on /proc type proc (rw)
devpts on /dev/pts type devpts (rw,gid=5,mode=620)
/dev/hda1 on /boot type ext2 (rw)
pvfs2 on /tmp/mnt type pvfs2 (rw)
\end{verbatim}
...
\begin{verbatim}
gil:/usr/src/pvfs2/src/kernel/linux-2.6# mkdir /tmp/mnt/newdir
gil:/usr/src/pvfs2/src/kernel/linux-2.6# ls -al /tmp/mnt/newdir
total 1
