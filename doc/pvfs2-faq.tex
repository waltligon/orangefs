...
file systems have unique \texttt{Name} and \texttt{RootHandle} values (in the
...
on a client, where the new file system has the same name or root handle as one
...
server, NFS will outperform PVFS2 in most benchmarks.  NFS has
...
The interaction between the PVFS2 servers and the local file system
hosting the storage space has not been fully explored.  No doubt a
great deal of time could be spent on different file systems and their
parameters.
