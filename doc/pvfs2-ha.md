...
Some preliminary notes about installing Linux (Debian) on this hardware:
  ...
  - We went with Debian on this system. We figured if the software
  ...
  - Instead of using the command-line tools, configure through the bios:
    ...
    \(\rightarrow\) Cluster Mode \(\rightarrow\) Enabled. Also disable
    the PERC BIOS: that’s in the Objects \(\rightarrow\) Adapter menu
    too. See the manual for more things you can tweak. The utility lists
    all the important keystrokes at the bottom of the screen. Not
    exactly intuitive, but at least they are documented. For more
    information, see
  ...
  - If toggle on the back of the PowerVault is in cluster mode, and you
    ...
    “weird stuff” happens. I’m not really sure what i did to make it
    go away, but it involved a lot of futzing around with cables and
    that toggle on the back and rebooting nodes.

  - The GigE chips in the PowerEdge machines don’t need a crossover
...
‘quorum’ partition, which the two nodes will write to using raw
devices. At some point, something scrambled the main (not raw)
...
Figure [\[fig:haconfig\]](#fig:haconfig),
Figure [\[fig:haresources\]](#fig:haresources), and
Figure [\[fig:authkeys\]](#fig:authkeys) for
    examples.
...
Figure [\[fig:alias\]](#fig:alias)), the file system mounted, and the
pvfs2 servers running). Ping the cluster IP from another machine. If
something is broken, consult the /var/log/ha-log file or /var/log/syslog
and see if any of the scripts in /etc/ha.d/resource.d failed.
...
haresources file in Figure [\[fig:haresources\]](#fig:haresources), the
output of `ifconfig` should still look like
Figure [\[fig:alias\]](#fig:alias), you would still have /dev/sdb3
mounted on /shared, and pvfs2-server would still be running.
...
Figure [\[fig:init\]](#fig:init) and
Figure [\[fig:init-other\]](#fig:init-other).
...
Figure [\[fig:haresources-aa\]](#fig:haresources-aa).
...
Figure [\[fig:ifconfig-aa\]](#fig:ifconfig-aa) shows node A after node
B goes down. Node A now has both of the two cluster IP addresses bound
to two aliased interfaces B while continuing to manage it’s default
resource.
