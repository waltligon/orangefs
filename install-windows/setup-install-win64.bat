...
rem LDAP files
copy ..\..\ldap-windows\Win64\bin\ldap*.dll OrangeFS\Client /y

rem OpenSSL files
copy ..\..\openssl-windows\bin64\release\*.dll OrangeFS\Client /y

rem Client executable
copy ..\..\projects\OrangeFS\x64\Release\orangefs-client.exe OrangeFS\Client /y

rem MS CRT files
copy "C:\Program Files (x86)\Microsoft Visual Studio 10.0\VC\redist\x64\Microsoft.VC100.CRT\*.dll" OrangeFS\Client /y
...
rem Create zip
del ..\orangefs-windows-client-2.8.8-win64.zip
winrar a ..\orangefs-windows-client-2.8.8-win64.zip Dokan OrangeFS
