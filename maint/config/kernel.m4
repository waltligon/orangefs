...
[
	...
	AC_DEFINE(HAVE_KCONFIG, 1, Define if kconfig.h exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_VMTRUNCATE, 1, Define if vmtruncate exists)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_VALID_USER_NAMESPACE, 1, Define if the user namespace has been added to posix_acl_valid)],[AC_MSG_RESULT(no)
    ])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_USER_NAMESPACE, 1, Define if the user namespace has been added to posix_acl_from_xattr and posix_acl_to_xattr)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
...
dnl newer 3.3 kernels and above use d_make_root instead of d_alloc_root
	...
	dnl around 4.1
        AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
                ...
                AC_DEFINE(BACKING_DEV_IN_ADDR_SPACE,
                          1,
                          Define if struct  backing_dev_info is defined)],[AC_MSG_RESULT(no)
        ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_STRUCT_XTVEC, 1, Define if struct xtvec is defined in the kernel)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_STRUCT_KMEM_CACHE, 1, Define if struct kmem_cache is defined in kernel)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_SLAB_KERNEL, 1, Define if SLAB_KERNEL is defined in kernel)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_BDI_MEMORY_BACKED, 1, Define if struct backing_dev_info in kernel has memory_backed)],[AC_MSG_RESULT(no)
	])
	...
	if test "x$enable_kernel_sendfile" = "xyes"; then
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(HAVE_SENDFILE_VFS_SUPPORT, 1, Define if struct file_operations in kernel has sendfile callback)],[AC_MSG_RESULT(no)
		])
	fi
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_READV_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has readv callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_WRITEV_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has writev callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_FIND_INODE_HANDLE_SUPER_OPERATIONS, 1, Define if struct super_operations in kernel has find_inode_handle callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		static struct inode i = {
			...
			AC_DEFINE(HAVE_I_BLKSIZE_IN_STRUCT_INODE, 1, Define if struct inode in kernel has i_blksize member)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		static struct inode i = {
			...
			AC_DEFINE(HAVE_I_SEM_IN_STRUCT_INODE, 1, Define if struct inode in kernel has i_sem member)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_STATFS_LITE_SUPER_OPERATIONS, 1, Define if struct super_operations in kernel has statfs_lite callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_FILL_HANDLE_INODE_OPERATIONS, 1, Define if struct inode_operations in kernel has fill_handle callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_GETATTR_LITE_INODE_OPERATIONS, 1, Define if struct inode_operations in kernel has getattr_lite callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_GET_FS_KEY_SUPER_OPERATIONS, 1, Define if struct super_operations in kernel has get_fs_key callback)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_READDIR_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has readdir callback)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_READDIRPLUS_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has readdirplus callback)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_READDIRPLUSLITE_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has readdirplus_lite callback)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_READX_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has readx callback)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_WRITEX_FILE_OPERATIONS, 1, Define if struct file_operations in kernel has writex callback)],[AC_MSG_RESULT(no)
	    ])
	...
	if test "x$have_aio" = "xyes" -a "x$enable_kernel_aio" = "xyes"; then
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(HAVE_AIO_VFS_SUPPORT, 1, Define if we are enabling VFS AIO support in kernel)],[AC_MSG_RESULT(no)
		])
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(HAVE_KIOCBSETCANCELLED, 1, Define if kiocbSetCancelled exists)],[AC_MSG_RESULT(no)
		])
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(KI_USERS_ATOMIC, 1, Define if ki_users is atomic)],[AC_MSG_RESULT(no)
		])
		tmp_cflags=$CFLAGS
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(AIO_PUT_REQ_RETURNS_INT, 1, Define if aio_put_req returns int)],[AC_MSG_RESULT(no)
		])
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(HAVE_KIOCB_SET_CANCEL_FN, 1, Define if we have kiocb_set_cancel_fn)],[AC_MSG_RESULT(no)
		])
		...
		AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
			...
			AC_DEFINE(HAVE_AIO_NEW_AIO_SIGNATURE, 1, Define if VFS AIO support in kernel has a new prototype)],[AC_MSG_RESULT(no)
		])
		CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_DENTRY_STATFS_SOP, 1, Define if super_operations statfs has dentry argument)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_VFSMOUNT_GETSB, 1, Define if file_system_type get_sb has vfsmount argument)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_GETSB_NODEV, 1, Define if get_sb_nodev function exists )],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_FSTYPE_GET_SB, 1, Define if only filesystem_type has get_sb)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_F_DENTRY, 1, Define if file struct has f_dentry)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_FSTYPE_MOUNT_ONLY, 1, Define if only filesystem_type has mount and HAVE_FSTYPE_GET_SB is false)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		have_xattr=yes],[AC_MSG_RESULT(no)
		have_xattr=no
	])
	...
	if test "x$have_xattr" = "xyes"; then
	   ...
	   AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_SETXATTR_CONST_ARG, 1, Define if kernel setxattr has const void* argument)],[AC_MSG_RESULT(no)
		])
	fi
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_PROC_HANDLER_FILE_ARG, 1, Define if sysctl proc handlers have 6th argument)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_PROC_HANDLER_PPOS_ARG, 1, Define if sysctl proc handlers have ppos argument)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_DEFINE(HAVE_THREE_PARAM_GENERIC_PERMISSION, 1, Define if generic_permission takes three parameters)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_FOUR_PARAM_GENERIC_PERMISSION, 1, Define if generic_permission takes four parameters)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_TWO_PARAM_GENERIC_PERMISSION, 1, Define if generic_permission takes two parameters)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_I_SET_NLINK, 1, Define if set_nlink exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_I_INC_NLINK, 1, Define if inc_nlink exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_I_DROP_NLINK, 1, Define if drop_nlink exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_I_CLEAR_NLINK, 1, Define if clear_nlink exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_POSIX_ACL_EQUIV_MODE_UMODE_T, 1, Define if posix_acl_equiv_mode accepts umode_t type)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_CREATE_3, 1, Define if posix_acl_create has three arguments)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_CREATE_4, 1, Define if posix_acl_create has four arguments)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_CHMOD_3, 1, Define if posix_acl_chmod exists and has three arguments)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_CHMOD_2, 1, Define if posix_acl_chmod exists and has two arguments)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_POSIX_ACL_CLONE, 1, Define if posix_acl_clone exists)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_FSYNC_LOFF_T_PARAMS, 1, Define if fsync has loff_t params)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_ARG_IN_READ_DESCRIPTOR_T, 1, Define if read_descriptor_t has an arg member)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_FHTODENTRY_EXPORT_OPERATIONS, 1, Define if export_operations has an fh_to_dentry member)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_ENCODEFH_EXPORT_OPERATIONS, 1, Define if export_operations has an encode_fh member)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_INT_ARG2_ADDRESS_SPACE_OPERATIONS_RELEASEPAGE, 1, Define if sceond argument to releasepage in address_space_operations is type int)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_INT_RETURN_INODE_OPERATIONS_FOLLOW_LINK, 1, Define if return value from follow_link in inode_operations is type int)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    AC_DEFINE(HAVE_INT_RETURN_KMEM_CACHE_DESTROY, 1, Define if return value from kmem_cache_destroy is type int)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_INT_RETURN_ADDRESS_SPACE_OPERATIONS_INVALIDATEPAGE, 1, Define if return type of invalidatepage should be int)],[AC_MSG_RESULT(NO)
		])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_THREE_ARGUMENT_INVALIDATEPAGE, 1, Define if invalidatepage function has three arguments)],[AC_MSG_RESULT(NO)
		])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_NOWARNINGS_WHEN_INCLUDING_LINUX_CONFIG_H, 1, Define if including linux/config.h gives no warnings)],[AC_MSG_RESULT(yes)
	])
	CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
	    ...
	    #include <linux/fs.h>
		 ...
		 AC_DEFINE(HAVE_COMPAT_IOCTL_HANDLER, 1, Define if there exists a compat_ioctl member in file_operations)],[AC_MSG_RESULT(no)
	    ])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_KMEM_CACHE_DESTROY_INT_RETURN, 1, Define if kmem_cache_destroy returns int)],[AC_MSG_RESULT(no)
	])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_WRITE_ITER, 1, file_operations has write_iter)],[AC_MSG_RESULT(no)
		])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_IOV_ITER, 1, iov_iter interface is supported)],[AC_MSG_RESULT(no)
		])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_ND_SET_LINK, 1, nd_set_link is a function)],[AC_MSG_RESULT(no)
		])
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(GWC_USES_KIOCB, 1, generic_write_checks with kiocb)],[AC_MSG_RESULT(no)
		])
	...
	AC_DEFINE(HAVE_KZALLOC, 1, Define if kzalloc exists)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_TWO_ARG_REGISTER_SYSCTL_TABLE, 1, Define if register_sysctl_table takes two arguments)],[AC_MSG_RESULT(no)
	])
	...
	]])],[AC_MSG_RESULT(yes)],[AC_DEFINE(HAVE_NO_FS_IOC_FLAGS, 1, Define if FS_IOC flags missing from fs.h)
	AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_OBSOLETE_STRUCT_PAGE_COUNT_NO_UNDERSCORE, 1, Define if struct page defines a count member without leading underscore)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_KERNEL_DEVICE_CLASSES, 1, Define if kernel has device classes)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_KMEM_CACHE_CREATE_DESTRUCTOR_PARAM, 1, Define if kernel kmem_cache_create has destructor param)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_KMEM_CACHE_CREATE_CTOR_ONE_PARAM, 1, Define if kernel kmem_cache_create constructor has newer-style one-parameter form)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_TWO_PARAM_PERMISSION, 1, Define if kernel's inode_operations has two parameters permission function)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_KMEM_CACHE_CREATE_CTOR_TWO_PARAM, 1, Define if kernel kmem_cache_create constructor has new-style two-parameter form)],[AC_MSG_RESULT(no)
	])
	CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_SPIN_LOCK_PAGE_ADDR_SPACE_STRUCT, 1, Define if kernel address_space struct has a spin_lock member named page_lock instead of rw_lock)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_RW_LOCK_TREE_ADDR_SPACE_STRUCT, 1, Define if kernel address_space struct has a rw_lock_t member named tree_lock)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_SPIN_LOCK_TREE_ADDR_SPACE_STRUCT, 1, Define if kernel address_space struct has a spin_lock_t member named tree_lock)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_RT_PRIV_LOCK_ADDR_SPACE_STRUCT, 1, Define if kernel address_space struct has a spin_lock for private data instead of rw_lock -- used by RT linux)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_MAPPING_NRPAGES_MACRO, 1, Define if kernel defines mapping_nrpages macro -- defined by RT linux)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_READ_INODE, 1, Define if kernel super_operations contains read_inode field)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_DROP_INODE, 1, Define if kernel super_operations contains drop_inode field)],[AC_MSG_RESULT(no)
	])
	...
	AC_DEFINE(HAVE_PUT_INODE, 1, Define if kernel super_operations contains put_inode field)],[AC_MSG_RESULT(no)
	])
	...
	MNT_NOATIME flags)],[AC_MSG_RESULT(no)
	])
	...
	MNT_NODIRATIME flags)],[AC_MSG_RESULT(no)
	])
        ...
        AC_DEFINE(HAVE_D_ALLOC_ANON, 1, Define if dcache.h contains 
                  d_alloc_annon)],[AC_MSG_RESULT(no)
        ])
        ...
        AC_DEFINE(HAVE_SB_DIRTY_LIST, 1, Define if struct super_block has s_dirty list)],[AC_MSG_RESULT(no)
        ])
        ...
        AC_DEFINE(HAVE_CURRENT_FSUID, 1, Define if cred.h contains current_fsuid)],[AC_MSG_RESULT(no)
        ])
        CFLAGS=$tmp_cflags
        ...
        AC_DEFINE(HAVE_FROM_KUID, 1, Define if from_kuid function is found)],[AC_MSG_RESULT(no)
        ])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_BACKING_DEV_INFO_NAME, 1, Define if kernel backing_dev_info struct has a name field)],[AC_MSG_RESULT(no)
	])
        ...
        AC_DEFINE(HAVE_BDI_INIT, 1, Define if bdi_init function is present)],[AC_MSG_RESULT(no)
        ])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_CTL_NAME, 1, Define if struct ctl_table has ctl_name member)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_STRATEGY_NAME, 1, Define if struct ctl_table has strategy member)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_XATTR_HANDLER_GET_2_6_33, 1, Define if kernel xattr_handle get function args are dentry, char, void, size_t, int)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_XATTR_HANDLER_GET_4_4, 1, Define if kernel xattr_handle get function args are xattr_handler, dentry, char, void and size_t)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_XATTR_HANDLER_SET_2_6_33, 1, Define if kernel xattr_handle set function args are dentry, char, void, size_t, int, int)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_XATTR_HANDLER_SET_4_4, 1, Define if kernel xattr_handle set function args are xattr_handler, dentry, char, void, size_t, int)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_CONST_S_XATTR_IN_SUPERBLOCK, 1, Define if s_xattr member of super_block struct is const)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_TRUE_FALSE_ENUM, 1, Define if kernel stddef has true/false enum)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_FSYNC_DENTRY_PARAM, 1, Define if fsync function in file_operations struct wants a dentry pointer as the second parameter)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_UNLOCKED_IOCTL_HANDLER, 1, Define if file_operations struct has unlocked_ioctl member)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_INODE_SETATTR, 1, Define if inode_setattr is defined)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_THREE_PARAM_D_HASH, 1, Define if d_hash member of dentry_operations has three params, the second inode paramsbeing the difference)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_TWO_PARAM_D_HASH_WITH_CONST, 1, Define if d_hash member of dentry_operations has two params, where the first param is a const )],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_SEVEN_PARAM_D_COMPARE, 1, Define if d_compare member of dentry_operations has seven params)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_FIVE_PARAM_D_COMPARE, 1, Define if d_compare member of dentry_operations has five params)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_D_DELETE_CONST, 1, Define if d_delete member of dentry_operations has a const dentry param)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_DENTRY_D_COUNT_ATOMIC, 1, Define if d_count member of dentry is of type atomic_t)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_DENTRY_LOCKREF_STRUCT, 1, Define if dentry struct has a lockref struct member)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
        ...
        AC_DEFINE(HAVE_D_COUNT_FUNCTION, 1, define if d_count() is provided)],[AC_MSG_RESULT(no)
        ])
        CFLAGS=$tmp_cflags 
        ...
        AC_DEFINE(C_22c6186_FLAVORED_DIO, 1, commit 22c6186 flavored direct_IO)],[AC_MSG_RESULT(no)
        ])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_THREE_PARAM_PERMISSION_WITH_FLAG, 1, Define if permission function pointer of inode_operations struct has three parameters and the third parameter is for flags (unsigned int))],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_THREE_PARAM_ACL_CHECK, 1, Define if acl_check param of generic_permission function has three parameters)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_SPIN_LOCK_UNLOCKED, 1, Define if SPIN_LOCK_UNLOCKED defined)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_DEFINE(HAVE_GET_SB_MEMBER_FILE_SYSTEM_TYPE, 1, Define if get_sb is a member of file_system_type struct)],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
	...
	AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
		...
		AC_DEFINE(HAVE_DIRTY_INODE_FLAGS, 1, Define if dirty_inode takes a flag argument )],[AC_MSG_RESULT(no)
	])
        CFLAGS=$tmp_cflags
