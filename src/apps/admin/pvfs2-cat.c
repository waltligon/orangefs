...
static int generic_cleanup(file_object *src, PVFS_credentials *credentials);
...
int main (int argc, char ** argv)
{
    ...
    while((current_size = generic_read(&src, buffer, 
									   total_written, user_opts->buf_size, 
									   &credentials)) > 0)
    {
		write(1, buffer, current_size);
  ...
  main_out:
    generic_cleanup(&src, &credentials);
    PVFS_sys_finalize();
    free(user_opts);
    free(buffer);
    return(ret);
