    ...
    char *destfile)
{
    ...
    else
    {
        ...
        if (PINT_remove_base_dir(pvfs_path, str_buf, PVFS_NAME_MAX))
        {
            ...
            fprintf(stderr,
                    "Error: cannot retrieve entry name for " "creation on %s\n",
                    pvfs_path);
            return -1;
        }
    ...
    ret =
        PVFS_sys_getattr(resp_lookup.ref, attrmask, &credentials,
                         &resp_getattr);
    if (ret < 0)
    {
    ...
    char *argv[])
{
    ...
    tmp_opts->perms =
        (user_perms << 6) | (group_perms << 3) | (other_perms << 0) |
        (special_perms << 9);
    ...
    char **argv)
{
    ...
    fprintf(stderr,
            "       S - the special permissions (setgid, etc.) for the file(s)\n");
