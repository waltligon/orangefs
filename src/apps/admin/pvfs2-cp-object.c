...
int main (int argc, char ** argv)
{
    ...
    time1 = Wtime();
    while((current_size = generic_read(&src,
                                       buffer, 
		                       total_written,
                                       user_opts->buf_size,
                                       &credentials)) > 0)
 ...
 * 'buffer' */
static size_t generic_read(file_object *src,
                           char *buffer, 
	                   int64_t offset,
                           size_t count,
                           PVFS_credential *credentials)
{
	...
	ret = PVFS_sys_io(src->u.pvfs2.ref,
                          file_req, offset,
		          buffer,
                          mem_req,
                          credentials,
...
static size_t generic_write(file_object *dest,
                            char *buffer, 
                            int64_t offset,
                            size_t count,
                            PVFS_credential *credentials)
{
	...
	PVFS_util_refresh_credential(credentials);
	ret = PVFS_sys_io_object(dest->u.pvfs2.ref,
                                 file_req,
                                 offset,
		                 buffer,
                                 mem_req,
                        ...
                        int open_type)
{
    ...
    obj->u.pvfs2.attr.dfile_count = 4;
	...
	memset(&resp_lookup, 0, sizeof(PVFS_sysresp_lookup));
	ret = PVFS_sys_ref_lookup(parent_ref.fs_id, 
                                  entry_name,
                                  parent_ref,
                                  credentials,
                                  &resp_lookup,
	...
	if (open_type == OPEN_DEST)
	{
		...
		make_attribs(&(obj->u.pvfs2.attr),
                             credentials,
                             nr_datafiles,
                             (int)stat_buf.st_mode);
                new_dist = PVFS_sys_dist_lookup("object_dist");
                /*if (strip_size > 0) {
                    
                    ret = PVFS_sys_dist_setparam(new_dist,
                                                 "strip_size",
                                                 &strip_size);
                    if (ret < 0)
                    {
		...
		ret = PVFS_sys_create(entry_name,
                                      parent_ref, 
                                      obj->u.pvfs2.attr,
                                      credentials,
                                      new_dist,
                           ...
                           PVFS_credential *credentials)
{
    ...
    {
        PVFS_sys_setattr(dest->u.pvfs2.ref,
                         src->u.pvfs2.attr,
                         credentials,
                         hints);
    }
