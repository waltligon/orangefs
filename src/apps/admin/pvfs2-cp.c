                        ...
                        char *srcname, int open_type)
{
    ...
    else
    {
        ...
        PVFS_object_ref_release(&parent_ref);
    }
    return 0;
}
                           ...
                           PVFS_credential *credentials)
{
    ...
    {
        ret = fchmod(dest->u.ufs.fd,
               convert_pvfs2_perms_to_mode(src->u.pvfs2.perms));
    }

    /* preserve permissions doing a unix => pvfs2 copy */
    if ((src->fs_type == UNIX_FILE) && (dest->fs_type == PVFS2_FILE))
    {
        make_attribs(&(dest->u.pvfs2.attr), credentials, -1,
                     src->u.ufs.mode);
    ...
    if ((src->fs_type == UNIX_FILE) &&
        ((dest->fs_type == UNIX_FILE) && (dest->u.ufs.fd != -1)))
    {
        ret = fchmod(dest->u.ufs.fd, src->u.ufs.mode);
    }
