...
static int generic_open(
    ...
    char *srcname,
    int open_type);
static size_t generic_read(
    file_object * src,
    char *buffer,
    int64_t offset,
    size_t count,
    PVFS_credentials * credentials);
static size_t generic_write(
    file_object * dest,
    char *buffer,
    int64_t offset,
    size_t count,
    PVFS_credentials * credentials);
static int generic_cleanup(
    file_object * src,
    file_object * dest,
    PVFS_credentials * credentials);
static void make_attribs(
    PVFS_sys_attr * attr,
    PVFS_credentials * credentials,
    int nr_datafiles,
    int mode);

static int convert_pvfs2_perms_to_mode(
    PVFS_permissions perms)
{
    int ret = 0, i = 0;
    static int modes[9] = {
    ...
    char **argv)
{
    ...
    ret =
        generic_open(&dest, &credentials, user_opts->num_datafiles,
                     user_opts->strip_size, user_opts->srcfile, OPEN_DEST);
    ...
    time1 = Wtime();
    while ((current_size =
            generic_read(&src, buffer, total_written, user_opts->buf_size,
                         &credentials)) > 0)
    {
        buffer_size = current_size;

        ret =
            generic_write(&dest, buffer, total_written, buffer_size,
                          &credentials);
        if (ret != current_size)
        {
            if (ret == -1)
...
static void usage(
    ...
    char **argv)
{
    fprintf(stderr, "Usage: %s ARGS src_file dest_file\n", argv[0]);
    ...
    PVFS_credentials * credentials)
{
    ...
    else
    {
        ...
        ret =
            PVFS_sys_read(src->u.pvfs2.ref, file_req, offset, buffer, mem_req,
                          credentials, &resp_io);
        if (ret == 0)
        {
    ...
    PVFS_credentials * credentials)
{
    ...
    else
    {
        ...
        ret =
            PVFS_sys_write(dest->u.pvfs2.ref, file_req, offset, buffer, mem_req,
                           credentials, &resp_io);
        if (ret == 0)
        {
            PVFS_Request_free(&mem_req);
    ...
    int open_type)
{
    ...
    else
    {
        ...
        if (strcmp(obj->u.pvfs2.pvfs2_path, "/") == 0)
        {
            ...
            {
                fprintf(stderr,
                        "unable to guess filename in " "toplevel PVFS2\n");
                return -1;
            }
        ...
        else    /* given either a pvfs2 directory or a pvfs2 file */
        {
            ...
            if (PINT_remove_base_dir
                (obj->u.pvfs2.pvfs2_path, str_buf, PVFS_NAME_MAX))
            {
                ...
                fprintf(stderr,
                        "Error: cannot retrieve entry name for "
                        "creation on %s\n", obj->u.pvfs2.user_path);
                return (-1);
            }
            ret =
                PINT_lookup_parent(obj->u.pvfs2.pvfs2_path, obj->u.pvfs2.fs_id,
                                   credentials, &parent_ref.handle);
            if (ret < 0)
            {
        ...
        if ((ret == 0) && (open_type == OPEN_SRC))
        {
            ...
            ret =
                PVFS_sys_getattr(resp_lookup.ref, PVFS_ATTR_SYS_ALL_NOHINT,
        ...
        if (open_type == OPEN_DEST)
        {
            ...
            else
            {
                ...
                if (strip_size > 0)
                {
                    ...
                    ret =
                        PVFS_sys_dist_setparam(new_dist, "strip_size",
                                               &strip_size);
                    if (ret < 0)
                    {
                ...
                ret =
                    PVFS_sys_create(entry_name, parent_ref, obj->u.pvfs2.attr,
                                    credentials, new_dist, &resp_create);
                if (ret < 0)
                {
    ...
    PVFS_credentials * credentials)
{
    ...
    {
        fchmod(dest->u.ufs.fd, convert_pvfs2_perms_to_mode(src->u.pvfs2.perms));
    }
