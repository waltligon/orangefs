    ...
    char **argv)
{
    ...
    ret =
        PVFS_mgmt_count_servers(cur_fs, &creds, PVFS_MGMT_IO_SERVER,
                                &io_server_count);
    if (ret < 0)
    {
    ...
    event_matrix =
        (struct PVFS_mgmt_event **) malloc(io_server_count *
    ...
    {
        event_matrix[i] =
            (struct PVFS_mgmt_event *) malloc(EVENT_DEPTH *
                                              sizeof(struct PVFS_mgmt_event));
    ...
    addr_array =
        (PVFS_BMI_addr_t *) malloc(io_server_count * sizeof(PVFS_BMI_addr_t));
    if (addr_array == NULL)
    {
        perror("malloc");
        return -1;
    ...
    ret =
        PVFS_mgmt_get_server_array(cur_fs, &creds, PVFS_MGMT_IO_SERVER,
    ...
    ret = PVFS_mgmt_event_mon_list(cur_fs,
                                   &creds,
                                   event_matrix, addr_array, io_server_count,
                                   EVENT_DEPTH, NULL /* detailed errors */ );
    if (ret < 0)
    {
    ...
    printf
        ("# (server number) (api) (operation) (value) (id) (flags) (sec) (usec)\n");
    for (i = 0; i < io_server_count; i++)
    {
