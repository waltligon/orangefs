...
void analyze_remaining_handles(
    PVFS_fs_id cur_fs,
...
static void handlelist_initialize(
    ...
    char **argv)
{
    ...
    traverse_directory_tree(cur_fs, addr_array, server_count, &creds,
                            user_opts);

    analyze_remaining_handles(cur_fs, addr_array, &creds,
    ...
    PVFS_credentials * creds)
{
    ...
    do
    {
        ...
        for (i = 0; i < server_count; i++)
        {
            ...
            handlelist_add_handles(handle_matrix[i], hcount_array[i], i);
        }
    ...
    struct options *opts_p)
{
    ...
    do
    {
        ...
        ret =
            PVFS_sys_readdir(pref, (!token ? PVFS_READDIR_START : token), count,
                             creds, &readdir_resp);
        ...
        for (i = 0; i < readdir_resp.pvfs_dirent_outcount; i++)
        {
            ...
            print_entry(cur_file, cur_handle, pref.handle,      /* parent handle */
                        getattr_resp.attr.objtype, server_idx, 0,
                        opts_p->dot_format, opts_p->fontname);

            switch (getattr_resp.attr.objtype)
            {
            case PVFS_TYPE_METAFILE:
                verify_datafiles(cur_fs, entry_ref,
                                 getattr_resp.attr.dfile_count, creds, opts_p);
                break;
    ...
    struct options *opts_p)
{
    ...
    for (i = 0; i < df_count; i++)
    {
        ...
        if (ret != 0)
        {
            printf("Datafile Handle %llu appears to be missing; " "skipping!\n",
                   llu(df_handles[i]));
            continue;
        }
        ...
        print_entry(NULL,
                    df_handles[i],
                    mf_ref.handle, PVFS_TYPE_DATAFILE, server_idx, 0,
                    opts_p->dot_format, opts_p->fontname);
    ...
    int dot_fmt)
{
    ...
    while (handlelist_return_handle(&handle, &server_idx) == 0)
    {
        ...
        PVFS_sys_getattr(entry_ref, PVFS_ATTR_SYS_ALL, creds, &getattr_resp);
        if (getattr_resp.attr.objtype != PVFS_TYPE_DIRDATA)
        {
            ...
            {
                printf("\t%s: %llu\n",
                       PVFS_mgmt_map_addr(cur_fs, creds, addr_array[server_idx],
                                          &tmp_type), llu(handle));
            }
        }
    }
    ...
    int server_idx)
{
    ...
    if ((handlelist_size[server_idx] - start_off) < handle_count)
    {
        fprintf(stderr,
                ...
                server_idx, handlelist_size[server_idx],
    ...
    void)
{
    ...
    {
        if (handlelist_used[i] != handlelist_size[i])
            assert(0);
        ...
        if (handlelist_list[server_idx][i] == handle)
        {
            if (i < (handlelist_used[server_idx] - 1))
            {
                ...
                handlelist_list[server_idx][i] =
                    handlelist_list[server_idx][handlelist_used[server_idx] -
                                                1];
    ...
    char *fontname)
{
    ...
    {
        printf
            ("\tH%llu [shape = record, fillcolor = grey, style = filled, label = \"{/ | %llu (%d)}\"];\n",
             llu(handle), llu(handle), server_idx);
    }
    else
    {
        printf("File: <Root>\n  handle = %llu, type = %s, server = %d\n",
               llu(handle), get_type_str(PVFS_TYPE_DIRECTORY), server_idx);
    }
}
    ...
    char *fontname)
{
    if (dot_fmt)
    {
        ...
        case PVFS_TYPE_DIRECTORY:
            printf
                ("\tH%llu [shape = record, fillcolor = grey, style = filled, label = \"{%s/ | %llu (%d)}\"];\n",
                 llu(handle), name, llu(handle), server_idx);
            break;
        case PVFS_TYPE_METAFILE:
            printf
                ("\tH%llu [shape = record, fillcolor = aquamarine, style = filled, label = \"{%s | %llu (%d)}\"];\n",
                 llu(handle), name, llu(handle), server_idx);
            break;
        case PVFS_TYPE_DATAFILE:
            printf
                ("\tH%llu [shape = ellipse, fillcolor = violet, style = filled, label =\"%llu (%d)\"];\n",
                 llu(handle), llu(handle), server_idx);
            break;
    ...
    else
    {
        ...
        case PVFS_TYPE_DATAFILE:
            printf("  handle = %llu, type = %s, server = %d\n", llu(handle),
                   get_type_str(objtype), server_idx);
            break;
        default:
            printf("File: %s\n  handle = %llu, type = %s, server = %d\n",
                   name, llu(handle), get_type_str(objtype), server_idx);
            break;
        }
    }
}
    ...
    char **argv)
{
    ...
    fprintf(stderr,
            "    -f <font>     when used with -d, specifies a font name (e.g. Helvetica)\n");
