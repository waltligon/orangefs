		     ...
		     PVFS_credentials *creds)
{
    ...
    kv_position_array = 
        (PVFS_kv_position *) calloc(server_count, sizeof(PVFS_kv_position));
			    ...
			    struct options *opts_p)
{
	...
	fprintf(stderr, "Cannot traverse object at "
                "%s,%d (Not a Valid Directory)\n",
                PVFS_handle_to_str(pref.handle), pref.fs_id);
        return -1;
    }

    if (handlelist_find_handle(pref.handle, &server_idx) < 0)
    {
        printf("Handle %s appears to be missing; skipping!\n",
	    ...
	    struct options *opts_p)
{
    ...
    do {
        ...
        ret = PVFS_sys_readdir(pref, token, &token_flag,
                         count,
                         creds,
                         &readdir_resp, NULL);
        ...
        for (i = 0; i < readdir_resp.pvfs_dirent_outcount; i++)
        {
            ...
            if ((ret = PVFS_sys_getattr(entry_ref,
                             ...
                             &getattr_resp, NULL)) != 0)
            {
                printf("Could not get attributes of handle %s [%d]\n",
                        PVFS_handle_to_str(cur_handle), ret);
                continue;
            }
		      ...
		      struct options *opts_p)
{
	...
	if (ret != 0)
	{
            printf("Datafile Handle %s appears to be missing; "
			       ...
			       int dot_fmt)
{
    ...
    while (handlelist_return_handle(&handle, &server_idx) == 0)
    {
        ...
        PVFS_handle_copy(entry_ref.handle, handle);
        entry_ref.fs_id  = cur_fs;
        ...
        if (getattr_resp.attr.objtype != PVFS_TYPE_DIRDATA)
        {
            ...
            {
                printf("\tH%s [shape=record, color=red, label = \"{(unknown) "
                       "| %s (%d)}\"];\n",
            ...
            else if(!dot_fmt)
            {
                if(getattr_resp.attr.objtype == PVFS_TYPE_INTERNAL)
                    fmt_string = "\t%s: %s (server internal use)\n";
                else if(getattr_resp.attr.objtype == PVFS_TYPE_DATAFILE)
                    fmt_string = "\t%s: %s (datafile, probably preallocated)\n";
                else if(getattr_resp.attr.objtype == PVFS_TYPE_METAFILE)
                    fmt_string = "\t%s: %s (metafile, probably preallocated)\n";
                else
                    fmt_string = "\t%s: %s (unknown)\n";
				   ...
				   int server_idx)
{
	...
	PVFS_handle_copy(handlelist_list[server_idx][start_off + i],
            handles[i]);
    }
...
static void handlelist_remove_handle(PVFS_handle handle, int server_idx)
{
	...
	if (handlelist_list[server_idx][i] == handle)
	{
		...
		PVFS_handle_copy(handlelist_list[server_idx][i],
...
static int handlelist_return_handle(PVFS_handle *handle_p, int *server_idx_p)
{
	...
	if (handlelist_used[i] > 0) {
	    PVFS_handle_copy(*handle_p, 
                handlelist_list[i][handlelist_used[i]-1]);
	    handlelist_used[i]--;
	    *server_idx_p = i;
	    return 0;
	}
			     ...
			     char *fontname)
{
    if (dot_fmt)
    {
	printf("\tH%s [shape = record, fillcolor = grey, style = filled, label = \"{/ | %s (%d)}\"];\n",
	       PVFS_handle_to_str(handle),
	       PVFS_handle_to_str(handle),
	       server_idx);
    }
			...
			char *fontname)
{
	...
	printf("\tH%s -> H%s [style = bold];\n",
	       PVFS_handle_to_str(parent_handle),
	       PVFS_handle_to_str(handle));
	switch (objtype) {
	    case PVFS_TYPE_DIRECTORY:
		printf("\tH%s [shape = record, fillcolor = grey, style = filled, label = \"{%s/ | %s (%d)}\"];\n",
		       PVFS_handle_to_str(handle), name, PVFS_handle_to_str(handle), server_idx);
		break;
	    case PVFS_TYPE_METAFILE:
		printf("\tH%s [shape = record, fillcolor = aquamarine, style = filled, label = \"{%s | %s (%d)}\"];\n",
		       PVFS_handle_to_str(handle), name, PVFS_handle_to_str(handle), server_idx);
		break;
	    case PVFS_TYPE_DATAFILE:
		printf("\tH%s [shape = ellipse, fillcolor = violet, style = filled, label =\"%s (%d)\"];\n",
		       PVFS_handle_to_str(handle), PVFS_handle_to_str(handle), server_idx);
		break;
