...
static void handlelist_initialize(unsigned long *handle_counts,
                                  int server_count);
		     ...
		     PVFS_credential *creds)
{
    ...
    {
	handle_matrix[i] = (PVFS_handle *) calloc(HANDLE_BATCH,
                                                  sizeof(PVFS_handle));
    ...
    handle_count_array = (unsigned long *) calloc(server_count,
    ...
    /* total_count_array */
    total_count_array = (unsigned long *) calloc(server_count,
                                                 sizeof(unsigned long));
	...
	for (i=0; i < server_count; i++)
	{
            ...
            /* V3 handles no longer map to one server */
#if 0
		 ...
		 */
                ...
                {
                    fprintf(stderr, "Ugh! handle does not seem "
                                    "to be owned by the server!\n");
                    return -1;
                }
	    }
#endif
    ...
    for (i = 0; i < server_count; i++)
    {
        ...
        {
            fprintf(stderr, "Ugh! Server %d, Received %ld "
                            "total handles instead of %ld\n",
			    ...
			    struct options *opts_p)
{
	...
	fprintf(stderr, "Cannot traverse object at "
                ...
                PVFS_OID_str(&pref.handle), pref.fs_id);
        return -1;
    }
...
static int handlelist_find_handle(PVFS_handle handle, int *server_idx_p)
{
	...
	for (j = 0; j < handlelist_used[i]; j++)
        {
	    if (!PVFS_OID_cmp(&handlelist_list[i][j], &handle))
            {
		*server_idx_p = i;
		return 0;
	    }
...
static void handlelist_remove_handle(PVFS_handle handle, int server_idx)
{
    ...
    {
	if (!PVFS_OID_cmp(&handlelist_list[server_idx][i], &handle))
	{
			     ...
			     char *fontname)
{
    if (dot_fmt)
    {
	printf("\tH%s [shape = record, fillcolor = grey, style = filled, "
               "label = \"{/ | %s (%d)}\"];\n",
	       PVFS_OID_str(&handle),
	       PVFS_OID_str(&handle),
	       server_idx);
			...
			char *fontname)
{
	...
	printf("\tH%s -> H%s [style = bold];\n",
	       PVFS_OID_str(&parent_handle),
	       PVFS_OID_str(&handle));
	switch (objtype) {
	    case PVFS_TYPE_DIRECTORY:
		printf("\tH%s [shape = record, fillcolor = grey, "
                       "style = filled, label = \"{%s/ | %s (%d)}\"];\n",
		       PVFS_OID_str(&handle),
                       name,
                       PVFS_OID_str(&handle),
                       server_idx);
		break;
	    case PVFS_TYPE_METAFILE:
		printf("\tH%s [shape = record, fillcolor = aquamarine, "
                       "style = filled, label = \"{%s | %s (%d)}\"];\n",
		       PVFS_OID_str(&handle),
                       name,
                       PVFS_OID_str(&handle),
		...
		printf("\tH%s [shape = ellipse, fillcolor = violet, "
