    ...
    char **argv)
{
    ...
    ret =
        PVFS_util_resolve(fsck_opts->mnt_point, &cur_fs, pvfs_path,
                          PVFS_NAME_MAX);
    if (ret != 0)
    {
    ...
    ret =
        PVFS_mgmt_count_servers(cur_fs, &creds,
    ...
    addr_array =
        (PVFS_BMI_addr_t *) malloc(server_count * sizeof(PVFS_BMI_addr_t));
    if (addr_array == NULL)
    {
        perror("malloc");
        return -1;
    ...
    ret = create_lost_and_found(cur_fs, &creds);
    if (ret != 0)
    {
        if (ret == -PVFS_EAGAIN)
        {
    ...
    traverse_directory_tree(cur_fs, hl_all, addr_array, server_count, &creds);
    ...
    hl_notree = find_sub_trees(cur_fs, hl_all, addr_array, &creds);
    ...
    PVFS_mgmt_setparam_list(cur_fs,
                            &creds,
                            PVFS_SERV_PARAM_MODE,
                            (uint64_t) PVFS_SERVER_NORMAL_MODE, addr_array,
                            NULL, server_count, NULL);
    in_admin_mode = 0;
    ...
    hl_unrefd = fill_lost_and_found(cur_fs, hl_notree, addr_array, &creds);
...
struct handlelist *build_handlelist(
    PVFS_fs_id cur_fs,
    ...
    PVFS_credentials * creds)
{
    ...
    stat_array =
        (struct PVFS_mgmt_server_stat *) malloc(server_count *
                                                sizeof(struct
    ...
    ret =
        PVFS_mgmt_statfs_list(cur_fs, creds, stat_array, addr_array,
                              server_count, NULL /* details */ );
    if (ret != 0)
    {
    ...
    {
        handle_matrix[i] =
            (PVFS_handle *) calloc(HANDLE_BATCH, sizeof(PVFS_handle));
    ...
    handle_count_array =
        (unsigned long *) calloc(server_count, sizeof(unsigned long));
    ...
    position_array =
        (PVFS_ds_position *) calloc(server_count, sizeof(PVFS_ds_position));
    ...
    while (more_flag)
    {
        ...
        if (ret < 0)
        {
            ...
            PVFS_mgmt_setparam_list(cur_fs,
                                    ...
                                    addr_array, NULL, server_count, NULL);
            return NULL;
        }
        ...
        for (i = 0; i < server_count; i++)
        {
            ...
            for (j = 0; j < hcount_array[i]; j++)
            {
                 ...
                 */
                ret =
                    PINT_cached_config_map_to_server(&tmp_addr,
                                                     handle_matrix[i][j],
                                                     cur_fs);
            ...
            handlelist_add_handles(hl, handle_matrix[i], hcount_array[i], i);
        }
    ...
    for (i = 0; i < server_count; i++)
    {
        ...
        {
            fprintf(stderr,
                    "Ugh! Server %d, Received %ld total handles instead of %ld\n",
    ...
    PVFS_credentials * creds)
{
    ...
    PVFS_sys_getattr(pref, PVFS_ATTR_SYS_ALL_NOHINT, creds, &getattr_resp);
    ...
    ret = match_dirdata(hl, NULL /* optional second handle list */ ,
                        pref, creds);
    if (ret != 0)
    {
        assert(0);
    ...
    PVFS_credentials * creds)
{
    ...
    ret = PVFS_mgmt_get_dirdata_handle(dir_ref, &dirdata_handle, creds);
    if (ret != 0)
    {
    ...
    PVFS_credentials * creds)
{
    ...
    do
    {
        ...
        PVFS_sys_readdir(dir_ref, (!token ? PVFS_READDIR_START : token), count,
                         creds, &readdir_resp);
        ...
        for (i = 0; i < readdir_resp.pvfs_dirent_outcount; i++)
        {
            ...
            if (!in_main_list && alt_hl
                && handlelist_find_handle(alt_hl, cur_handle, &server_idx) == 0)
            {
                in_alt_list = 1;
            }
            ...
            {
                ret =
                    remove_directory_entry(dir_ref, entry_ref, cur_file, creds);
                assert(ret == 0);
            ...
            ret =
                PVFS_sys_getattr(entry_ref, PVFS_ATTR_SYS_ALL_NOHINT, creds,
                                 &getattr_resp);
            if (ret != 0)
            {
                ret =
            ...
            else
            {
                ...
                case PVFS_TYPE_METAFILE:
                    if (verify_datafiles
                        (cur_fs, hl, alt_hl, entry_ref,
                         getattr_resp.attr.dfile_count, creds) < 0)
                    {
                        /* not recoverable; remove */
                        printf("* File %s (%llu) is not recoverable.\n",
                               cur_file, llu(cur_handle));

                        /* verify_datafiles() removed the datafiles */
                        ret =
                            remove_object(entry_ref, getattr_resp.attr.objtype,
                                          creds);
                        assert(ret == 0);

                        ret =
                            remove_directory_entry(dir_ref, entry_ref, cur_file,
                                                   creds);
                        assert(ret == 0);
                    }
                ...
                case PVFS_TYPE_DIRECTORY:
                    ret = match_dirdata(hl, alt_hl, entry_ref, creds);
                    if (ret != 0)
                    {
                    ...
                    if (in_main_list)
                    {
                        ret = descend(cur_fs, hl, alt_hl, entry_ref, creds);
                        assert(ret == 0);
                    }
                    break;
    ...
    PVFS_credentials * creds)
{
    ...
    for (i = 0; i < df_count; i++)
    {
        ...
        if (handlelist_find_handle(hl, df_handles[i], &server_idx) == 0)
        {
            in_main_list = 1;
                 ...
                 && (handlelist_find_handle(alt_hl, df_handles[i], &server_idx)
                     == 0))
        {
            in_alt_list = 1;
        }
    ...
    for (i = 0; i < df_count; i++)
    {
        if (df_handles[i] != PVFS_HANDLE_NULL)
        {
            ...
            if (handlelist_find_handle(hl, df_handles[i], &server_idx) == 0)
            {
                handlelist_remove_handle(hl, df_handles[i], server_idx);
            }
            else
            {
                handlelist_remove_handle(alt_hl, df_handles[i], server_idx);
            }
        }
    }
...
struct handlelist *find_sub_trees(
    PVFS_fs_id cur_fs,
    ...
    PVFS_credentials * creds)
{
    ...
    alt_hl = handlelist_initialize(hl_all->size_array, hl_all->server_ct);
     ...
     */
    while (handlelist_return_handle(hl_all, &handle, &server_idx) == 0)
    {
        ...
        ret =
            PVFS_sys_getattr(handle_ref, PVFS_ATTR_SYS_ALL_NOHINT, creds,
                             &getattr_resp);
        if (ret)
        {
            /* remove anything we can't get attributes on */
            ret = remove_object(handle_ref, 0, creds);
            continue;
        }
        ...
        case PVFS_TYPE_DIRECTORY:
            ...
            descend(cur_fs, hl_all, alt_hl, handle_ref, creds);
        ...
        case PVFS_TYPE_SYMLINK:
            ...
            break;
        default:
            ret = remove_object(handle_ref, getattr_resp.attr.objtype, creds);
            assert(ret == 0);
            break;
        }
...
struct handlelist *fill_lost_and_found(
    PVFS_fs_id cur_fs,
    struct handlelist *hl_all,
    PVFS_BMI_addr_t * addr_array,
    PVFS_credentials * creds)
{
    ...
    while (handlelist_return_handle(hl_all, &handle, &server_idx) == 0)
    {
        ...
        {
            printf
                ("warning: problem calling getattr on %llu; assuming datafile for now.\n",
                 llu(handle));
        ...
        case PVFS_TYPE_METAFILE:
            printf("# trying to salvage %s %lld.\n",
                   get_type_str(getattr_resp.attr.objtype), llu(handle));
            if (verify_datafiles
                (cur_fs, hl_all, alt_hl, handle_ref,
                 getattr_resp.attr.dfile_count, creds) != 0)
            {
                ret =
                    remove_object(handle_ref, getattr_resp.attr.objtype, creds);
                assert(ret == 0);
            }
            else
            {
                sprintf(filename + 9, "%llu", llu(handle));
                ret = create_dirent(laf_ref, filename, handle, creds);
                assert(ret == 0);
            }
            break;
        case PVFS_TYPE_DIRECTORY:
            ...
            if (match_dirdata(hl_all, alt_hl, handle_ref, creds) != 0)
            {
                ret =
                    remove_object(handle_ref, getattr_resp.attr.objtype, creds);
                assert(ret == 0);
            ...
            ret = create_dirent(laf_ref, dirname, handle, creds);
            if (ret != 0)
            {
                ret =
                    remove_object(handle_ref, getattr_resp.attr.objtype, creds);
    ...
    PVFS_credentials * creds)
{
    ...
    ret =
        PVFS_sys_lookup(cur_fs, "/lost+found", creds, &lookup_resp,
                        PVFS2_LOOKUP_LINK_NO_FOLLOW);
    if (ret == 0)
    {
    ...
    {
        ret = PVFS_sys_mkdir("lost+found", root_ref, attr, creds, &mkdir_resp);
        if (ret == 0)
        {
    ...
    PVFS_credentials * creds)
{
    ...
    printf("* %s creating new reference to %s (%llu) in %llu.\n",
           fsck_opts->destructive ? "" : "not", name, llu(handle),
           llu(dir_ref.handle));

    if (fsck_opts->destructive)
    {
        ret = PVFS_mgmt_create_dirent(dir_ref, name, handle, creds);
        if (ret != 0)
        {
    ...
    PVFS_credentials * creds)
{
    ...
    printf
        ("* %s deleting directory entry for missing object %s (%llu) from dir %llu.\n",
         fsck_opts->destructive ? "" : "not", name, llu(entry_ref.handle),
         llu(dir_ref.handle));

    if (fsck_opts->destructive)
    {
        ret = PVFS_mgmt_remove_dirent(dir_ref, name, creds);
        if (ret != 0)
        {
    ...
    PVFS_credentials * creds)
{
    ...
    printf("* %s removing %s %llu.\n",
           fsck_opts->destructive ? "" : "not", get_type_str(obj_type),
           llu(obj_ref.handle));

    if (fsck_opts->destructive)
    {
        ret = PVFS_mgmt_remove_object(obj_ref, creds);
        if (ret != 0)
        {
 ...
 */
static struct handlelist *handlelist_initialize(
    unsigned long *handle_counts,
    int server_count)
{
    int i;
    struct handlelist *hl;
    ...
    do
    {
        ...
        {
            hl->list_array[i] =
                (PVFS_handle *) calloc(handle_counts[i], sizeof(PVFS_handle));
 ...
 * a particular server.
 */
static void handlelist_add_handles(
    struct handlelist *hl,
    PVFS_handle * handles,
    ...
    int server_idx)
{
    ...
    {
        fprintf(stderr,
                "server %d, exceeding number of handles it declared (%ld), currently (%ld)\n",
                server_idx, hl->size_array[server_idx],
                (start_off + handle_count));
        assert(0);
    }
...
static void handlelist_add_handle(
    struct handlelist *hl,
    PVFS_handle handle,
    int server_idx)
{
...
static void handlelist_finished_adding_handles(
    struct handlelist *hl)
{
    int i;
    ...
    for (i = 0; i < hl->server_ct; i++)
    {
        if (hl->used_array[i] != hl->size_array[i])
        {
            printf("warning: only found %ld of %ld handles for server %d.\n",
                   hl->used_array[i], hl->size_array[i], i);
        }
    }
}
 ...
 */
static int handlelist_find_handle(
    struct handlelist *hl,
    PVFS_handle handle,
    int *server_idx_p)
{
...
static void handlelist_remove_handle(
    struct handlelist *hl,
    PVFS_handle handle,
    int server_idx)
{
 ...
 */
static int handlelist_return_handle(
    struct handlelist *hl,
...
static void handlelist_finalize(
    struct handlelist **hlp)
{
    int i;
    ...
    char **argv)
{
    ...
    fprintf(stderr,
            "  -p              automatically repair with no questions\n");
