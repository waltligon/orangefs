				    ...
				    PVFS_credentials *creds)
{
    ...
    /* FIX: need error handling to clean up memory if malloc failure later */
    ...
    ds_position_array = (PVFS_ds_position *) calloc(server_count, 
        sizeof(PVFS_ds_position));
    if (ds_position_array == NULL)
    {
	perror("malloc");
	return NULL;
    }

    kv_position_array = (PVFS_kv_position *) calloc(server_count, 
        sizeof(PVFS_kv_position));
    if (kv_position_array == NULL)
    {
	perror("malloc");
	return NULL;
    }
	...
	for (i=0; i < server_count; i++)
	{
		 ...
		 */
                ret = PINT_cached_config_map_to_server(&tmp_addr, 
                    handle_matrix[i][j], cur_fs);
                if (ret || tmp_addr != addr_array[i])
                {
                    fprintf(stderr, "Ugh! handle does not seem to be owned "
                                    "by the server!\n");
                    return NULL;
                }
	    }
    ...
    for (i = 0; i < server_count; i++)
    {
        ...
        {
            fprintf(stderr, "Ugh! Server %d, Received %ld total handles "
    ...
    free(ds_position_array);
    free(kv_position_array);
		  ...
		  PVFS_credentials *creds)
{
    ...
    printf("# looking for dirdata match to %s.\n",
	   PVFS_handle_to_str(dir_ref.handle));
	    ...
	    PVFS_credentials *creds)
{
    ...
    unsigned int token_flag;
    PVFS_sysresp_readdir readdir_resp;
    PVFS_sysresp_getattr getattr_resp;
    PVFS_object_ref entry_ref;

    entry_ref.fs_id = PVFS_FS_ID_NULL;
    PVFS_handle_clear(entry_ref.handle);
    ...
    do {
        ...
        PVFS_sys_readdir(dir_ref, token, &token_flag,
                         count,
                         creds,
                         &readdir_resp, NULL);
        ...
        for (i = 0; i < readdir_resp.pvfs_dirent_outcount; i++)
        {
            ...
            PVFS_handle_copy(cur_handle, readdir_resp.dirent_array[i].handle);
            cur_file   = readdir_resp.dirent_array[i].d_name;

            PVFS_handle_copy(entry_ref.handle, cur_handle);
                ...
                switch (getattr_resp.attr.objtype)
                {
                    case PVFS_TYPE_METAFILE:
                                             ...
                                             creds) < 0)
                        {
                            ...
                            printf("* File %s (%s) is not recoverable.\n",
                                   cur_file,
                    ...
                    case PVFS_TYPE_DIRECTORY:
                        ...
                        if (ret != 0)
                        {
                            printf("* Directory %s (%s) is missing DirData.\n",
                                   cur_file,
                                   PVFS_handle_to_str(cur_handle));
		     ...
		     PVFS_credentials *creds)
{
	...
	if ((!in_main_list) && (!in_alt_list))
	{
	    printf("# datafile handle %s missing from list\n",
		   PVFS_handle_to_str(df_handles[i]));
	    /* if possible, rebuild the datafile. */
	    /* otherwise delete datafiles, return error to get 
	     * handle and dirent removed.
	     */
	    PVFS_handle_clear(df_handles[i]);
	    error++;
	}
    ...
    for (i = 0; i < df_count; i++)
    {
	if (PVFS_handle_is_null(df_handles[i])) {
				  ...
				  PVFS_credentials *creds)
{
	...
	PVFS_handle_copy(handle_ref.handle, handle);
				       ...
				       PVFS_credentials *creds)
{
	...
	ret = PVFS_sys_getattr(handle_ref,
			       ...
			       &getattr_resp, NULL);
	if (ret) {
	    printf("warning: problem calling getattr on %s; assuming datafile for now.\n",
		   PVFS_handle_to_str(handle));
	...
	switch (getattr_resp.attr.objtype)
	{
		...
		{
		    sprintf(filename + 9, "%s", PVFS_handle_to_str(handle));
		...
		sprintf(dirname + 8, "%s", PVFS_handle_to_str(handle));
		ret = create_dirent(laf_ref,
				    dirname,
				    handle,
				    creds);
	    ...
	    case PVFS_TYPE_DATAFILE:
#if 0
		printf("# saving %s (datafile) for later.\n", PVFS_handle_to_str(handle));
#endif
		handlelist_add_handle(alt_hl, handle, server_idx);
		break;
	    case PVFS_TYPE_DIRDATA:
#if 0
		printf("# saving %s (dirdata) for later.\n", PVFS_handle_to_str(handle));
#endif
		handlelist_add_handle(alt_hl, handle, server_idx);
		break;
	    case PVFS_TYPE_SYMLINK:
		...
		printf("* delete handle %s (unknown type).\n",
		       PVFS_handle_to_str(handle));
		break;
	}
		    ...
		    PVFS_credentials *creds)
{
	...
	ret = PVFS_sys_getattr(handle_ref,
			       ...
			       &getattr_resp, NULL);
	if (ret) {
	    printf("warning: problem calling getattr on %s\n",
		   PVFS_handle_to_str(handle));
			   ...
			   PVFS_credentials *creds)
{
    ...
    printf("* %s deleting directory entry for missing object %s (%s) from dir %s.\n",
	   fsck_opts->destructive ? "" : "not",
	   name,
	   PVFS_handle_to_str(entry_ref.handle),
	   PVFS_handle_to_str(dir_ref.handle));
				   ...
				   int server_idx)
{
	...
	PVFS_handle_copy(hl->list_array[server_idx][start_off + i],
                        handles[i]);
    }
				  ...
				  int server_idx)
{
    ...
    PVFS_handle_copy(hl->list_array[server_idx][start_off], handle);
        ...
        for (i = 0; i < hl->used_array[server_idx]; i++)
        {
            if (hl->list_array[server_idx][i] == handle)
            {
                if (i < (hl->used_array[server_idx] - 1))
                {
                    ...
                    PVFS_handle_copy( hl->list_array[server_idx][i],
                        hl->list_array[server_idx]
                            [hl->used_array[server_idx]-1]);
				     ...
				     int server_idx)
{
	...
	printf("! problem removing %s/%d.\n", PVFS_handle_to_str(handle), server_idx);
    }
				    ...
				    int *server_idx_p)
{
	...
	if (hl->used_array[i] > 0) {
	    PVFS_handle_copy(*handle_p,
                hl->list_array[i][hl->used_array[i]-1]);
	    hl->used_array[i]--;
	    *server_idx_p = i;
	    return 0;
	}
...
static void handlelist_print(struct handlelist *hl)
{
	...
	printf("%s ", PVFS_handle_to_str(hl->list_array[0][i]));
    }
    printf("\n");
}
#endif
