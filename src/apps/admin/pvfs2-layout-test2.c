...
main_out:
    ...
    if (hints != NULL) PVFS_hint_free(hints);
    return(ret);
}
    ...
    int64_t offset, size_t count, PVFS_credentials *credentials)
{
    ...
    PVFS_size ssize = 65536; /* stripe size */
    int i, t1;
    ...
    for(i=0; i < new_count; i++)
    {
        displacements[i] = ssize*i*3;
        displacements0[i] = ssize + ssize*i*3;
        displacements1[i] = ssize*2 + ssize*i*3;
    }
