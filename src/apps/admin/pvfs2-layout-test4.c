...
{
    PVFS_Request mem_req, file_req;
    PVFS_Request file_req0;
    PVFS_Request file_req1;
    PVFS_sysresp_io resp_io;
    int ret;
    int new_count; 
    int num_io_servers;
    int32_t *blocklengths, *blocklengths0, *blocklengths1;
    PVFS_size *displacements, *displacements0, *displacements1;
    PVFS_size ssize = 65536;
    int i, t1;
    double t2 = (double)((double)count/(double)ssize);
    t1 = (count/ssize);
    if (t2 == (double)t1)
        new_count = (int)(t1);
    else
        new_count = (int)(t1+1); /* ceiling */


    ret = PVFS_mgmt_count_servers(dest->u.pvfs2.fs_id, credentials, PVFS_MGMT_IO_SERVER, &num_io_servers);

    blocklengths = malloc(sizeof(int32_t)*new_count);
    blocklengths0 = malloc(sizeof(int32_t)*new_count);
    blocklengths1 = malloc(sizeof(int32_t)*new_count);
    displacements = malloc(sizeof(PVFS_size)*new_count);
    displacements0 = malloc(sizeof(PVFS_size)*new_count);
    displacements1 = malloc(sizeof(PVFS_size)*new_count);

    for(i=0; i < new_count; i++)
    {
        blocklengths[i] = ssize;
        blocklengths0[i] = ssize;
        blocklengths1[i] = ssize;
    }

    blocklengths[new_count-1] = count - ssize*(new_count-1);
    blocklengths0[new_count-1] = count - ssize*(new_count-1);
    blocklengths1[new_count-1] = count - ssize*(new_count-1);

    int i0 = 0, i1 = 1, i2 = 2;
    displacements[0] = ssize*i0;
    displacements0[0] = ssize*i1;
    displacements1[0] = ssize*i2;
    ...
    for(i=1; i < new_count; i++)
    {
        ...
        i1 = i1+2;
        i2 = i2+2;
        if ((i1%num_io_servers == 0) || ((i1-1)%num_io_servers == 0)) i1++;
        if ((i2%num_io_servers == 0) || ((i2-1)%num_io_servers == 0)) i2++;
        displacements[i] = i0*ssize;
        displacements0[i] = i1*ssize;
        displacements1[i] = i2*ssize;
        //printf("i0=%d, i1=%d, i2=%d\n", i0, i1, i2);        
    }
    ...
    {
        ret = PVFS_Request_hindexed(new_count, blocklengths, displacements, PVFS_BYTE, &file_req);
        if (ret < 0)
        {
            PVFS_perror("PVFS_Request_hvector", ret);
	    return(ret);
        }
        ret = PVFS_Request_hindexed(new_count, blocklengths0, displacements0, PVFS_BYTE, &file_req0);
        if (ret < 0)
        {
            PVFS_perror("PVFS_Request_hvector", ret);
	    return(ret);
        }
        ret = PVFS_Request_hindexed(new_count, blocklengths1, displacements1, PVFS_BYTE, &file_req1);
        if (ret < 0)
        {
            PVFS_perror("PVFS_Request_hvector", ret);
	    return(ret);
	...
	if (ret == 0) 
        ...
        /* write the first remote copy */
        ret = PVFS_sys_write(dest->u.pvfs2.ref, file_req0, offset,
		buffer, mem_req, credentials, &resp_io, hints);
	if (ret == 0) 
        {
            //PVFS_Request_free(&mem_req);
	    ...
	    PVFS_perror("PVFS_sys_write", ret);
        /* write the second remote copy */
        ret = PVFS_sys_write(dest->u.pvfs2.ref, file_req1, offset,
		buffer, mem_req, credentials, &resp_io, hints);
	if (ret == 0) 
        {
            PVFS_Request_free(&mem_req);
            PVFS_Request_free(&file_req);
            PVFS_Request_free(&file_req0);
            PVFS_Request_free(&file_req1);
            free(blocklengths);
            free(blocklengths0);
            free(blocklengths1);
            free(displacements);
	...
	if (open_type == OPEN_DEST)
	{
		...
		make_attribs(&(obj->u.pvfs2.attr), credentials, nr_datafiles,
		       ...
		       return -1; 
                ...
                local_layout = malloc (sizeof(PVFS_sys_layout));
                local_layout->algorithm = PVFS_SYS_LAYOUT_LOCAL_RR;
		ret = PVFS_sys_create(entry_name, parent_ref, 
                                      obj->u.pvfs2.attr, credentials,
                                      new_dist, &resp_create, local_layout, hints);
		if (ret < 0)
		{
