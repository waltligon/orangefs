    ...
    char **argv)
{
    ...
    ret =
        PVFS_util_resolve(user_opts.pszLinkName, &fs_id, szPvfsPath,
                          PVFS_NAME_MAX);
    ...
    ret =
        make_link(&credentials, fs_id, user_opts.pszLinkTarget, szPvfsPath,
                  user_opts.nVerbose);
    if (ret != 0)
    {
        fprintf(stderr, "cannot create softlink [%s] to [%s]\n",
                user_opts.pszLinkName, user_opts.pszLinkTarget);
        return (-1);
    }
    ...
    const int nVerbose)
{
     ...
     */
    ret =
        PINT_remove_base_dir((char *) pszPvfsPath, szBaseName,
                             sizeof(szBaseName));
     ...
     */
    ret =
        PINT_lookup_parent((char *) pszPvfsPath, fs_id, pCredentials,
                           &parent_ref.handle);
    ...
    ret =
        PVFS_sys_symlink(szBaseName, parent_ref, (char *) pszLinkTarget, attr,
                         pCredentials, &resp_sym);
...
static int parse_args(
    ...
    struct options *opts)
{
    int ret = 0, option_index = 0, create_softlink = 0;
    ...
    while ((ret =
            getopt_long_only(argc, argv, flags, long_opts,
                             &option_index)) != -1)
    {
        switch (ret)
        {
    ...
    {
        fprintf(stderr,
                "Hard links not supported. Use -s option for softlinks\n");
        usage(argc, argv);
        exit(1);
    }
