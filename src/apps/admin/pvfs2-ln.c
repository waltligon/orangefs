...
static void enable_symbolic(struct options * opts);
...
int main(int argc, char **argv)
{
    ...
    if(ret != 0)
    {
        fprintf(stderr, "cannot create link [%s] to [%s]\n",
                user_opts.pszLinkName,
                     ...
                     const char           * pszPvfsPath)
{
    ...
    if (!opts->nSymbolic)
    {
        ...
        ret = PVFS_sys_lookup(
            ...
            pCredentials,
            &resp_lookup,
            0, // follow_link, could be an option for ln, later
            NULL);

        if(ret < 0)
        {
            fprintf(stderr, "Could not locate target.\n");
            return(-1);
        }

        ret = PVFS_sys_hardlink(szBaseName,
            parent_ref,
            resp_lookup.ref.handle,
            attr,
            pCredentials,
            &resp,
            NULL);
    ...
    if (ret < 0)
    {
        fprintf(stderr, "Unable to create link.\n");
        return(ret);
    }
    ...
    while((ret = getopt_long_only(argc, argv, flags, long_opts, &option_index)) != -1)
    {
        switch (ret)
        {
            case 0:
                ...
                else if (strcmp("symbolic", cur_option) == 0)
                {
                    enable_symbolic(opts);
                }
...
static void usage(int argc, char **argv)
{
    ...
    fprintf(stderr,"  -s, --symbolic      create softlink\n");
