    ...
    struct options *opts)
{
    ...
    PVFS_size size = 0;
    char scratch_owner[16] = { 0 }, scratch_group[16] =
    {
    0};
    char scratch_size[16] = { 0 }, scratch_inode[16] =
    {
    0};
    char f_type = '-';
    char group_x_char = '-';
    ...
    else if ((attr->objtype == PVFS_TYPE_SYMLINK) && (attr->link_target))
    {
    ...
    snprintf(buf, 128, "%s%c%c%c%c%c%c%c%c%c%c    1 %s %s %s "
             ...
             (time->tm_hour), (time->tm_min), entry_name);
    ...
    struct options *opts)
{
    ...
    ret =
        PVFS_sys_lookup(fs_id, name, &credentials, &lk_response,
    ...
    memset(&getattr_response, 0, sizeof(PVFS_sysresp_getattr));
    if (PVFS_sys_getattr
        (ref, PVFS_ATTR_SYS_ALL_NOHINT, &credentials, &getattr_response) == 0)
    {
             ...
             && (opts->list_directory)))
        {
            ...
            {
                if (PVFS_sys_getparent
                    (ref.fs_id, name, &credentials, &getparent_resp) == 0)
                {
                    print_dot_and_dot_dot_info_if_required(getparent_resp.
                                                           parent_ref);
                }
            }
            ...
            if (opts->list_long)
            {
                print_entry_attr(ref.handle, segment, &getattr_response.attr,
                                 opts);
            }
            else
            {
    ...
    char *argv[])
{
    ...
    while ((ret =
            getopt_long(argc, argv, "hVndGoAaigl", long_opts,
                        &option_index)) != -1)
    {
        switch (ret)
        {
    ...
    char **argv)
{
    ...
    fprintf(stderr,
            "  -d, --directory            list directory "
            "entries instead of contents\n");
    fprintf(stderr,
            "  -g                         like -l, but do " "not list owner\n");
    fprintf(stderr,
            "  -G, --no-group             inhibit display "
            "of group information\n");
    fprintf(stderr,
            "  -h, --human-readable       print sizes in human "
            "readable format\n\t\t\t\t(e.g. 1K 234M 2G)\n");
    fprintf(stderr,
            "      --si                   likewise, but use powers "
            "of 1000, not 1024\n");
    fprintf(stderr,
            "  -i, --inode                print index number "
            "of each file\n");
    fprintf(stderr,
            "  -l                         use a long listing " "format\n");
    fprintf(stderr,
            "  -n, --numeric-uid-gid      like -l, but list "
    ...
    fprintf(stderr,
            "  -V, --verbose              reports if the dir is "
    ...
    char **argv)
{
    ...
    {
        fprintf(stderr,
                "Error: could not initialize any file systems " "from %s\n",
                tab->tabfile_name);
        PVFS_sys_finalize();
        return (-1);
    }
    ...
    {
        ret =
            PVFS_util_resolve(user_opts->start[i], &fs_id_array[i],
                              pvfs_path[i], PVFS_NAME_MAX);
