...
static int src_get_version(
    ...
    char *ver_string,
    int ver_string_max);
static int remove_collection_entry(
    char *storage_space,
    char *collname);

int migrate_collection(
    void *config,
    void *sconfig);
void fs_config_dummy_free(
    void *);
int recursive_rmdir(
    char *dir);
...
static int translate_dist_0_0_1(
    PINT_dist * dist);
static int translate_keyval_key_0_0_1(
    TROVE_keyval_s * keyval,
    DBT * db_key);
    ...
    char **argv)
{
    ...
    else
    {
        ...
        PVFS_fs_id fs_id =
            PINT_config_get_fs_id_by_fs_name(&server_config, opts.fs);
        fs_configs = PINT_llist_new();
        PINT_llist_add_to_head(fs_configs,
                               (void *) PINT_config_find_fs_id(&server_config,
                                                               fs_id));
    }
    ...
    void *sconfig)
{
    ...
    ret =
        src_get_version(server_config->storage_path, fs_config->coll_id,
                        fs_config->file_system_name, version, 254);
    if (ret < 0)
    {
        fprintf(stderr,
    ...
    {
        sprintf(old_coll_path, "%s/%08x-old-%s", server_config->storage_path,
        ...
        ret = translate_0_0_1(server_config->storage_path, old_coll_path,
                              fs_config->file_system_name, fs_config->coll_id);
        if (ret < 0)
        {
            fprintf(stderr,
        ...
        if (opts.cleanup_set)
        {
            ...
            while ((next_dirent = readdir(storage_dir)) != NULL)
            {
                ...
                if (collname_length < d_namelen
                    && strncmp(next_dirent->d_name, collname,
                               collname_length) == 0)
                {
                    char old_coll_path[PATH_MAX];

                    sprintf(old_coll_path, "%s/%s", server_config->storage_path,
                            next_dirent->d_name);
    ...
    char *progname) /**< executable name */
{
    ...
    fprintf(stderr, "--------------\n");
    fprintf(stderr,
            "  --fs=<fs name>     " "name of the file system to migrate\n");
    fprintf(stderr,
            "  --all              "
    ...
    fprintf(stderr, "  --help             " "show this help listing\n");
    fprintf(stderr,
    ...
    int ver_string_max)        /**< maximum size of version string */
{
    ...
    ret =
        src_get_version_0_0_1(storage_space, coll_id, ver_string,
                              ver_string_max);
    ...
    int ver_string_max)    /**< maximum size of version string */
{
    ...
    sprintf(coll_db, "%s/%08x/collection_attributes.db", storage_space,
            coll_id);
    ...
    TROVE_coll_id coll_id) /**< collection id in string format */
{
    ...
    ret =
        pvfs2_mkspace(storage_space, coll_name, coll_id, TROVE_HANDLE_NULL,
                      handle_range, NULL, 1, 0);
    if (ret != 0)
    {
    ...
    if (ret < 0)
    {
        ...
        if (verbose)
            printf("VERBOSE Destroying temporary collection.\n");
    ...
    ret =
        translate_coll_eattr_0_0_1(old_coll_path, coll_id, coll_name,
                                   trove_context);
    if (ret < 0)
    {
        fprintf(stderr,
                "Error: failed to migrate collection extended attributes.\n");
        if (verbose)
            printf("VERBOSE Destroying temporary collection.\n");
        pvfs2_rmspace(storage_space, coll_name, coll_id, 1, 0);
        return (-1);
    ...
    ret =
        translate_dspace_attr_0_0_1(old_coll_path, coll_id, coll_name,
                                    trove_context);
    if (ret < 0)
    {
    ...
    ret =
        translate_keyvals_0_0_1(old_coll_path, coll_id, coll_name,
                                trove_context);
    if (ret < 0)
    {
    ...
    /* convert bstreams */
    ret =
        translate_bstreams_0_0_1(storage_space, old_coll_path, coll_id,
    ...
    if (!opts.cleanup_set)
    {
        ...
        printf
            ("Please delete the old collection once you have tested and confirmed\n");
        printf("the results of the migration.\n");
        printf
            ("Command: \"pvfs2-migrate-collection -cleanup <fs config> <server config>\"\n");
        printf
            ("===================================================================\n");
    }
    ...
    char *collname)
{
    ...
    ret = dbp->open(dbp,
                    ...
                    NULL,
#endif
                    collections_db, NULL, DB_UNKNOWN, 0, 0);
    if (ret != 0)
    {
        fprintf(stderr, "Error: dbp->open: %s.\n", db_strerror(ret));
        return (-1);
    ...
    TROVE_context_id trove_context) /**< open trove context */
{
    ...
    do
    {
        ...
        {
            if (verbose)
                printf("VERBOSE Migrating collection eattr: %s\n",
                       (char *) key.data);
            ...
            state = 0;
            ret =
                trove_collection_seteattr(coll_id, &t_key, &t_val, 0, NULL,
                                          trove_context, &op_id);
            while (ret == 0)
            {
                ret =
    ...
    TROVE_context_id trove_context) /**< open trove context */
{
    ...
    do
    {
        ...
        if (ret == 0)
        {
            ...
            if (verbose)
                printf
                    ("VERBOSE Migrating attributes for handle: %llu, type: %d\n",
            ...
            state = 0;
            ret = trove_dspace_create(coll_id, &extent_array, &new_handle,
                                      tmp_attr->type, NULL,
                                      (TROVE_SYNC |
                                       TROVE_FORCE_REQUESTED_HANDLE), NULL,
                                      trove_context, &op_id);
            ...
            state = 0;
            ret =
                trove_dspace_setattr(coll_id, *tmp_handle, &new_attr,
    ...
    for (i = 0; i < KEYVAL_MAX_NUM_BUCKETS_0_0_1; i++)
    {
        ...
        while ((tmp_ent = readdir(tmp_dir)))
        {
            if (strcmp(tmp_ent->d_name, ".") && strcmp(tmp_ent->d_name, ".."))
            {
                ...
                ret =
                    sscanf(tmp_ent->d_name, "%llx.keyval", &(llu(tmp_handle)));
#elif SIZEOF_LONG_INT == 8
                ret = sscanf(tmp_ent->d_name, "%lx.keyval", &(tmp_handle));
#else
#error Unexpected sizeof(long int)
                ...
                ret =
                    translate_keyval_db_0_0_1(coll_id, keyval_db, tmp_handle,
                                              coll_name, trove_context);
                if (ret < 0)
                {
    ...
    TROVE_context_id trove_context) /**< open trove context */
{
    ...
    ret = dbp->open(dbp,
                    ...
                    NULL,
#endif
                    full_db_path, NULL, DB_UNKNOWN, 0, 0);
    if (ret != 0)
    {
        fprintf(stderr, "Error: dbp->open: %s.\n", db_strerror(ret));
        return (-1);
    ...
    do
    {
        ...
        if (ret == 0)
        {
            ...
            state = 0;
            ret =
                trove_keyval_write(coll_id, handle, &t_key, &t_val, trove_flags,
    ...
    TROVE_context_id trove_context) /**< open trove context */
{
    ...
    {
        snprintf(bucket_dir, PATH_MAX, "%s/bstreams/%.8d", old_coll_path, i);
        ...
        while ((tmp_ent = readdir(tmp_dir)))
        {
            ...
            {
                if (verbose)
                    printf("VERBOSE Migrating bstream: %s.\n", tmp_ent->d_name);
