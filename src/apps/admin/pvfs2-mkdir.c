    ...
    char **argv)
{
    ...
    {
        ret =
            PVFS_util_resolve(user_opts.dir_array[i], &pfs_id[i], pvfs_path[i],
                              PVFS_NAME_MAX);
    ...
    for (i = 0; i < user_opts.numdirs; i++)
    {
        ret = make_directory(&credentials,
                             pfs_id[i],
                             user_opts.mode,
                             user_opts.dir_array[i], pvfs_path[i],
                             user_opts.make_parent_dirs, user_opts.verbose);
        if (ret != 0)
        {
    ...
    const int verbose)
{
    ...
    ret =
        PVFS_sys_lookup(fs_id, parentdir_ptr, credentials, &resp_lookup,
    ...
    if (ret == -PVFS_ENOENT && make_parent_dirs)
    {
        ...
        ret = make_directory(credentials,
                             fs_id, mode, dirname(realpath),
                             dirname(parent_dir), make_parent_dirs, verbose);
    ...
    ret =
        PVFS_sys_mkdir(basename_ptr, parent_ref, attr, credentials,
                       &resp_mkdir);
    ...
    struct options *opts)
{
    ...
    {
        fprintf(stderr,
                "Directory limit of [%d] exceeded. [%d] directories entered\n",
                MAX_NUM_DIRECTORIES, opts->numdirs);
        usage(argc, argv);
        return (-1);
    }
