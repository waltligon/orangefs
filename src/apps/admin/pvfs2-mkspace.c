    ...
    options_t * opts)
{
    ...
    while ((ret =
            getopt_long(argc, argv, "s:c:i:r:vVhadDM:N:", long_opts,
                        &option_index)) != -1)
    {
        switch (ret)
        {
        ...
        case 'r':
          do_root_handle:
#ifdef HAVE_STRTOULL
            opts->root_handle = (PVFS_handle) strtoull(optarg, NULL, 10);
#else
            opts->root_handle = (PVFS_handle) strtoul(optarg, NULL, 10);
#endif
            break;
        case 'M':
    ...
    options_t * opts)
{
    if (opts)
    {
        ...
        printf("\tcollection name     : %s\n",
               (strlen(opts->collection) ? opts->
                collection : "None specified"));
        printf("\tmeta handle ranges  : %s\n",
               (strlen(opts->meta_ranges) ? opts->
                meta_ranges : "None specified"));
        printf("\tdata handle ranges  : %s\n",
               (strlen(opts->data_ranges) ? opts->
                data_ranges : "None specified"));
        printf("\tstorage space       : %s\n",
               (strlen(opts->storage_space) ? opts->
                storage_space : "None specified"));
    }
}
    ...
    options_t * opts)
{
    ...
    fprintf(stderr,
            "The following arguments can be used to "
    ...
    fprintf(stderr,
            "  -r, --root-handle=HANDLE             "
            "create collection with this root handle\n");
    fprintf(stderr,
            "  -M, --meta-handle-range=RANGE        "
            "create collection with the specified\n        "
            "                                meta handle range\n");
    fprintf(stderr,
            "  -N, --data-handle-range=RANGE        "
            "create collection with the specified\n        "
            "                                data handle range\n");
    fprintf(stderr,
            "  -s, --storage-space=PATH             "
    ...
    char **argv)
{
    ...
    if ((strlen(opts.meta_ranges) == 0) && (strlen(opts.data_ranges) == 0))
    {
        fprintf(stderr,
                "Error: You MUST specify either a meta or "
    ...
    {
        ret =
            pvfs2_rmspace(opts.storage_space, opts.collection, opts.coll_id,
