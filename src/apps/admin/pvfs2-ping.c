...
static void print_mntent(
    struct PVFS_sys_mntent *entries,
    int num_entries);
...
static void print_root_check_error_details(
    PVFS_error_details * error_details);
    ...
    char **argv)
{
    ...
    ret =
        PVFS_util_resolve(user_opts->fs_path_hack, &cur_fs, pvfs_path,
                          PVFS_NAME_MAX);
    if (ret < 0)
    {
    ...
    printf("\n(6) Verifying that fsid %ld is acceptable " "to all servers...\n",
           (long) cur_fs);
     ...
     */
    ret =
        PVFS_mgmt_setparam_all(cur_fs, &creds, PVFS_SERV_PARAM_FSID_CHECK,
    ...
    ret =
        PVFS_sys_lookup(cur_fs, "/", &creds, &resp_lookup,
    ...
    ret = PVFS_mgmt_setparam_all(cur_fs, &creds, PVFS_SERV_PARAM_ROOT_CHECK,
                                 (uint64_t) resp_lookup.ref.handle, NULL,
                                 error_details);
    ...
    if (err)
    {
        printf("\nThe PVFS2 filesystem at %s appears to have " "problems.\n\n",
               user_opts->fs_path_real);
    }
    else
    {
