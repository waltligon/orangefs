    ...
    char **argv)
{
    ...
    if (user_opts->single_server)
    {
        ...
        ret =
            PVFS_mgmt_setparam_single(cur_fs, &creds,
                                      PVFS_SERV_PARAM_GOSSIP_MASK,
                                      user_opts->debug_mask,
                                      user_opts->single_server, NULL, NULL);
    }
    else
    {
        ...
        ret =
            PVFS_mgmt_setparam_all(cur_fs, &creds, PVFS_SERV_PARAM_GOSSIP_MASK,
    ...
    char **argv)
{
    ...
    while ((ret =
            getopt_long(argc, argv, "hvm:s:", long_opts, &option_index)) != -1)
    {
        switch (ret)
        {
        case 0:
    ...
    char **argv)
{
    ...
    fprintf(stderr,
            ...
            "information and exit\n");
    fprintf(stderr,
            "-m, --mount=<mount_point>   use the specified " "mount point\n");
    fprintf(stderr,
            "-s, --server=<server_addr>  use the specified "
            "server address\n");
    fprintf(stderr, "\nExamples:\n  %s -m /mnt/pvfs2 \"network,server" "\"\n",
            argv[0]);
    fprintf(stderr,
            "\t[ enables network and server debugging on all " "servers ]\n\n");
    fprintf(stderr,
            "  %s -m /mnt/pvfs2 \"none\" -s \"tcp://localhost:"
            "3334/pvfs2-fs\"\n", argv[0]);
    fprintf(stderr,
            "\t[ disable debugging on the specified server only " " ]\n\n");
