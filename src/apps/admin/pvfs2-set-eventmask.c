    ...
    char **argv)
{
    ...
    if (!user_opts->op_mask || !user_opts->api_mask)
    {
        ...
        ret =
            PVFS_mgmt_setparam_all(cur_fs, &creds, PVFS_SERV_PARAM_EVENT_ON, 0,
                                   NULL, NULL);
    }
    else
    {
        ...
        ret = PVFS_mgmt_setparam_all(cur_fs, &creds,
                                     ...
                                     (int64_t) (((int64_t) user_opts->
                                                 op_mask << 32) +
                                                user_opts->api_mask), NULL,
                                     NULL);
        if (ret < 0)
        {
        ...
        ret =
            PVFS_mgmt_setparam_all(cur_fs, &creds, PVFS_SERV_PARAM_EVENT_ON, 1,
                                   NULL, NULL);
    }
