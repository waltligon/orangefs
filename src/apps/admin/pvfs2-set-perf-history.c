...
int main(int argc, char **argv)
{
    ...
    param_value.u.value = user_opts->history;
    ...
    if (user_opts->server)
    {
       ...
       {
          fprintf(stderr,
                  "Error(%d) setting history on server(%s)\n",
                  ret,
                  user_opts->server);
          goto out;
       }        
       ...
       {
          fprintf(stderr,
                  "Successfully set history(%d) on server (%s)\n",
                  user_opts->history,
                  user_opts->server);
       }
    }
    else
    {
        ...
        {
           fprintf(stderr,
                   "Error(%d) setting history for mount point(%s)\n",
                   ret,
                   user_opts->mnt_point);
           goto out;
        }
        ...
        {
           fprintf(stderr,
                   "Successfully set history (%d) for mount point(%s)\n",
                   user_opts->history,
                   user_opts->mnt_point);
        }
    }
...
static void usage(int argc, char** argv)
{
    fprintf(stderr, "\n");
    fprintf(stderr,
            "Usage  : %s [-s server] -m <filesystem mount point>  <history>\n\n",
	    argv[0]);
    fprintf(stderr,
            "Mount point and history are required. "
            "If server is given, then history will be set only on that server; "
            "otherwise, history is set on "
            "all servers for the given mount point.\n\n");
    fprintf(stderr,
            "Example:All-Severs: %s -m /mnt/pvfs2 5\n\n",
	    argv[0]);
    fprintf(stderr,
            "Example:One-Server: %s -s tcp://localhost:3334/pvfs2-fs -m /mnt/pvfs2 10\n\n",argv[0]);
    fprintf(stderr, "History is an integer greater than 0\n");
