...
static struct options* parse_args(int argc, char* argv[])
{
	...
	switch(one_opt)
		...
		break;
	    case('p'):
		sscanf(optarg, "%d", &tmp_opts->param);
		if(ret < 1){
		    if(tmp_opts->mnt_point) free(tmp_opts->mnt_point);
		    free(tmp_opts);
		    return(NULL);
		}
		tmp_opts->param_set = 1;
		break;
	    case('s'):
		sscanf(optarg, "%x", &tmp_opts->value);
		if(ret < 1){
		    if(tmp_opts->mnt_point) free(tmp_opts->mnt_point);
		    free(tmp_opts);
		    return(NULL);
		}
		tmp_opts->value_set = 1;
		break;
	    case('?'):
		usage(argc, argv);
		exit(EXIT_FAILURE);
    ...
    if(!tmp_opts->mnt_point_set || !tmp_opts->param_set ||
	!tmp_opts->value_set)
    {
	if(tmp_opts->mnt_point) free(tmp_opts->mnt_point);
	free(tmp_opts);
...
static void usage(int argc, char** argv)
{
    fprintf(stderr, 
            ...
            "on all the servers\n\n"
            "Usage  : %s [-m fs_mount_point] "
            "[-p param number] [-s value]\n", argv[0]);
    fprintf(stderr, "Example: %s -m /mnt/pvfs2 -p 7 -s 0x00\n",
            argv[0]);
    return;
}
