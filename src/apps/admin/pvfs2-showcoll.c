...
static int verbose = 0, got_collection = 0, print_keyvals =
    0, got_dspace_handle = 0;
TROVE_handle dspace_handle;

static int parse_args(
    int argc,
    char **argv);
static int print_collections(
    void);
static int print_dspaces(
    TROVE_coll_id coll_id,
    TROVE_handle root_handle,
    TROVE_context_id trove_context,
    int no_root_handle);
static int print_dspace(
    TROVE_coll_id coll_id,
    TROVE_handle handle,
    TROVE_context_id trove_context);
static int print_dspace_keyvals(
    TROVE_coll_id coll_id,
    TROVE_handle handle,
    TROVE_context_id trove_context,
    TROVE_ds_type type);
static char *type_to_string(
    TROVE_ds_type type);
static int print_keyval_pair(
    TROVE_keyval_s * key,
    TROVE_keyval_s * val,
    TROVE_ds_type type,
    int sz);
static void print_object_attributes(
    struct PVFS_object_attr *a_p);
static void print_datafile_handles(
    PVFS_handle * h_p,
    int sz);
    ...
    char **argv)
{
    ...
    if (ret < 0)
    {
        fprintf(stderr, "%s: error: argument parsing failed; aborting!\n",
                argv[0]);
        return -1;
    }
    ...
    if (ret < 0)
    {
        fprintf(stderr, "%s: error: trove initialize failed; aborting!\n",
                argv[0]);
        return -1;
    }

    if (verbose)
        fprintf(stderr, "%s: info: initialized with storage space '%s'.\n",
                argv[0], storage_space);
    ...
    if (!got_collection)
    {
        ...
        if (ret != 0)
        {
            fprintf(stderr, "%s: error: collection iterate failed; aborting!\n",
                    argv[0]);
            trove_finalize();
            return -1;
        }
    ...
    if (verbose)
        fprintf(stderr, "%s: info: found collection '%s'.\n", argv[0],
                collection);
    ...
    ret =
        trove_collection_geteattr(coll_id, &key, &val, 0, NULL, trove_context,
                                  &op_id);
    ...
    {
        ret =
            print_dspaces(coll_id, root_handle, trove_context, no_root_handle);
    }
    ...
    char **argv)
{
    ...
    while ((c = getopt(argc, argv, "s:c:d:kvh")) != EOF)
    {
        ...
        case 'h':
            ...
            fprintf(stderr,
                    "\tdefault storage space is '/tmp/pvfs2-test-space'.\n");
            fprintf(stderr, "\t'-v' turns on verbose output.\n");
            fprintf(stderr, "\t'-k' prints data in keyval spaces.\n");
            fprintf(stderr,
                    "\t'-d' prints data for a single dspace only, given a handle (in hex).\n");
    ...
    int no_root_handle)
{
    ...
    while (count > 0)
    {
        ...
        ret =
            trove_dspace_iterate_handles(coll_id, &pos, harray, &count,
                                         0 /* flags */ ,
                                         NULL /* vtag */ ,
                                         NULL /* user ptr */ ,
                                         trove_context, &op_id);
        while (ret == 0)
            ret =
                trove_dspace_test(coll_id, op_id, trove_context, &opcount, NULL,
    ...
    TROVE_context_id trove_context)
{
    ...
    ret = trove_dspace_getattr(coll_id, handle, &ds_attr, 0 /* flags */ ,
                               NULL /* user ptr */ ,
                               trove_context, &op_id);
    while (ret == 0)
    {
    ...
    fprintf(stdout,
            ...
            llu(handle), type_to_string(ds_attr.type), lld(ds_attr.b_size));

    if (print_keyvals)
    {
        ret =
            print_dspace_keyvals(coll_id, handle, trove_context, ds_attr.type);
        if (ret != 0)
            return -1;
    }
    ...
    TROVE_ds_type type)
{
    ...
    while (count > 0)
    {
        int opcount;
        ret =
            trove_keyval_iterate(coll_id, handle, &pos, &key, &val, &count,
                                 0 /* flags */ ,
                                 NULL /* vtag */ ,
                                 NULL /* user ptr */ ,
                                 trove_context, &op_id);
    ...
    struct PVFS_object_attr *a_p)
{
    fprintf(stdout,
            ...
            a_p->owner, a_p->group, a_p->perms, type_to_string(a_p->objtype));
}
    ...
    int count)
{
    ...
    for (i = 0; i < count && i < 10; i++)
        fprintf(stdout, "0x%08llx ", llu(h_p[i]));
    ...
    int sz)
{
    ...
    if (isprint(((char *) key_p->buffer)[0])
        && (strnlen(key_p->buffer, sz) < 64))
        key_printable = 1;
    if (isprint(((char *) val_p->buffer)[0])
        && (strnlen(val_p->buffer, sz) < 64))
        val_printable = 1;
        ...
        && val_p->read_sz == sizeof(struct PVFS_object_attr))
    {
        fprintf(stdout,
                ...
                (char *) key_p->buffer, key_p->read_sz, (char *) val_p->buffer,
                val_p->read_sz);
    ...
    else if (type == PVFS_TYPE_DIRECTORY && !strncmp(key_p->buffer, "de", 3))
    {
        fprintf(stdout,
                ...
                (char *) key_p->buffer,
                key_p->read_sz, (char *) val_p->buffer, val_p->read_sz,
    ...
    {
        fprintf(stdout, "\t\t'%s' (%d): <data> (%d)\n", (char *) key_p->buffer,
                key_p->read_sz, val_p->read_sz);
    }
    else
    {
        fprintf(stdout, "\t\t<data> (%d): <data> (%d)\n", key_p->read_sz,
                val_p->read_sz);
    }
    return 0;
}
    ...
    void)
{
    ...
    {
        ret =
            trove_collection_iterate(&pos, &name, &coll_id, &count,
                                     0 /* flags */ ,
                                     0 /* vtag */ ,
                                     NULL /* user ptr */ ,
                                     &op_id);
