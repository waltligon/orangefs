...
static void enable_verbose(
    struct options *opts);
static void enable_dereference(
    struct options *opts);
static int do_stat(
    const char *pszFile,
    ...
    char **argv)
{
    ...
    {
        ret =
            PVFS_util_resolve(user_opts.pszFiles[i], &pfs_id[i],
                              ppszPvfsPath[i], PVFS_NAME_MAX);
    ...
    {
        ret =
            do_stat(user_opts.pszFiles[i], ppszPvfsPath[i], pfs_id[i],
                    &credentials, &user_opts);
        if (ret != 0)
        {
    ...
    const struct options *opts)
{
    ...
    if (opts->nFollowLink)
    {
        ret = PVFS_sys_lookup(fs_id,
                              ...
                              (PVFS_credentials *) credentials, &lk_response,
    ...
    print_stats(&ref, pszFile, pszRelativeFile, &(getattr_response.attr));
    ...
    struct options *opts)
{
    ...
    while ((ret =
            getopt_long_only(argc, argv, "VL", long_opts, &option_index)) != -1)
    {
        switch (ret)
        {
        case 0:
    ...
    const PVFS_sys_attr * attr)
{
    ...
    if (attr->mask & PVFS_ATTR_SYS_SIZE)
    {
         ...
         */
        if ((attr->size == 0) && (attr->objtype & PVFS_TYPE_DIRECTORY))
        {
