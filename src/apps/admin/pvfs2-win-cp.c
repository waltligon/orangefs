...
typedef struct win_file_object_s {
    FILE *f;
    int mode;
...
/* fs tab */
extern PVFS_util_tab s_stat_tab_array[8];
extern int s_stat_tab_count;

static PVFS_hint hints = NULL;

static int init_credential(PVFS_credential *cred);
...
int main(int argc, char ** argv)
{
    ...
    ret = init_credential(&credentials);
    if (ret < 0)
    {
    ...
    time1 = Wtime();
    while((current_size = generic_read(&src, (char *) buffer,
                    total_written, user_opts->buf_size, &credentials)) > 0)
    {
        buffer_size = current_size;

        ret = generic_write(&dest, (char *) buffer, total_written,
 ...
 * init_credential()
 *
 * initializes credential to the root user
 *
 */
static int init_credential(PVFS_credential *cred)
{    
    ...
    ret = gethostname(cred->issuer+2, 254);
    /* TODO: ret */

    cred->userid = 0;

    cred->group_array = (PVFS_gid *) calloc(sizeof(PVFS_gid), 1);
...
static struct options* parse_args(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            printf("OrangeFS %d.%d.%d\n", PVFS2_VERSION_MAJOR, PVFS2_VERSION_MINOR,
                PVFS2_VERSION_SUB);
            exit(0);
        }
        else if (!_stricmp(argv[i], "-t"))
        {
            tmp_opts->show_timings = 1;
        }
        else if (!_stricmp(argv[i], "-s"))
        {            
            ret = sscanf(argv[++i], SCANF_lld, (SCANF_lld_type *)&tmp_opts->strip_size);
            if(ret < 1){
                free(tmp_opts);
                return(NULL);
            }
        ...
        {
            ret = sscanf(argv[++i], "%d", &tmp_opts->num_datafiles);
            if(ret < 1){
                free(tmp_opts);
                return(NULL);
            }
        ...
        {
            ret = sscanf(argv[++i], "%d", &tmp_opts->buf_size);
            if(ret < 1){
                free(tmp_opts);
                return(NULL);
            }            
        }
        else if (!_stricmp(argv[i], "-h") || !_stricmp(argv[i], "-?"))
        {
            usage(argc, argv);
            exit(0);
        }
...
static void print_timings( double time, int64_t total)
{
    printf("Wrote %lld bytes in %f seconds. %1.3f MB/seconds\n",
        ...
        int64_t offset, size_t count, PVFS_credential *credentials)
{
    ...
    if(src->fs_type == WIN_FILE)
        return (fread(buffer, 1, count, src->u.wfs.f));
    else
    {
        file_req = PVFS_BYTE;
    ...
    int64_t offset, size_t count, PVFS_credential *credentials)
{
    ...
    if (dest->fs_type == WIN_FILE)
        return(fwrite(buffer, 1, count, dest->u.wfs.f));
    else
    {
        file_req = PVFS_BYTE;
        ret = PVFS_Request_contiguous(count, PVFS_BYTE, &mem_req);
...
{
    int i, j, found = 0;
    char mnt_dir[PVFS_NAME_MAX];

    /* search tab array for matching mount point */
    for (i = 0; i < s_stat_tab_count; i++)
    {
        for (j = 0; j < s_stat_tab_array[i].mntent_count; j++)
        {
            if (!strncmp(s_stat_tab_array[i].mntent_array[j].mnt_dir, filename, 
                strlen(s_stat_tab_array[i].mntent_array[j].mnt_dir)))
            {
                strcpy(mnt_dir, s_stat_tab_array[i].mntent_array[j].mnt_dir);
                obj->u.pvfs2.fs_id = s_stat_tab_array[i].mntent_array[j].fs_id;
                found = 1;
                break;
            }
        }
    ...
    if (!found)
    {
        obj->fs_type = WIN_FILE;
        strncpy(obj->u.wfs.path, filename, NAME_MAX);
    } else {
        obj->fs_type = PVFS2_FILE;
        strncpy(obj->u.pvfs2.user_path, filename, PVFS_NAME_MAX);
        /* remove mount dir for pvfs2 path */
        strncpy(obj->u.pvfs2.pvfs2_path, filename+strlen(mnt_dir), PVFS_NAME_MAX);
    }
                        ...
                        char *srcname, int open_type)
{
    ...
    char fname[NAME_MAX], ext[NAME_MAX], filename[NAME_MAX*2];
    ...
    if (obj->fs_type == WIN_FILE)
    {
        memset(&stat_buf, 0, sizeof(struct _stat));

        ret = _stat(obj->u.wfs.path, &stat_buf);
        if (ret != 0)
        {
            return -1;
        }
        if (open_type == OPEN_SRC)
        {
            ...
            obj->u.wfs.f = fopen(obj->u.wfs.path, "r");
            obj->u.wfs.mode = (int)stat_buf.st_mode;
        }
        else
        {
            if (stat_buf.st_mode & _S_IFDIR)
            {
                if (srcname)
                {
                    _splitpath(srcname, NULL, NULL, fname, ext);
                    strcpy(filename, fname);
                    strcat(filename, ext);
                    if (obj->u.wfs.path[strlen(obj->u.wfs.path)-1] != '\\')
                    {
                        strcat(obj->u.wfs.path, "\\");
                    }
                    strncat(obj->u.wfs.path, filename, NAME_MAX);
                }
                else
                {
            ...
            obj->u.wfs.f = fopen(obj->u.wfs.path, "w");
        }
        if (obj->u.wfs.f == NULL)
        {
            perror("open");
    ...
    else
    {
        ...
        if (open_type == OPEN_DEST)
        {
            ...
            {
                /* make initial attributes with all permissions set so the file can
                   be written; actual permissions are written in generic_cleanup */
                make_attribs(&(obj->u.pvfs2.attr), credentials, nr_datafiles, 0777);
                if (strip_size > 0) {
                           ...
                           PVFS_credential *credentials)
{
    ...
    /* preserve read-only doing a pvfs2 => win copy */
    if ((src->fs_type == PVFS2_FILE) && (dest->fs_type == WIN_FILE))
    {
        if (!(src->u.pvfs2.perms & PVFS_O_WRITE|PVFS_G_WRITE|PVFS_U_WRITE))
        {
            _chmod(dest->u.wfs.path, _S_IREAD);
        }            
    }

    /* preserve permissions doing a win => pvfs2 copy */
    if ((src->fs_type == WIN_FILE) && (dest->fs_type == PVFS2_FILE))
    {
        mode = 0660;
        if (!(src->u.wfs.mode &_S_IWRITE))
        {
            mode = 0440;
        }
        make_attribs(&dest->u.pvfs2.attr, credentials, -1, mode);
        ret = PVFS_sys_setattr(dest->u.pvfs2.ref, dest->u.pvfs2.attr, credentials,
                               hints);
    }

    /* preserve permissions doing a unix => unix copy */
    if ((src->fs_type == WIN_FILE) && (dest->fs_type == WIN_FILE))
    {
        /* TODO ret = fchmod(dest->u.ufs.fd, src->u.ufs.mode); */
    }

    /* preserve permissions doing a pvfs2 => pvfs2 copy */
    if ((src->fs_type == PVFS2_FILE) && (dest->fs_type == PVFS2_FILE))
    {
        ret = PVFS_sys_setattr(dest->u.pvfs2.ref, src->u.pvfs2.attr, credentials,
                               hints);
    }

    if ((src->fs_type == WIN_FILE) && (src->u.wfs.f != NULL))
    {
        fclose(src->u.wfs.f);
    }

    if ((dest->fs_type == WIN_FILE) && (dest->u.wfs.f != NULL))
    {
        fclose(dest->u.wfs.f);
    }
    ...
    if (ret)
    {
        fprintf(stderr, "warning: could not set dest. file attributes (%d)\n",
                ret);
    }
