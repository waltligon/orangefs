...
/* pvfs2-write: 
 * 	write a file from an in-memory buffer to a unix or PVFS2 file
 * 	system. This can be helpful for testing network performance,
 *      sort of like running "dd if=/dev/zero ..."
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main (int argc, char ** argv)
{
    ...
    memset(buffer, 0, user_opts->buf_size);

    time1 = Wtime();
    while(total_written < user_opts->file_size)
    {
	if (user_opts->file_size - total_written > user_opts->buf_size)
        {
...
static uint64_t parse_bytes(char *argument)
{
    uint64_t value = 0, multiplier = 0;
    char *mod_ptr = NULL;
    int ret = -1;

    ret = sscanf(argument, "%lu", &value);
    if(ret < 1){
        return(0);
    }

    for (mod_ptr = argument; *mod_ptr && isdigit(*mod_ptr); mod_ptr++) { /* empty */; }
    if (*mod_ptr == 'K' || *mod_ptr == 'k') {
        multiplier = 1024;
    }
    else if (*mod_ptr == 'M' || *mod_ptr == 'm') {
        multiplier = 1024*1024;
    }
    else if (*mod_ptr == 'G' || *mod_ptr == 'g') {
...
static struct options* parse_args(int argc, char* argv[])
{
	...
	switch(one_opt){
		...
		break;
	    case('b'):
		printf("Buffer size: %s  ", optarg);
                tmp_opts->buf_size = parse_bytes(optarg);
		if(tmp_opts->buf_size < 1){
		    free(tmp_opts);
		    return(NULL);
		}
		break;
	    case('f'):
                tmp_opts->file_size = parse_bytes(optarg);
		if(tmp_opts->file_size < 1){
		    free(tmp_opts);
		    return(NULL);
		}
		break;
    ...
    tmp_opts->destfile = argv[argc-2];

    printf("File size: %s  ", argv[argc-1]);
    if ((tmp_opts->file_size = parse_bytes(argv[argc-1])) < 1)
    {
        usage(argc, argv);
        exit(EXIT_FAILURE);
    }
...
{
    fprintf(stderr, 
	"Usage: %s ARGS dest_file file_size\n", argv[0]);
	...
	"\n-b <buffer_size in bytes>\thow much data to write at once"
