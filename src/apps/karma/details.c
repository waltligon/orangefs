...
static gint gui_details_float_string_compare(
    GtkTreeModel * model,
    GtkTreeIter * iter_a,
    GtkTreeIter * iter_b,
    gpointer col_id);

static gint gui_details_text_compare(
    GtkTreeModel * model,
    GtkTreeIter * iter_a,
    GtkTreeIter * iter_b,
    gpointer col_id);
...
static void gui_details_view_insert(
    GtkListStore * list,
...
/* gui_details_setup()
 */
GtkWidget *gui_details_setup(
    void)
{
    gui_details_view = gui_details_view_new(&gui_details_list, gui_details_col,
#ifdef GUI_DETAILS_ENABLE_SORTING
                                            1
#else
                                            0
 ...
 */
GtkWidget *gui_details_view_new(
    GtkListStore ** list_p,
    ...
    gint sortable)
{
    ...
    list = gtk_list_store_new(GUI_DETAILS_TYPE + 1, G_TYPE_STRING,      /* name */
    ...
    gtk_tree_view_set_model(GTK_TREE_VIEW(list_view), GTK_TREE_MODEL(list));
 ...
 */
void gui_details_update(
    struct PVFS_mgmt_server_stat *s_stat,
    int s_stat_ct)
{
    gui_details_view_fill(gui_details_view,
                          gui_details_list,
                          gui_details_col, s_stat, s_stat_ct, NULL);
    ...
    int *server_list)
{
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].ram_free_bytes > s_stat[max].ram_free_bytes)
            max = i;
    }
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].uptime_seconds > s_stat[max].uptime_seconds)
            max = i;
    }
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].handles_total_count > s_stat[max].handles_total_count)
            max = i;
    }
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].handles_available_count >
            s_stat[max].handles_available_count)
            max = i;
    }
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].bytes_total > s_stat[max].bytes_total)
            max = i;
    }
    ...
    for (max = 0, i = 1; i < s_stat_ct; i++)
    {
        if (s_stat[i].bytes_available > s_stat[max].bytes_available)
            max = i;
    }
    ...
    gtk_tree_view_column_set_title(col[GUI_DETAILS_UPTIME], titlebuf);

    snprintf(titlebuf, sizeof(titlebuf), "%s\n(%s)",
             gui_details_column_name[GUI_DETAILS_HANDLES_TOT], ht_units);
    gtk_tree_view_column_set_title(col[GUI_DETAILS_HANDLES_TOT], titlebuf);

    snprintf(titlebuf, sizeof(titlebuf), "%s\n(%s)",
             gui_details_column_name[GUI_DETAILS_HANDLES_AVAIL], ha_units);
    gtk_tree_view_column_set_title(col[GUI_DETAILS_HANDLES_AVAIL], titlebuf);

    snprintf(titlebuf, sizeof(titlebuf), "%s\n(%s)",
             gui_details_column_name[GUI_DETAILS_SPACE_TOT], bt_units);
    gtk_tree_view_column_set_title(col[GUI_DETAILS_SPACE_TOT], titlebuf);

    snprintf(titlebuf, sizeof(titlebuf), "%s\n(%s)",
             gui_details_column_name[GUI_DETAILS_SPACE_AVAIL], ba_units);
    gtk_tree_view_column_set_title(col[GUI_DETAILS_SPACE_AVAIL], titlebuf);
    ...
    if (server_list == NULL)
    {
        for (i = 0; i < s_stat_ct; i++)
        {
            gui_details_view_insert(list,
                                    ...
                                    rt_div,
                                    ra_div,
                                    up_div, ht_div, ha_div, bt_div, ba_div);
        }
    }
    else
    {
    ...
    gpointer col_id)
{
    ...
    gtk_tree_model_get(model, iter_a, (gint) (unsigned long) col_id, &string_a,
                       -1);
    gtk_tree_model_get(model, iter_b, (gint) (unsigned long) col_id, &string_b,
                       -1);
