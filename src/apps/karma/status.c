...
static void gui_status_graph_draw_stacked(struct gui_status_graph_state *g_state,
					  ...
					  int *bar_color,
					  char *footer);
static GtkWidget *gui_status_graph_setup(gint width,
					 gint height,
					 struct gui_status_graph_state *g_state);
/* internal callback fn prototypes */
static gint gui_status_graph_expose_callback(GtkWidget *drawing_area,
			     ...
			     struct gui_status_graph_data *graph_data)
{
    ...
    if (graph_data->has_second_val) {
	memcpy(g_state->secondval, graph_data->second_val, sizeof(float) * count);
    }
    ...
    gui_status_graph_draw_stacked(&gui_status_graphs[graph_id],
				  ...
				  (graph_data->has_second_val) ? graph_data->second_val: NULL,
				  graph_data->bar_color,
				  graph_data->footer);
}
					  ...
					  char *footer)
{
    ...
    /* reset title */
    gtk_frame_set_label(GTK_FRAME(g_state->frame), (title == NULL) ? "" : title);
    ...
    else                   maxval = firstval[0];

    for (i=1; i < nr_bars; i++) {
	float tmpval;

	if (secondval != NULL) tmpval = firstval[i] + secondval[i];
	else                   tmpval = firstval[i];
	...
	if (secondval != NULL) {
	    ...
	    /* aesthetics: always draw something */
	    if (secondbarlength == 0) secondbarlength = 1;
						...
						gpointer graph_state_ptr)
{
    ...
    g_state->red_gc = gui_get_new_fg_color_gc(g_state->drawing_area,
					      255, 0, 0);
