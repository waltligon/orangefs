...
static gint gui_traffic_graph_configure_callback(GtkWidget *drawing_area,
						 GdkEvent *event,
						 gpointer nullptr);

static gint gui_traffic_graph_expose_callback(GtkWidget *drawing_area,
					      GdkEventExpose *event,
					      gpointer nullptr);

#if 0
static gint gui_traffic_graph_visibility_callback(GtkWidget *drawing_area,
...
GtkWidget *gui_traffic_setup(void)
{
    ...
    g_signal_connect(G_OBJECT(gui_traffic_graph.drawing_area),
		     "configure_event",
		     G_CALLBACK(gui_traffic_graph_configure_callback),
		     NULL);
    g_signal_connect(G_OBJECT(gui_traffic_graph.drawing_area),
		     "expose_event",
...
void gui_traffic_graph_update(struct gui_traffic_graph_data *data)
{
    ...
    if (gui_traffic_graph_configured) gui_traffic_graph_draw();
}
...
static void gui_traffic_graph_draw(void)
{
	...
	snprintf(io_string, 64, "%.2f",
		 (max_bw * (float) i) / (float) (GUI_TRAFFIC_GRAPH_NR_TICS-1));
 ...
 */
#if 0
static gint gui_traffic_graph_visibility_callback(GtkWidget *widget,
						  GdkEventVisibility *event,
						  gpointer nullptr)
{
