...
int main(int argc, char **argv)
{
    ...
    for (s = srv_index; s < io_server_count; s++)
    {
         ...
         * address.  We have this in the SIDcache, but no way to do the lookup.
         * Probably we need to add a new index.  For now I'm breaking this driver
         * until such time as I have a clearer picture and can fix this. 
         * I am also leaving it causing a warning so I don't forget it - WBL
         */

        /* cp = PVFS_mgmt_map_addr(cur_fs, addr_array[s], &servertype); */
        /* ret = PVFS_SID_get_addr(PVFS_BMI_addr_t *, const PVFS_SID_*sid); */
        p = strdup(cp);
