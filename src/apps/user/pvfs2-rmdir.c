...
int main(int argc, char **argv)
{
    ...
    struct options user_opts = {0, 0, 0};
    ...
    for (i = 0; i < user_opts.num_files; ++i)
    {
        ...
        {
            ret = rmdir(working_path);
            if(ret < 0)
            {
                perror("rmdir failed: ");
                fprintf(stderr, "rmdir failed on path: %s\n", working_path);
                error_seen = 1;
            }
        }
        else
        {
            fprintf(stderr, "rmdir: path is not directory: %s\n", working_path);
            error_seen = 1;
        }
    }
...
static void usage(int argc, char **argv)
{
    fprintf(stderr, "Usage: %s [-f] pvfs2_filename[s]\n", argv[0]);
}
