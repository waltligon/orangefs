    ...
    void)
{
    ...
    read_bw_matrix =
        (double **) malloc(2 * pint_vis_shared.io_count * sizeof(double *));
    if (!read_bw_matrix)
    {
        perror("malloc");
        TTF_Quit();
    ...
    {
        read_bw_matrix[i] =
            (double *) malloc(2 * pint_vis_shared.io_depth * sizeof(double));
        if (!read_bw_matrix[i])
        {
            perror("malloc");
            TTF_Quit();
    ...
    channel_width =
        (screen->w - SIDE_BORDER * 2) / (pint_vis_shared.io_count * 3);
    if (channel_width > SIDE_BORDER)
        channel_width = SIDE_BORDER;

    left_offset =
        (screen->w - (channel_width * 3 * pint_vis_shared.io_count)) / 2;
    ...
    while (1)
    {
        ...
        for (i = 0; i < pint_vis_shared.io_count; i++)
        {
            for (j = 0; j < pint_vis_shared.io_depth; j++)
            {
                ...
                {
                    read_bw_matrix[i][j] =
                        ((double) pint_vis_shared.io_perf_matrix[i][j].read *
                         1000.0) /
                        (double) (pint_vis_shared.io_perf_matrix[i][j + 1].
                                  start_time_ms -
                                  pint_vis_shared.io_perf_matrix[i][j].
                                  start_time_ms);
                    write_bw_matrix[i][j] =
                        ((double) pint_vis_shared.io_perf_matrix[i][j].write *
                         1000.0) /
                        (double) (pint_vis_shared.io_perf_matrix[i][j + 1].
                                  start_time_ms -
                                  pint_vis_shared.io_perf_matrix[i][j].
        ...
        for (i = 0; i < pint_vis_shared.io_count; i++)
        {
            ...
            text =
                TTF_RenderText_Shaded(font, scratch_string, font_color,
                                      black_color);
            assert(text);
            text_rect.h = text->h;
            text_rect.w = text->w;
        ...
        for (i = 0; i < pint_vis_shared.io_count; i++)
        {
            ...
            read_bws[i].bar.y =
                TOP_BORDER + read_bws[i].full.h - read_bws[i].bar.h;
            ...
            read_bws[i].peak.y =
                TOP_BORDER + read_bws[i].full.h - read_bws[i].peak.h;
            read_bws[i].peak.h = 3;
            ...
            write_bws[i].bar.y =
                TOP_BORDER + write_bws[i].full.h - write_bws[i].bar.h;
            ...
            write_bws[i].peak.y =
                TOP_BORDER + write_bws[i].full.h - write_bws[i].peak.h;
    ...
    char **argv)
{
    fprintf(stderr, "\n");
    fprintf(stderr,
            "Usage  : %s -m [fs_mount_point] -w [width in pixels] -i [update interval (msecs)]\n",
            argv[0]);
