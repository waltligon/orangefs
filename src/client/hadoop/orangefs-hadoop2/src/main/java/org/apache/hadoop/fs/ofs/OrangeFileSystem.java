...
public class OrangeFileSystem extends FileSystem {
  ...
  public FSDataOutputStream append(Path f, int bufferSize, Progressable progress)
    ...
    OrangeFileSystemOutputStream ofsOutputStream =
        new OrangeFileSystemOutputStream(fOFS.toString(), bufferSize, (short) 0, 0L, true);
  ...
  public FSDataOutputStream create(Path f, FsPermission permission, boolean overwrite,
    ...
    /* New file */
      ...
      if (fParent != null && !exists(fParent)) {
        ...
        if (!mkdirs(fParent)) {
           ...
           */
          if (!exists(fParent)) {
            throw new IOException("Failed to create parent directory/directories: "
                + fParent.toString());
          }
        }
      }
    ...
    fsdos =
        new FSDataOutputStream(new OrangeFileSystemOutputStream(fOFS.toString(), bufferSize,
            replication, blockSize, false), statistics);
    /* Set the desired permission. */
    setPermission(f, permission);
    return fsdos;
  }

  /* This method is intended to support HBase (HBASE-11045) */
  @Deprecated
  @Override
  public FSDataOutputStream createNonRecursive(Path f, FsPermission permission,
    ...
    /* New file */
      ...
      OFSLOG.debug("fParent = " + fParent);
      if (!exists(fParent)) {
        throw new IOException("Parent directory/directories does not exist: " + fParent.toString());
      }
    }
    fsdos =
        new FSDataOutputStream(new OrangeFileSystemOutputStream(fOFS.toString(), bufferSize,
  ...
  public boolean delete(Path f, boolean recursive) throws IOException {
    ...
    if (status.isDirectory()) {
      ...
      OFSLOG.debug("Path f =" + f + " is a directory and recursive is true."
          + " Recursively deleting directory.");
      ret = orange.stdio.recursiveDelete(fOFS.toString()) == 0;
    } else {
      OFSLOG.debug("Path f =" + f + " exists and is a regular file. unlinking.");
  ...
  public FileStatus getFileStatus(Path f) throws IOException {
    ...
    /**/
    fileStatus =
        new FileStatus(stats.st_size, isdir, block_replication, stats.st_blksize,
            stats.st_mtime * 1000, stats.st_atime * 1000, permission, username, groupname,
            makeAbsolute(f).makeQualified(this.uri, this.workingDirectory));
    return fileStatus;
  }

  @Override
  public Path getHomeDirectory() {
    return new Path("/user/" + System.getProperty("user.name")).makeQualified(this.uri,
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    if (uri.getAuthority() == null) {
      throw new IOException("Incomplete OrangeFS URI, no authority: " + uri);
    }
    ...
    OFSLOG.debug("Number of specified OrangeFS systems: " + ofsSystems.length);
    ...
    if (ofsSystems.length != ofsMounts.length) {
      throw new IOException("Configuration values fs.ofs.systems and"
          + " fs.ofs.mntLocations must contain the same number of" + " comma-separated elements.");
    }

    OFSLOG.debug("Determining file system associated with URI authority.");
    for (index = 0; index < ofsSystems.length; index ++) {
      OFSLOG.debug("{ofsSystems[" + index + "], ofsMounts[" + index + "]} = {" + ofsSystems[index]
          + ", " + ofsMounts[index] + "}");
    ...
    if (uriAuthorityMatchFound) {
      ...
      OFSLOG.error("No OrangeFS file system found matching the " + "following authority: "
          + uriAuthority);
      throw new IOException("There was no matching authority found in"
          + " fs.ofs.systems. Check your configuration. -->" + uriAuthority + " = "
          + ofsSystems[index - 1]);
    }
    ...
    OFSLOG.debug("OrangeFileSystem.statistics: " + this.statistics.toString());

    this.localFS = FileSystem.getLocal(conf);
    workingDirectory =
        new Path("/user/" + System.getProperty("user.name")).makeQualified(this.uri, null);
  ...
  public boolean mkdirs(Path f, FsPermission permission) throws IOException {
    ...
    OFSLOG.debug("mkdirs attempting to create directory: " + makeAbsolute(f).toString());
    ...
    if (exists(f)) {
      if (isDir(f)) {
        OFSLOG.warn("directory=" + makeAbsolute(f).toString() + " already exists");
        setPermission(f, permission);
        return true;
      } else {
        OFSLOG.warn("path exists but is not a directory: " + makeAbsolute(f));
        return false;
      }
    }
    /*
    ...
    if (parents != null) {
      ...
      for (int i = 0; i < parents.length; i ++) {
        if (exists(parents[i])) {
          if (!isDir(parents[i])) {
            OFSLOG.warn("parent path is not a directory: " + parents[i]);
            return false;
          }
        } else {
          ...
          if (ret == 0) {
            ...
            OFSLOG.error("mkdir failed on parent directory = " + parents[i] + ", permission = "
                + permission.toString());
            return false;
          }
        }
    ...
    if (ret == 0) {
      ...
      OFSLOG.error("mkdir failed on path f =" + makeAbsolute(f) + ", permission = "
          + permission.toString());
      return false;
    }
  ...
  public FSDataInputStream open(Path f, int bufferSize) throws IOException {
    Path fOFS = new Path(getOFSPathName(f));
    return new FSDataInputStream(new OrangeFileSystemFSInputStream(fOFS.toString(), bufferSize,
        statistics));
  }
  ...
  public void setPermission(Path p, FsPermission permission) throws IOException {
    ...
    if ((mode & 01000) == 01000) {
      ...
      OFSLOG.warn("new mode (symbolic) = " + newModeAsPermission.toString());
    }
    if (orange.posix.chmod(fOFS.toString(), mode) < 0) {
      throw new IOException("Failed to set permissions on path = " + makeAbsolute(p) + ", mode = "
          + mode);
    }
  }
