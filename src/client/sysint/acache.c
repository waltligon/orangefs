...
static int acache_compare_key_entry(
    void *key,
    struct qhash_head *link);
static int acache_hash_key(
    void *key,
    int table_size);
static int acache_free_payload(
    void *payload);
    ...
    PVFS_object_ref refn)
{
    ...
    ret = PINT_tcache_lookup(acache, &refn, &tmp_entry, &tmp_status);
    if (ret == 0)
    {
        PINT_tcache_delete(acache, tmp_entry);
        PINT_perf_count(acache_pc, PERF_ACACHE_DELETIONS, 1, PINT_PERF_ADD);
    }
    ...
    struct qhash_head *link)
{
    ...
    tmp_entry = qhash_entry(link, struct PINT_tcache_entry,
                            hash_link);
    assert(tmp_entry);
