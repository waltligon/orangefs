...
/* Read Directory server request processing implementation */
...
/* pint_serv_readdir()
 *
 * handles the server interaction for the "readdir" request
 *
		...
		credentials, bmi_addr_t *serv_arg)
{
   ...
   bmi_addr_t server_addr = *serv_arg;	 /*PVFS address type structure*/ 
	...
	PVFS_servreq_readdir *arg = (PVFS_servreq_readdir *)req;
	...
	/* Fill in the jobs */
	/* Request job */
	ret = readdirreq_alloc((void *)req_job,arg,server_addr,credentials,\
			&req_size);
	if (ret < 0)
	{
		ret = -ENOMEM;
	...
	/* Response job */
	ret = readdirack_alloc((void *)ack_job,arg,server_addr,&ack_size);
	if (ret < 0)
	{
		ret = -ENOMEM;
		goto send_failure;
...
/* readdirreq_alloc()
 *
 * sets up a readdir request using a pre-existing job structure
 *
 * returns 0 on success, -errno on failure
 */
static int readdirreq_alloc(void *pjob,void *preq,bmi_addr_t server,
		PVFS_credentials credentials,int *sz)
{
	...
	int size = 0;
	PVFS_servreq_readdir *req = (PVFS_servreq_readdir *)preq;
	...
	*serv_req = BMI_memalloc(server,size,BMI_SEND_BUFFER);
	...
	/* Set up the request for readdir */
	(*serv_req)->op = PVFS_SERV_READDIR;	
	/* Copy the attribute mask */
	(*serv_req)->u.readdir.handle = req->handle;	
	(*serv_req)->u.readdir.fs_id = req->fs_id;
	(*serv_req)->u.readdir.token = req->token;
	(*serv_req)->u.readdir.pvfs_dirent_count = req->pvfs_dirent_count;
	(*serv_req)->rsize = size;
...
/* readdirack_alloc()
 *
 * sets up a readdir acknowledgement using a pre-existing job structure
 *
 * returns 0 on success, -errno on failure
 */
static int readdirack_alloc(void *pjob,void *preq,bmi_addr_t server,
		int *sz)
{
	...
	/* Fill up the job structure */
	size = sizeof(struct PVFS_server_resp_s) + req->pvfs_dirent_count *\
		sizeof(PVFS_dirent);
	...
	*serv_resp = BMI_memalloc(server,size,BMI_RECV_BUFFER);
	...
	/* Set up the response for readdir */
	memset(*serv_resp,0,size); 
	(*serv_resp)->op = PVFS_SERV_READDIR;	

	/* Point the buffer to allocated memory */
	(*serv_resp)->u.readdir.pvfs_dirent_array = (PVFS_dirent *)((char *)\
				(*serv_resp) + sizeof(struct PVFS_server_resp_s));
	(*serv_resp)->u.readdir.pvfs_dirent_count = req->pvfs_dirent_count;
