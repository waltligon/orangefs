...
static struct PINT_state_s ST_flush_fd;
static struct PINT_pjmp_tbl_s ST_flush_fd_pjtbl[];
static struct PINT_tran_tbl_s ST_flush_fd_trtbl[];
...
struct PINT_state_machine_s pvfs2_client_aio_close_sm = {
	.name = "pvfs2_client_aio_close_sm",
	.first_state = &ST_init
};

static struct PINT_state_s ST_init = {
	 .state_name = "init" ,
	 .parent_machine = &pvfs2_client_aio_close_sm ,
	 .flag = SM_RUN ,
	 .action.func = close_init ,
	 .pjtbl = NULL ,
	 .trtbl = ST_init_trtbl 
};

static struct PINT_tran_tbl_s ST_init_trtbl[] = {
	{ .return_value = CLOSE_FLUSH_FD ,
	 .next_state = &ST_flush_fd },
	{ .return_value = -1 ,
...
static struct PINT_state_s ST_flush_fd = {
	 ...
	 .parent_machine = &pvfs2_client_aio_close_sm ,
	 .flag = SM_JUMP ,
	 .action.nested = &pvfs2_client_flush_sm ,
	 .pjtbl = NULL ,
	 .trtbl = ST_flush_fd_trtbl 
};
...
#ifndef WIN32
# 44 "src/client/usrint/aio-close.sm"
#endif
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    if (S_ISREG(sm_p->u.aio_close.pd->s->mode))
    {
        ...
        PINT_init_sysint_credentials(close_frame->cred_p, sm_p->cred_p);
