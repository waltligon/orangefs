...
int rename (const char *oldname, const char *newname)
{
    ...
    oldp = is_pvfs_path(&oldname,0);
    newp = is_pvfs_path(&newname,0);
    if(oldp && newp)
    { 
        rc = pvfs_rename(oldname, newname);
    }
    else if (!oldp && !newp)
    {
        rc = glibc_ops.rename(oldname, newname);
    }
    else
    {
        errno = EXDEV;
...
errorout:
    rc = -1;
cleanup:
    PVFS_free_expanded(oldname);
    PVFS_free_expanded(newname);
    return rc;
}
...
int linkat(int olddirfd, const char *oldname,
           int newdirfd, const char *newname, int flags)
{
    pvfs_descriptor *oldpd, *newpd;
