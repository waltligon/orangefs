    ...
    while(i < count)
    {
        ...
        while(++i < count && iov[i].iov_len == bsz)
        {
            if(vlen == 1)
            {
                ...
                stride = (char *)iov[i].iov_base - (char *)iov[i - 1].iov_base;
                if (stride < bsz)
                {
            ...
            else if (((char *)iov[i].iov_base - (char *)iov[i - 1].iov_base) == stride)
            {
        ...
        if (vlen == 1)
        {
            ...
            disp_array[rblk] = (PVFS_size)((char *)iov[vstart].iov_base - *(char *)buf);
