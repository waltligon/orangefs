...
void test_12(void)
{
    FILE *file = fopen("ucache_test/12.log","w");
    if(LOG){out = file;}
    else{out = stdout;}
    /********** Begin Test  **********/
    ucache_lock_t lock; /*  Currently a semaphore   */
    int sval;   /*  Semaphore Value */

    ucache_lock_init(&lock);
    ucache_lock_lock(&lock);
    ucache_lock_getvalue(&lock, &sval);
    ucache_lock_unlock(&lock);
    ucache_lock_destroy(&lock);
}
...
int main(int argc, char **argv)
{
    ...
    if(argc==2){
        ...
        else if(atoi(argv[1])==12)test_12();    /* Mutex Testing    */
    }
