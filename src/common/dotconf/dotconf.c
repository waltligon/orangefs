    ...
    char *ext)
{
    ...
    if ((dh = opendir(path)) != NULL)
    {
        ...
        {
            match_state =
                PINT_dotconf_question_mark_match(dirptr->d_name, pre, ext);
    ...
    char *ext)
{
    ...
    while (t_ext != NULL && !(PINT_dotconf_is_wild_card(*t_ext))
           && *t_ext != '\0')
    {
...
DOTCONF_CB(dotconf_cb_include)
{
    ...
    if (PINT_dotconf_find_wild_card(filename, &wild_card, &path, &pre, &ext) >=
        0)
    {
