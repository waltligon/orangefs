            ...
            while (cur)
            {
                ...
                if (((tmp_extent->last - tmp_extent->first) > 0) ||
                    ((tmp_extent->last > 0) &&
                     ...
                     !handle_is_excluded(tmp_extent->last, handles_to_exclude,
                                         num_handles_to_exclude)))
                {
    ...
    int verbose)
{
        ...
        (data_handle_ranges && strlen(data_handle_ranges)))
    {
        merged_handle_ranges =
            PINT_merge_handle_range_strs(meta_handle_ranges,
                                         data_handle_ranges);
    }
     ...
     */
    ret =
        trove_collection_setinfo(coll_id, trove_context,
    ...
    if (new_root_handle != TROVE_HANDLE_NULL)
    {
        ...
        ret = trove_dspace_create(coll_id, &extent_array, &new_root_handle,
        ...
        if ((ret != 1) && (state != 0))
        {
            mkspace_print(verbose, "dspace create (for root dir) failed.\n");
            return -1;
        }
        ...
        ret =
            trove_dspace_setattr(coll_id, new_root_handle, &attr, TROVE_SYNC,
                                 NULL, trove_context, &op_id);
        ...
        {
            get_handle_extent_from_ranges(meta_handle_ranges, &cur_extent,
                                          s_used_handles, 1);
            ...
            else
            {
                mkspace_print(verbose,
                              "info: using meta handle range %llu-%llu for "
        ...
        ret = trove_keyval_write(coll_id, new_root_handle, &key, &val,
                                 TROVE_SYNC, 0, NULL, trove_context, &op_id);
        ...
        mkspace_print(verbose, "info: wrote attributes for root directory.\n");
        ...
        *****************************************************/
        ...
        if (meta_handle_ranges)
        {
            ...
            else
            {
                mkspace_print(verbose,
                              ...
                              llu(cur_extent.first), llu(cur_extent.last));
            }
        }
        extent_array.extent_count = 1;
        extent_array.extent_array = &cur_extent;
        ...
        ret =
            trove_dspace_create(coll_id, &extent_array, &lost_and_found_handle,
        ...
        ret =
            trove_dspace_setattr(coll_id, lost_and_found_handle, &attr,
        ...
        ret =
            trove_dspace_create(coll_id, &extent_array,
        ...
        ret = trove_keyval_write(coll_id, lost_and_found_handle, &key, &val,
                                 TROVE_SYNC, 0, NULL, trove_context, &op_id);
        ...
        ret = trove_keyval_write(coll_id, root_dirdata_handle, &key, &val,
                                 TROVE_SYNC | TROVE_NOOVERWRITE |
                                 TROVE_KEYVAL_HANDLE_COUNT, 0, NULL,
                                 trove_context, &op_id);
    ...
    int verbose)
{
    ...
    mkspace_print(verbose, "PVFS2 Collection %s removed %s\n", collection,
