...
do {                                       \
    ...
    {                                       \
        ...
        PVFS_debug_afield((state)->svr_addr, int64_t);           \
        ...
        PVFS_debug_afield((state)->send_id, job_id_t);           \
        PVFS_debug_afield((state)->recv_id, job_id_t);           \
        PVFS_debug_afield((state)->flow_id, job_id_t);           \
        PVFS_debug_afield((state)->ack_id, job_id_t);           \
        PVFS_debug_afield(&(state)->send_status, job_status_s);           \
        PVFS_debug_afield(&(state)->recv_status, job_status_s);           \
        PVFS_debug_afield(&(state)->flow_status, job_status_s);           \
        PVFS_debug_afield(&(state)->ack_status, job_status_s);           \
        PVFS_debug_afield(&(state)->flow_desc, pointer); /*flow_descriptor*/     \
        PVFS_debug_afield((state)->op_status, PVFS_error);           \
        PVFS_debug_afield((state)->op_action, int);           \
        PVFS_debug_afield((state)->complete, int);           \
        PVFS_debug_afield((state)->dfile_size, int);           \
        gossip_lsadebug("MsgPair Op End:\n");          \
    }           \
    gossip_end;           \
} while (0)
...
{   
    ...
    int fork_flag;    /* indicates if MPA should PJMP */
