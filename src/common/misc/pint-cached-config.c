...
/* V3 - no more handle ranges */
    ...
    */
    phys_server_desc_t *io_server_array;
    int io_server_count;
    phys_server_desc_t *meta_server_array;
    int meta_server_count;
    phys_server_desc_t *server_array;
    int server_count;
                                  ...
                                  PVFS_handle_extent_array *ext_array)
{
    ...
    ext_array->extent_count = cur_mapping->handle_extent_array.extent_count;
    ext_array->extent_array = cur_mapping->handle_extent_array.extent_array;

    return(0);
}
#endif

/* V3 This should probably move to the server dir 
 * not sure which source file
 */
#if defined(__PVFS2_SERVER__)
/* PINT_cached_config_server_local()
 *
 * returns true if the SID provided matches the SID of the current
 * server stored in the configration
 */
...
/* V3 no more extents or mappings */
#if 0
...
/* V3 no more extents or mappings */
    ...
    cur_mapping = PINT_llist_head(cur_config_cache->meta_server_cursor);
...
/* V3 no more ranges, server count may not make sense */
#if 0
...
/* V3 no more ranges, server count may not make sense */
		...
		{
		    ret = BMI_addr_lookup(io_addr_array,
                                          data_server_bmi_str);
		    if (ret)
		    {
			break;
 ...
 * may pass in num_dfiles_requested as a hint, if no hint is given (<=0),
 * the server configuration is checked to find a hint there.  The
 * distribution will choose a correct number of dfiles even if no hint is set.
 */
...
/* V3 cleanup */
    ...
    *num_dfiles = dist->methods->get_num_dfiles(dist->params,
    ...
    if(*num_dfiles < 1)
    {
        gossip_err("Error: distribution failure for %d servers and "
                   "%d requested datafiles.\n",
                   num_io_servers, num_dfiles_requested);
        return(-PVFS_EINVAL);
    }
 
    if (*num_dfiles > num_io_servers && 0 < num_io_servers)
    {
        gossip_err("%s: Distribution requires more datafiles(%d) than "
                   "I/O servers(%d) currently defined in the system. Capping "
...
/* V3 doesn't really make sense, but might be used for soem other timeout
 * */
...
int PINT_cached_config_get_server_list(PVFS_fs_id fs_id,
                                       PINT_dist *dist,
                                       int num_dfiles_req,
                                       PVFS_sys_layout *layout,
...
/* V3 obsolete - info handled by SID cache now */
    ...
    if (cur_config_cache->server_count < 1)
    {
        ...
        memset(cur_config_cache->server_array,
               0, 
                ...
                struct config_fs_cache_s *cur_config_fs_cache)
{
    ...
    for(j = 0; j < 2; j++)
    {
        ...
        while(cur_mapping)
        {
            for(i = 0; i < cur_mapping->handle_extent_array.extent_count; i++)
            {
                ...
                {
                    free(cur_config_fs_cache->handle_lookup_table);
                    gossip_err("Error: failed to resolve "
                               "address of server: %s\n",
                               cur_config_fs_cache->handle_lookup_table[
                                        table_offset].server_name);
                    return(ret);
                }
                table_offset++;
            }
    ...
    /* sort table */
    qsort(cur_config_fs_cache->handle_lookup_table,
          table_offset, 
