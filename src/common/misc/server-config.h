...
typedef struct filesystem_configuration_s
{
    ...
    int default_num_dfiles;               /* num dfiles for each file */
    int default_dfile_replication_factor; /* num sids for each dfile */
    ...
    size_t db_max_size; /* used to create keyval, dspace, and collection databases.
                         * LMDB only.
                         */
...
typedef struct server_configuration_s
{
    ...
    int my_server_options;         /* indicates current config entries
                                    * being processed are intended
                                    * for the local server
                                    */
    int server_defaults;           /* indicates the current config entries
                                    * being processed are generated by 
                                    * PINT_dotconf_set_defaults not
                                    * actual config entries
                                    */
    ...
    size_t db_max_size;             /* size used for collections and        */
                                    /*    storage_attributes databases      */
...
/* V3 ECQ - unneeded? */
    ...
    void *private_data;
    int32_t tree_width;             /* treee based collective operations ?  */
