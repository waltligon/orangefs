    ...
    PVFS_capability *cap)
{
    ...
    gossip_debug(GOSSIP_SECURITY_DEBUG, "client_capcache lookup: H=%s "
                 "uid=%d\n", PVFS_OID_str(&refn.handle), uid);
    ...
    PVFS_uid uid)
{
    ...
    gossip_debug(GOSSIP_SECURITY_DEBUG, "%s: H=%s uid=%d\n",
                 __func__, PVFS_OID_str(&refn.handle), uid);
    ...
    const PVFS_capability *cap)
{
    ...
    gossip_debug(GOSSIP_SECURITY_DEBUG, "client_capcache update: H=%s "
                 "uid=%d\n", PVFS_OID_str(&refn.handle), uid);
...
static int client_capcache_compare_key_entry(const void *key, struct qhash_head *link)
{
    ...
    if (real_key->uid == tmp_payload->uid &&
        PVFS_OID_EQ(&real_key->refn.handle, &tmp_payload->refn.handle) &&
...
static int client_capcache_hash_key(const void *key, int table_size)
{
    ...
    tmp_ret = (PVFS_OID_hash64(&real_key->refn.handle) + real_key->uid) %
              table_size;
