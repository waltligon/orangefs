...
#define _DLM_DISPATCH_FN(x) dlm_mgr_ ## x
#define DLM_DISPATCH_FN(x)  _DLM_DISPATCH_FN(x)

extern void DLM_DISPATCH_FN(dlm_v1)(struct svc_req *, register SVCXPRT *);
...
static int dlm_port    = DLM_REQ_PORT; /* Although we use RPCs association with port numbers is crucial for communication */
static int num_threads = DLM_NUM_THREADS;
...
static int startup(int argc, char **argv)
{
	...
	{
		int logfd, nullfd;
		char logname[] = "/tmp/dlmlog.XXXXXX";

		if ((logfd = mkstemp(logname)) == -1) 
		{
			if ((nullfd = open("/dev/null", O_RDWR)) == -1) {
				gossip_err("dlm: error opening log");
				return -1;
			}
...
static void *dlm_slave_func(void *args)
{
	...
	rqstp = sl_args->rqstp;
	xprt  = sl_args->transp;
	DLM_DISPATCH_FN(dlm_v1)(rqstp, xprt);
...
static void dlm_dispatch(struct svc_req *rqstp, SVCXPRT *xprt)
{
	/*
	...
	if (is_it_tcp == 0) {
		...
		if (use_tpool) {
			...
			if ((ret = tp_assign_work_by_id(id, (pthread_fn) dlm_slave_func, (void *)sl_args)) != 0) {
			...
			if (pthread_create(&tid, &attr, (pthread_fn) dlm_slave_func, (void *)sl_args) != 0) {
		...
		dlm_slave_func(sl_args);
	}
	return;
}

/* Sets the ball rolling after parsing the command line options */
static int dlm_init(int argc, char **argv)
{
	int opt, timeout;
	static tp_info tinfo = {
tpi_count:DLM_NUM_THREADS,
tpi_name :NULL,
tpi_stack_size:-1, /* default stack size is good enough */
	...
	if (use_tpool) {
		gossip_debug(GOSSIP_DLM_DEBUG, 
                "---------- Starting dlm server servicing RPCs using a thread pool "
                " [%d threads] ---------- \n", tinfo.tpi_count);
	}
	else {
		gossip_debug(GOSSIP_DLM_DEBUG, 
                "---------- Starting dlm server servicing RPCs by creating a thread "
                " each time  ---------- \n");
	}
	if (use_tpool) {
	...
	if (setup_service(-1 /* first available prog# */,
				...
				-1 /* both tcp & udp */,
				dlm_port /* port */,
				dlm_dispatch, /* dispatch function */
				&info) < 0) 
	{
		if (use_tpool) {
			tp_cleanup_by_id(id);
...
/* Main function where the dlm server is fired up from */
int main (int argc, char **argv)
{
	srand(time(NULL));
	gossip_enable_stderr();
	gossip_set_debug_mask(1, GOSSIP_DLM_DEBUG);
	/* Unset the program number and version from the portmapper */
	pmap_unset (DLM_MGR, dlm_v1);
	/* Start up the thread pool and register a service with the portmapper */
	if (dlm_init(argc, argv) < 0) {
		exit(1);
	}
	/* NOTREACHED */
	exit (0);
