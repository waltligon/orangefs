    ...
    int flags)
{
    ...
    known_method_table =
        malloc(known_method_count * sizeof(*known_method_table));
    ...
    int flags)
{
    ...
    active_method_count = PINT_split_string_list(&modules, module_string);
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    ret =
        tmp_ref->interface->BMI_meth_post_recv(id, tmp_ref->method_addr, buffer,
                                               expected_size, actual_size,
                                               buffer_type, tag, user_ptr,
                                               context_id);
    return (ret);
}
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    ret =
        tmp_ref->interface->BMI_meth_post_send(id, tmp_ref->method_addr, buffer,
                                               size, buffer_type, tag, user_ptr,
                                               context_id);
    return (ret);
}
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    ret =
        tmp_ref->interface->BMI_meth_post_sendunexpected(id,
                                                         tmp_ref->method_addr,
                                                         buffer, size,
                                                         buffer_type, tag,
                                                         user_ptr, context_id);
    ...
    bmi_context_id context_id)
{
    ...
    *outcount = 0;
    ...
    ret =
        active_method_table[target_op->addr->method_type]->BMI_meth_test(id,
                                                                         outcount,
                                                                         error_code,
                                                                         actual_size,
                                                                         user_ptr,
    ...
    bmi_context_id context_id)
{
    ...
    *outcount = 0;
    ...
    if (tmp_active_method_count == 1)
    {
        ...
        ret =
            active_method_table[0]->BMI_meth_testsome(incount, id_array,
    ...
    for (i = 0; i < tmp_active_method_count; i++)
    {
        ...
        for (j = 0; j < incount; j++)
        {
            if (id_array[j])
            {
                query_op = (struct method_op *) id_gen_safe_lookup(id_array[j]);
        ...
        if (need_to_test)
        {
            tmp_outcount = 0;
            ret =
                active_method_table[i]->BMI_meth_testsome(need_to_test,
                                                          tmp_id_array,
                                                          &tmp_outcount,
                                                          &(index_array
                                                            [*outcount]),
 ...
 */
static void construct_poll_plan(
    int nmeth,
    int *idle_time_ms)
{
    ...
    int max_idle_time_ms)
{
    ...
    *outcount = 0;
    ...
    while (position < incount && i < tmp_active_method_count)
    {
        ...
        {
            ret = active_method_table[i]->BMI_meth_testunexpected((incount -
                                                                   position),
                                                                  &tmp_outcount,
                                                                  (&
                                                                   (sub_info
    ...
    bmi_context_id context_id)
{
    ...
    *outcount = 0;
    ...
    while (position < incount && i < tmp_active_method_count)
    {
        ...
        {
            ret =
                active_method_table[i]->BMI_meth_testcontext(incount - position,
                                                             &out_id_array
                                                             [position],
                                                             &tmp_outcount,
                                                             &error_code_array
 ...
 */
const char *BMI_addr_rev_lookup(
    ...
    PVFS_BMI_addr_t addr)
{
    ...
    return (tmp_ref->interface->
            BMI_meth_rev_lookup_unexpected(tmp_ref->method_addr));
}
    ...
    void *inout_parameter)
{
    ...
    case BMI_GET_UNEXP_SIZE:
        ...
        gen_mutex_unlock(&ref_mutex);
        ret = tmp_ref->interface->BMI_meth_get_info(option, inout_parameter);
        if (ret < 0)
        {
            return ret;
        }
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    {
        ret =
            tmp_ref->interface->BMI_meth_post_send_list(id,
                                                        tmp_ref->method_addr,
                                                        buffer_list, size_list,
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    {
        ret =
            tmp_ref->interface->BMI_meth_post_recv_list(id,
                                                        tmp_ref->method_addr,
                                                        buffer_list, size_list,
                                                        list_count,
                                                        total_expected_size,
    ...
    bmi_context_id context_id)
{
    ...
    *id = 0;
    ...
    {
        ret =
            tmp_ref->interface->BMI_meth_post_sendunexpected_list(id,
                                                                  tmp_ref->
                                                                  method_addr,
                                                                  buffer_list,
                                                                  size_list,
    ...
    bmi_context_id context_id)
{
    ...
    {
        ret =
            active_method_table[target_op->addr->method_type]->
            BMI_meth_cancel(id, context_id);
    }
    else
    {
 ...
 * returns 0 on success, -errno on failure
 */
int bmi_method_addr_reg_callback(
    method_addr_p map)
{
    ref_st_p new_ref = NULL;
    ...
    int flags)
{
    ...
    active_method_table = malloc((active_method_count +
                                  1) * sizeof(*active_method_table));
    if (!active_method_table)
    {
