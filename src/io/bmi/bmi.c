...
static int split_string_list(char*** tokens, const char* comma_list);
 ...
 */
int BMI_initialize(const char* method_list, const char* listen_addr, 
	bmi_flag_t flags)
{
	int ret = -1;
	int i = 0;
	...
	for(i=0; i<active_method_count; i++)
	{
		...
		while((*tmp_method_ops) != NULL && 
			strcmp((*tmp_method_ops)->method_name, requested_methods[i]) != 0)
		{
			tmp_method_ops++;
		}
	...
	for(i=0; i<active_method_count; i++){
		if(flags & BMI_INIT_SERVER){
			if((new_addr =
				active_method_table[i]->BMI_meth_method_addr_lookup(listen_addr)) 
				!= NULL){
				/* this is a bit of a hack */
				new_addr->method_type = i;
				ret = active_method_table[i]->BMI_meth_initialize(new_addr, i, flags);
			}
			else{
				ret = -1;
			}
	...
	bmi_flag_t flags)
{
	...
	active_method_table = (struct bmi_method_ops**)malloc(active_method_count*sizeof(struct
		bmi_method_ops*));
	if(!active_method_table){
		ret = -ENOMEM;
		goto bmi_initialize_failure;
 ...
 * returns 0 on success, -errno on failure
 */
int BMI_post_recv(bmi_op_id_t* id, bmi_addr_t src, void* buffer,
	...
	bmi_msg_tag_t tag, void* user_ptr)
{
	...
	*id = 0;
	...
	ret = tmp_ref->interface->BMI_meth_post_recv(id, tmp_ref->method_addr, 
		buffer, expected_size, actual_size, buffer_flag, tag, user_ptr);
	gen_mutex_unlock(&interface_mutex);
	return(ret);
}
 ...
 * returns 0 on success, -errno on failure
 */
int BMI_post_send(bmi_op_id_t* id, bmi_addr_t dest, void* buffer,
	...
	void* user_ptr)
{
	...
	*id = 0;
	...
	ret = tmp_ref->interface->BMI_meth_post_send(id, tmp_ref->method_addr, 
		buffer, size, buffer_flag, tag, user_ptr);
	gen_mutex_unlock(&interface_mutex);
	return(ret);
}
 ...
 * returns 0 on success, -errno on failure
 */
int BMI_post_sendunexpected(bmi_op_id_t* id, bmi_addr_t dest, void* buffer,
	bmi_size_t size, bmi_flag_t buffer_flag, bmi_msg_tag_t tag, void*
	user_ptr)
{
	ref_st_p tmp_ref = NULL;
	int ret = -1;
	...
	*id = 0;
	...
	ret = tmp_ref->interface->BMI_meth_post_sendunexpected(id, 
		tmp_ref->method_addr, buffer, size, buffer_flag, tag, user_ptr);
	gen_mutex_unlock(&interface_mutex);
	return(ret);
}
 ...
 */
void* BMI_memalloc(bmi_addr_t addr, bmi_size_t size, bmi_flag_t send_recv)
{
	...
	new_buffer = 
		tmp_ref->interface->BMI_meth_memalloc(size, send_recv);
 ...
 * returns 0 on success, -errno on failure
 */
int BMI_memfree(bmi_addr_t addr, void* buffer, bmi_size_t size, 
...
int BMI_set_info(bmi_addr_t addr, int option, void* inout_parameter)
{
	...
	{	
		...
		for(i=0; i<active_method_count; i++){
			ret = active_method_table[i]->BMI_meth_set_info(option, inout_parameter);
...
int BMI_get_info(bmi_addr_t addr, int option, void* inout_parameter)
{
	...
	switch(option)
	{
		...
		case BMI_CHECK_MAXSIZE:
			...
			{
				ret =
					active_method_table[i]->BMI_meth_get_info(option,
					&tmp_maxsize);
				if(ret < 0)
				{
					gen_mutex_unlock(&interface_mutex);
...
int BMI_addr_lookup(bmi_addr_t* new_addr, const char* id_string){
	...
	*new_addr = 0;
	...
	while(i<active_method_count && !(meth_addr = 
		active_method_table[i]->BMI_meth_method_addr_lookup(id_string)))
	{
		i++;
	}
	...
	*id = 0;
	...
	if(tmp_ref->interface->BMI_meth_post_send_list)
	{
		ret = tmp_ref->interface->BMI_meth_post_send_list(id, 
			tmp_ref->method_addr, buffer_list, size_list, list_count, 
			total_size, buffer_flag, tag, user_ptr);
			gen_mutex_unlock(&interface_mutex);
			return(ret);
	}
	...
	*id = 0;
	...
	if(tmp_ref->interface->BMI_meth_post_recv_list)
	{
		ret = tmp_ref->interface->BMI_meth_post_recv_list(id, 
			tmp_ref->method_addr, buffer_list, size_list, list_count,
			total_expected_size, total_actual_size, buffer_flag, tag,
			user_ptr);
			gen_mutex_unlock(&interface_mutex);
			return(ret);
	}
