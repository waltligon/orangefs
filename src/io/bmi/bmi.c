...
#endif
#ifdef __STATIC_METHOD_BMI_IB_EXP__
extern struct bmi_method_ops bmi_ib_exp_ops;
#endif
#ifdef __STATIC_METHOD_BMI_RDMA__
extern struct bmi_method_ops bmi_rdma_ops;
#endif
                  ...
                  bmi_hint hints)
{
    ...
    gossip_debug(GOSSIP_BMI_DEBUG_OFFSETS,
                 "BMI_post_recv: addr: %ld, offset: 0x%lx, "
                 "size: %ld, tag: %d\n",
                  ...
                  bmi_hint hints)
{
    ...
    gossip_debug(GOSSIP_BMI_DEBUG_OFFSETS,
                 "BMI_post_send: addr: %ld, offset: 0x%lx, "
                 "size: %ld, tag: %d\n",
                            ...
                            bmi_hint hints)
{
    ...
    gossip_debug(GOSSIP_BMI_DEBUG_OFFSETS,
                 "BMI_post_sendunexpected: addr: %ld, offset: 0x%lx, "
                 "size: %ld, tag: %d\n", 
                 (long) dest, (long) buffer, (long) size, (int) tag);
 ...
 */
static void construct_poll_plan(struct method_usage_t *method_usage,
                                int nmeth, 
                                int *idle_time_ms)
{
    int i, numplan;
                 ...
                 void *inout_parameter)
{
    ...
    if (!addr)
    {
        ...
        for (i = 0; i < active_method_count; i++)
        {
            ret = active_method_table[i]->set_info(option, 
                                                   inout_parameter);
                 ...
                 void *inout_parameter)
{
    ...
    switch (option)
    {
        ...
        case BMI_TRANSPORT_METHODS_STRING:
            {
            /*
             * [OUT] inout_parameter : contains comma-separated list 
             *                         of transport protocols, memory allocated 
                                 ...
                                 bmi_hint hints)
{
    ...
    *id = 0;
    ...
    if (tmp_ref->interface->post_send_list)
    {
        ret = tmp_ref->interface->post_sendunexpected_list(id, 
                                                           tmp_ref->method_addr,
                                                           buffer_list, 
                                                           size_list,
                                                           list_count, 
 ...
 */
static int grow_method_usage (struct method_usage_t ** p, 
                              int newflags)
{
                ...
                char *options)
{
    ...
    active_method_table = malloc((active_method_count + 1) * 
    ...
    if (listen_addr) 
    {
        ...
        if (!new_addr) 
        {
            gossip_err("Error: failed to lookup listen "
                       "address %s for method %s.\n",
                       listen_addr, name);
            --active_method_count;
            return -EINVAL;
        }
...
static void bmi_check_forget_list(void)
{
    ...
    {
        tmp_item = qlist_entry(forget_list.next, 
                               struct forget_item,
                               link);     
