 ...
 */
method_addr_p gm_addr_search(struct qlist_head* head, unsigned int node_id)
{
	...
	{
		gm_addr_data = qlist_entry(tmp_entry, struct gm_addr, gm_addr_list);
		if(gm_addr_data->node_id == node_id)
		{
			...
			return((method_addr_p)((unsigned long)gm_addr_data -
				(unsigned long)sizeof(struct method_addr)));
		}
	}
	return(NULL);
}
