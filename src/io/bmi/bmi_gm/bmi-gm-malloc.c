...
/* Modified May 2002 for use in BMI GM method by Phil Carns */
...
/* 
  Patrick : modification for MPI-GM
- registration makes releasing memory to the system more costly, so changes
  some configuration parameters accordingly (TRIM and MMAP THRESHOLD)
- catch sbrk and munmap to synchronize the regcache
- protect the malloc functionalities for MPI-GM progression thread.
*/

void *bmi_gm_sbrk(int);

#define MORECORE bmi_gm_sbrk
#define munmap bmi_gm_munmap

#define DEFAULT_TRIM_THRESHOLD 4*1024*1024
...
void *sbrk(long size);
static void *mmap(void *ptr,
		  long size,
		  long prot,
		  long type,
		  long handle,
		  long arg);
static long munmap(void *ptr,
		   long size);

static void vminfo(unsigned long *free,
		   unsigned long *reserved,
		   unsigned long *committed);
static int cpuinfo(int whole,
...
static Void_t *rEALLOc(Void_t *,
		       size_t);
static Void_t *mEMALIGn(size_t,
			size_t);
static Void_t *vALLOc(size_t);
static Void_t *pVALLOc(size_t);
static Void_t *cALLOc(size_t,
		      size_t);
static Void_t **iCALLOc(size_t,
			size_t,
			Void_t **);
static Void_t **iCOMALLOc(size_t,
			  size_t *,
			  Void_t **);
...
#if __STD_C
static Void_t *sYSMALLOc(INTERNAL_SIZE_T,
			 mstate);
static int sYSTRIm(size_t,
		   mstate);
static void malloc_consolidate(mstate);
static Void_t **iALLOc(size_t,
		       size_t *,
		       int,
		       Void_t **);
#else
...
#if __STD_C
static void do_check_remalloced_chunk(mchunkptr p,
				      INTERNAL_SIZE_T s)
#else
static void do_check_remalloced_chunk(p,
				      s) mchunkptr p;
INTERNAL_SIZE_T s;
#endif
{
...
#if __STD_C
static void do_check_malloced_chunk(mchunkptr p,
				    INTERNAL_SIZE_T s)
#else
static void do_check_malloced_chunk(p,
				    s) mchunkptr p;
INTERNAL_SIZE_T s;
#endif
{
...
static Void_t *sYSMALLOc(INTERNAL_SIZE_T nb,
			 mstate av)
#else
static Void_t *sYSMALLOc(nb,
			 av) INTERNAL_SIZE_T nb;
mstate av;
#endif
{
	...
	if ((unsigned long) (size) > (unsigned long) (nb))
	{
		...
		front_misalign =
		    (INTERNAL_SIZE_T) chunk2mem(mm) & MALLOC_ALIGN_MASK;
		if (front_misalign > 0)
		{
	...
	else
	{
		...
		correction +=
		    ((end_misalign + pagemask) & ~pagemask) - end_misalign;
		...
		assert(((unsigned long) chunk2mem(brk) & MALLOC_ALIGN_MASK) ==
		       0);
...
#endif
{
	...
	cp = (char *) mremap((char *) oldp - offset, oldsize + offset, newsize,
			     1);
...
Void_t *cALLOc(size_t n_elements,
	       size_t elem_size)
#else
Void_t *cALLOc(n_elements,
	       elem_size) size_t n_elements;
size_t elem_size;
#endif
{
    mchunkptr p;
...
#if __STD_C
Void_t **iCALLOc(size_t n_elements,
		 size_t elem_size,
		 Void_t * chunks[])
#else
Void_t **iCALLOc(n_elements,
		 elem_size,
		 chunks) size_t n_elements;
size_t elem_size;
Void_t *chunks[];
#endif
{
...
#if __STD_C
Void_t **iCOMALLOc(size_t n_elements,
		   size_t sizes[],
		   Void_t * chunks[])
#else
Void_t **iCOMALLOc(n_elements,
		   sizes,
		   chunks) size_t n_elements;
size_t sizes[];
Void_t *chunks[];
...
static Void_t **iALLOc(size_t n_elements,
		       ...
		       Void_t * chunks[])
#else
static Void_t **iALLOc(n_elements,
		       sizes,
		       opts,
		       chunks) size_t n_elements;
size_t *sizes;
...
int mALLOPt(int param_number,
	    int value)
#else
int mALLOPt(param_number,
	    value) int param_number;
int value;
#endif
{
    mstate av = get_malloc_state();
...
void *sbrk(long size)
{
	...
	long to_commit =
	    (char *) g_last->top_allocated + allocate_size -
	    (char *) g_last->top_committed;
	...
	if (to_commit > 0)
	{
	    ...
	    long to_reserve =
		(char *) g_last->top_committed + commit_size -
		(char *) g_last->top_reserved;
	    /* Do we reach the reserve limit? */
	    if (to_reserve > 0)
	    {
		/* Compute the remaining size to commit in the current region */
		long remaining_commit_size =
		    (char *) g_last->top_reserved -
		    (char *) g_last->top_committed;
		if (remaining_commit_size > 0)
		{
			...
			g_last->top_committed =
			    (char *) base_committed + remaining_commit_size;
		    }
		}
		{
			...
			assert((unsigned) memory_info.BaseAddress %
			       g_pagesize == 0);
			assert(0 < reserve_size
			       && reserve_size % g_regionsize == 0);
			while (VirtualQuery
			       (memory_info.BaseAddress, &memory_info,
				sizeof(memory_info)))
			{
			    /* Assert postconditions */
			    assert((unsigned) memory_info.BaseAddress %
			...
			/* Try to reserve this */
			base_reserved =
			    VirtualAlloc(memory_info.BaseAddress, reserve_size,
		    ...
		    if (!region_list_append
			(&g_last, base_reserved, reserve_size))
			goto sbrk_exit;
		...
		/* Commit this */
		void *base_committed =
		    VirtualAlloc(g_last->top_committed, commit_size,
				 MEM_COMMIT, PAGE_READWRITE);
		/* Check returned pointer for consistency */
		if (base_committed != g_last->top_committed)
		    goto sbrk_exit;
	...
	}
	{
		...
		void *base_committed =
		    (char *) g_last->top_committed - decommit_size;
		/* Assert preconditions */
		assert((unsigned) base_committed % g_pagesize == 0);
		...
		deallocate_size -=
		    (char *) g_last->top_allocated - (char *) base_committed;
		   ...
		   unsigned long *committed)
{
    ...
    *free = *reserved = *committed = 0;
    while (VirtualQuery
	   (memory_info.BaseAddress, &memory_info, sizeof(memory_info)))
    {
	...
	memory_info.BaseAddress =
	    (char *) memory_info.BaseAddress + memory_info.RegionSize;
    }
}
