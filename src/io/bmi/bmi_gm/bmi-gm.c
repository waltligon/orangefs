...
/* function prototypes */
int BMI_gm_initialize(method_addr_p listen_addr, bmi_flag_t
	method_id, bmi_flag_t init_flags);
int BMI_gm_finalize(void);
int BMI_gm_set_info(int option, void* inout_parameter);
int BMI_gm_get_info(int option, void* inout_parameter);
void* BMI_gm_memalloc(bmi_size_t size, bmi_flag_t send_recv);
int BMI_gm_memfree(void* buffer, bmi_size_t size, bmi_flag_t send_recv);
int BMI_gm_post_send(bmi_op_id_t* id, method_addr_p dest, void* buffer,
	bmi_size_t size, bmi_size_t expected_size, bmi_flag_t buffer_flag, 
	bmi_msg_tag_t tag, void* user_ptr);
int BMI_gm_post_sendunexpected(bmi_op_id_t* id, method_addr_p dest, 
	void* buffer, bmi_size_t size, bmi_flag_t buffer_flag, bmi_msg_tag_t
	tag, void* user_ptr);
int BMI_gm_post_recv(bmi_op_id_t* id, method_addr_p src, void* buffer,
	bmi_size_t expected_size, bmi_size_t* actual_size, bmi_flag_t 
	buffer_flag, bmi_msg_tag_t tag, void* user_ptr);
int BMI_gm_wait(bmi_op_id_t id, int* outcount, bmi_error_code_t*
	error_code, bmi_size_t* actual_size, void** user_ptr);
int BMI_gm_waitsome(int incount, bmi_op_id_t* id_array, int* outcount, int*
	index_array, bmi_error_code_t* error_code_array, bmi_size_t* 
	actual_size_array, void** user_ptr_array);
int BMI_gm_waitunexpected(int incount, int* outcount, struct
	method_unexpected_info* info);
int BMI_gm_test(bmi_op_id_t id, int* outcount, bmi_error_code_t*
	error_code, bmi_size_t* actual_size, void** user_ptr);
int BMI_gm_testsome(int incount, bmi_op_id_t* id_array, int* outcount, int*
	index_array, bmi_error_code_t* error_code_array, bmi_size_t* 
	actual_size_array, void** user_ptr_array);
int BMI_gm_testunexpected(int incount, int* outcount, struct
	method_unexpected_info* info);
...
static int gm_post_send_check_resource(bmi_op_id_t* id, 
	method_addr_p dest, void* buffer, bmi_size_t size, bmi_size_t
	expected_size, bmi_msg_tag_t tag, bmi_flag_t mode, bmi_flag_t
	buffer_status, void* user_ptr);
static void ctrl_req_callback(struct gm_port *port, void* context,
	gm_status_t status);
...
static int high_recv_handler(gm_recv_event_t* poll_event, int fast);
static void ctrl_ack_handler(bmi_op_id_t ctrl_op_id, unsigned int node_id,
	gm_remote_ptr_t remote_ptr, bmi_op_id_t peer_op_id);
static int ctrl_req_handler_rend(bmi_op_id_t ctrl_op_id, bmi_size_t
	ctrl_actual_size, bmi_size_t ctrl_expected_size, 
	bmi_msg_tag_t ctrl_tag, unsigned int node_id);
static int immed_unexp_recv_handler(bmi_size_t size, bmi_msg_tag_t
	msg_tag, method_addr_p map, void* buffer);
static int immed_recv_handler(bmi_size_t actual_size, bmi_size_t
	expected_size, bmi_msg_tag_t msg_tag, method_addr_p map, void* buffer);
static void put_recv_handler(bmi_op_id_t ctrl_op_id);
static void ctrl_ack_callback(struct gm_port *port, void* context,
	gm_status_t status);
static void data_send_callback(struct gm_port *port, void* context,
	gm_status_t status);
static void immed_send_callback(struct gm_port *port, void* context,
   gm_status_t status);
static void ctrl_put_callback(struct gm_port *port, void* context,
   gm_status_t status);
static int test_done_some(int incount, bmi_op_id_t* id_array, int* outcount, 
	...
	actual_size_array, void** user_ptr);
...
static int gm_generic_testwait(bmi_op_id_t id, int* outcount, 
	bmi_error_code_t* error_code, bmi_size_t* actual_size, 
	void** user_ptr, int wait_time);
static int gm_generic_testwaitsome(int incount, bmi_op_id_t* id_array, 
...
static int gm_generic_testwaitunexpected(int incount, int* outcount, struct
	...
	*id = 0;
	...
	if(expected_size <= GM_IMMED_LENGTH)
	{
		...
		return (gm_post_send_check_resource(id, dest, buffer, size,
			expected_size, tag, new_ctrl_msg->u.immed.mode, buffer_status, 
			user_ptr));
	}
	else
	{
		...
		return (gm_post_send_check_resource(id, dest, buffer, size,
			expected_size, tag, GM_MODE_REND, buffer_status, user_ptr));
	}
}
	...
	tag, void* user_ptr)
{
	...
	*id = 0;
	...
	return (gm_post_send_check_resource(id, dest, buffer, size,
		0, tag, new_ctrl_msg->u.immed.mode, buffer_status, user_ptr));
}
 ...
 */
int BMI_gm_post_recv(bmi_op_id_t* id, method_addr_p src, void* buffer,
	bmi_size_t expected_size, bmi_size_t* actual_size, bmi_flag_t buffer_flag, 
	bmi_msg_tag_t tag, void* user_ptr)
{
	bmi_flag_t mode = 0;
		...
		if(mode == GM_MODE_REND)
		{
			...
			if(gm_alloc_send_token(local_port, GM_HIGH_PRIORITY))
			{
				...
				else
				{
					...
					op_list_add(op_list_array[IND_NEED_SEND_TOK_HI_CTRLACK], query_op);
					ret = 0;
				}
#endif /* ENABLE_GM_BUFPOOL */
			}
 ...
 * returns 0 on success, -errno on failure
 */
static int gm_post_send_check_resource(bmi_op_id_t* id, method_addr_p dest, 
	void* buffer, bmi_size_t size, bmi_size_t expected_size, bmi_msg_tag_t tag, 
	bmi_flag_t mode, bmi_flag_t buffer_status, void* user_ptr)
{
	method_op_p new_method_op = NULL;
	int ret = -1;
	struct gm_addr* gm_data = NULL;
...
static void initiate_send_immed(method_op_p mop)
{
	...
	gm_send_to_peer_with_callback(local_port, mop->buffer,
		GM_IMMED_SIZE, true_msg_len, GM_HIGH_PRIORITY, gm_addr_data->node_id, 
...
static void initiate_put_announcement(method_op_p mop)
{
	...
	gm_send_to_peer_with_callback(local_port, my_ctrl,
		GM_IMMED_SIZE, GM_CTRL_LENGTH, GM_HIGH_PRIORITY, gm_addr_data->node_id, 
...
#endif /* ENABLE_GM_BUFPOOL */
				query_op = 
					op_list_shownext(op_list_array[IND_NEED_SEND_TOK_HI_CTRLACK]);
				op_list_remove(query_op);
				prepare_for_recv(query_op);
				return;
#ifdef ENABLE_GM_BUFPOOL
...
static int high_recv_handler(gm_recv_event_t* poll_event, int fast)
{
	...
	if(fast)
	{
		my_ctrl = (struct ctrl_msg*)(gm_ntohp(poll_event->recv.message) + 
			gm_ntohl(poll_event->recv.length) - sizeof(struct ctrl_msg));
	}
	else
	{
	...
	{	
		...
		gm_provide_receive_buffer(local_port,
			gm_ntohp(poll_event->recv.buffer), 
			GM_IMMED_SIZE, GM_HIGH_PRIORITY);
		ret = immed_unexp_recv_handler(ctrl_actual_size, ctrl_tag, map, tmp_buffer);
	}
	else if(my_ctrl->ctrl_type == CTRL_PUT_TYPE)
	{
		ctrl_op_id = my_ctrl->u.put.receiver_op_id;
		gm_provide_receive_buffer(local_port,
			gm_ntohp(poll_event->recv.buffer), GM_IMMED_SIZE,
			GM_HIGH_PRIORITY);
 ...
 * returns 0 on success, -errno on failure
 */
static void ctrl_ack_handler(bmi_op_id_t ctrl_op_id, unsigned int
	node_id, gm_remote_ptr_t remote_ptr, bmi_op_id_t peer_op_id)
{
	method_op_p query_op = NULL;
	struct gm_op* gm_op_data = NULL;
...
static void send_data_buffer(method_op_p mop)
{
	...
	gm_directed_send_with_callback(local_port, mop->buffer,
		...
		mop->actual_size, GM_LOW_PRIORITY, gm_addr_data->node_id, 
		BMI_GM_PORT_NUM, data_send_callback, mop);
...
static void prepare_for_recv(method_op_p mop)
{
	...
	if(gm_op_data->buffer_status == GM_BUF_METH_REG)
	{
		...
		new_ctrl->u.ack.remote_ptr = 0;
		new_ctrl->u.ack.remote_ptr += (unsigned long)gm_op_data->tmp_xfer_buffer;
#endif /* ENABLE_GM_BUFCOPY */
#ifdef ENABLE_GM_BUFPOOL
		pinned_size = mop->actual_size;
		gm_op_data->tmp_xfer_buffer = bmi_gm_bufferpool_get(io_pool);
...
#endif /* ENABLE_GM_REGCACHE */
#ifdef ENABLE_GM_REGISTER
		gm_deregister_memory(local_port, my_op->buffer,
			my_op->actual_size);
#endif /* ENABLE_GM_REGISTER */
#ifdef ENABLE_GM_BUFCOPY
		gm_dma_free(local_port, my_op->buffer);
	...
	bmi_msg_tag_t ctrl_tag, unsigned int node_id)
{
	...
	if(active_method_op)
	{
		...
		{
			/* we don't have enough tokens */
			op_list_add(op_list_array[IND_NEED_SEND_TOK_HI_CTRLACK], active_method_op);
			ret = 0;
		}
	}
	else
