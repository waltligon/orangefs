 ...
 *
 * $Id: ib.c,v 1.34.2.1 2006-07-21 17:40:26 kunkel Exp $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void encourage_send_incoming_cts(
    buf_head_t * bh,
    u_int32_t byte_len);
static void encourage_recv_incoming(
    ib_connection_t * c,
    buf_head_t * bh,
    u_int32_t byte_len);
static void encourage_recv_incoming_cts_ack(
    ib_recv_t * rq);
static int send_cts(
    ib_recv_t * rq);
static void maybe_free_connection(
    ib_connection_t * c);
static void ib_close_connection(
    ib_connection_t * c);
#ifndef __PVFS2_SERVER__
static int ib_tcp_client_connect(
    ib_method_addr_t * ibmap,
    struct method_addr *remote_map);
#endif
static int ib_tcp_server_check_new_connections(
    void);
static int ib_block_for_activity(
    int timeout_ms);

/*
 * Return string form of work completion opcode field.
 */
static const char *wc_opcode_string(
    int opcode)
{
 ...
 */
static void encourage_send_waiting_buffer(
    ib_send_t * sq)
{
    /*
    ...
    if (sq->buflist.tot_len <= ib_device->eager_buf_payload)
    {
        ...
        memcpy_from_buflist(&sq->buflist, (msg_header_eager_t *) bh->buf + 1);
 ...
 */
static ib_recv_t *find_matching_recv(
    rq_state_t statemask,
 ...
 */
static ib_recv_t *alloc_new_recv(
    ib_connection_t * c,
    buf_head_t * bh)
{
    ...
    if (mh_common.type == MSG_EAGER_SEND)
    {
        ...
        if (rq)
        {
            ...
            memcpy_to_buflist(&rq->buflist,
                              (msg_header_eager_t *) bh->buf + 1, len);
...
static int BMI_ib_post_send_list(
    bmi_op_id_t * id,
    struct method_addr *remote_map,
    const void *const *buffers,
    const bmi_size_t * sizes,
...
static int BMI_ib_post_sendunexpected(
    bmi_op_id_t * id,
    struct method_addr *remote_map,
    const void *buffer,
    bmi_size_t size,
...
static int BMI_ib_post_sendunexpected_list(
    bmi_op_id_t * id,
    struct method_addr *remote_map,
    const void *const *buffers,
    const bmi_size_t * sizes,
    ...
    bmi_context_id context_id)
{
    ...
    rq = find_matching_recv(RQ_EAGER_WAITING_USER_POST |
                            RQ_RTS_WAITING_USER_POST, c, tag);
    if (rq)
    {
...
static int BMI_ib_post_recv_list(
    bmi_op_id_t * id,
    struct method_addr *remote_map,
    void *const *buffers,
    const bmi_size_t * sizes,
 ...
 */
static int BMI_ib_testunexpected(
    ...
    *outcount = 0;
    qlist_for_each(l, &ib_device->recvq)
    {
        ...
        if (rq->state == RQ_EAGER_WAITING_USER_TESTUNEXPECTED)
        {
            ...
            memcpy(ui->buffer,
                   (msg_header_eager_t *) rq->bh->buf + 1, (size_t) ui->size);
            ui->tag = rq->bmi_tag;
 ...
 */
static int BMI_ib_open_context(
    bmi_context_id context_id __unused)
{
    return 0;
}

static void BMI_ib_close_context(
    bmi_context_id context_id __unused)
{
}
...
static const char *BMI_ib_rev_lookup(
    struct method_addr *meth)
{
 ...
 */
static struct method_addr *ib_alloc_method_addr(
    ib_connection_t * c,
    const char *hostname,
    int port)
{
 ...
 */
static struct method_addr *BMI_ib_method_addr_lookup(
    const char *id)
{
...
static ib_connection_t *ib_new_connection(
    int sock,
    const char *peername,
    int is_server)
{
    ...
    c->eager_send_buf_head_contig = Malloc(ib_device->eager_buf_num
                                           *
                                           sizeof(*c->
                                                  eager_send_buf_head_contig));
    c->eager_recv_buf_head_contig =
        Malloc(ib_device->eager_buf_num *
 ...
 */
static void ib_tcp_server_init_listen_socket(
    struct method_addr *addr)
{
  ...
  retry:
    if (bind(ib_device->listen_sock, (struct sockaddr *) &skin, sizeof(skin)) <
        0)
    {
        if (errno == EINTR)
            goto retry;
...
#ifdef OPENIB
extern int openib_ib_initialize(
    void);
#endif
#ifdef VAPI
extern int vapi_ib_initialize(
    void);
#endif
 ...
 */
static int BMI_ib_initialize(
    struct method_addr *listen_addr,
    int method_id,
    int init_flags)
{
    int ret;
    ...
    ib_device->eager_buf_payload =
        ib_device->eager_buf_size - sizeof(msg_header_eager_t);
