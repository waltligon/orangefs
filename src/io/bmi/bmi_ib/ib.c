...
static int ib_check_cq(void)
{
    ...
    for (;;)
    {
        ...
        if (wc.status != 0)
        {
            ...
            {
                debug(0,
                      "%s: entry id 0x%llx SEND error %s to %s",
                      __func__,
                      llu(wc.id),
        ...
        if (wc.opcode == BMI_IB_OP_RECV)
        {
            ...
            debug(2,
                  "%s: recv from %s len %d type %s credit %d",
                  __func__,
                  bh->c->peername,
                  byte_len,
        ...
        else if (wc.opcode == BMI_IB_OP_SEND)
        {
            ...
            if (sq == NULL)
            {
                ...
                debug(2,
                      "%s: MSG_BYE or MSG_CREDIT completed locally",
                      __func__);
            }
            else if (sq->type == BMI_SEND)
            {
                ...
                {
                    bmi_ib_assert(0,
                                  "%s: unknown send state %s (%d) of sq %p",
                                  __func__,
                                  sq_state_name(sq->state.send),
                                  sq->state.send, sq);
                }
                ...
                debug(2,
                      "%s: send to %s completed locally: sq %p -> %s",
                      __func__,
                      bh->c->peername,
                      sq,
                      sq_state_name(sq->state.send));
            ...
            else
            {
                ...
                {
                    bmi_ib_assert(0,
                                  "%s: unknown recv state %s of rq %p",
                                  __func__,
                ...
                debug(2,
                      "%s: send to %s completed locally: rq %p -> %s",
                      __func__,
                      bh->c->peername,
                      rq,
                      rq_state_name(rq->state.recv));
        ...
        {
            error("%s: cq entry id 0x%llx opcode %d unexpected",
                  __func__,
                  llu(wc.id),
                  wc.opcode);
        }
...
static struct buf_head *get_eager_buf(ib_connection_t *c)
{
    ...
    if (c->send_credit > 0)
    {
        ...
        bmi_ib_assert(bh,
                      "%s: empty eager_send_buf_free list, peer %s",
                      __func__,
                      c->peername);
    }
    return bh;
...
static void encourage_send_waiting_buffer(struct ib_work *sq)
{
    ...
    {
        debug(2,
              "%s: sq %p no free send buffers to %s",
              __func__,
              sq,
              c->peername);
        return;
    ...
    else
    {
        ...
        debug(2,
              "%s: sq %p sent RTS mopid %llx len %lld",
              __func__,
              sq,
              llu(sq->mop->op_id),
              lld(sq->buflist.tot_len));
                                        ...
                                        u_int32_t byte_len)
{
    ...
    {
        debug(8,
              "%s: looking for op_id 0x%llx, consider 0x%llx",
              __func__,
                                    ...
                                    u_int32_t byte_len)
{
    ...
    if (type == MSG_EAGER_SEND)
    {
        ...
        if (rq)
        {
            ...
            {
                debug(2,
                      "%s: early registration not needed, dereg after eager",
                      __func__);
...
static void encourage_rts_done_waiting_buffer(struct ib_work *sq)
{
    ...
    debug(2,
          "%s: sq %p sent RTS_DONE mopid %llx",
...
static int send_cts(struct ib_work *rq)
{
    ...
    debug(2,
          "%s: rq %p from %s mopid %llx len %lld", 
          __func__,
          rq,
          rq->c->peername,
          llu(rq->rts_mop_id),
    ...
    {
        debug(2,
              "%s: rq %p no free send buffers to %s",
                     ...
                     bmi_context_id context_id)
{
    ...
    *id = mop->op_id;
    ...
    if (rq->state.recv == RQ_EAGER_WAITING_USER_POST)
    {
        ...
        debug(2,
              "%s: rq %p state %s finish eager directly",
              __func__,
              rq,
              rq_state_name(rq->state.recv));
                                          ...
                                          int is_server)
{
    ...
    c->eager_send_buf_head_contig =
            bmi_ib_malloc(ib_device->eager_buf_num *
...
static int ib_tcp_server_check_new_connections(void)
{
    ...
    else
    {
        ...
        debug(2,
              "%s: accepted new connection %s at server",
              __func__, c->peername);
 ...
 * them.
 */
static int BMI_ib_get_info(int option,
                           void *param)
{
    int ret = 0;
