 ...
 *
 * $Id: openib.c,v 1.3.2.1 2006-07-21 17:40:27 kunkel Exp $
 */
#include <string.h>
#include <errno.h>
#include <fcntl.h>
...
static void init_connection_modify_qp(
    ...
    uint32_t remote_qp_num,
    int remote_lid);
static void openib_post_rr(
    const ib_connection_t * c,
    buf_head_t * bh);
int openib_ib_initialize(
    void);
static void openib_ib_finalize(
    void);

/*
 * Build new conneciton.
 */
static int openib_new_connection(
    ib_connection_t * c,
    int sock,
    int is_server)
{
    ...
    else if (att.cap.max_send_wr < od->max_unsignaled_sends)
    ...
    else if (att.cap.max_send_wr < od->max_ack_unsignaled_sends)
    ...
    int remote_lid)
{
    ...
    mask = IBV_QP_STATE | IBV_QP_ACCESS_FLAGS | IBV_QP_PKEY_INDEX | IBV_QP_PORT;
    ...
    mask =
        ...
        | IBV_QP_AV
        | IBV_QP_PATH_MTU
        | IBV_QP_RQ_PSN | IBV_QP_DEST_QPN | IBV_QP_MIN_RNR_TIMER;
    memset(&attr, 0, sizeof(attr));
    ...
    ib_connection_t * c)
{
    ...
    sg.addr = int64_from_ptr(bh->buf),
        ...
        sg.lkey = oc->eager_send_mr->lkey, memset(&sr, 0, sizeof(sr));
 ...
 * later.
 */
static void openib_close_connection(
    ib_connection_t * c)
{
    int ret;
    struct openib_connection_priv *oc = c->priv;
 ...
 * at QP build time.
 */
static void openib_post_sr_ack(
    ib_connection_t * c,
    int his_bufnum)
{
    struct openib_connection_priv *oc = c->priv;
 ...
 * thinking.
 */
static void openib_post_rr_ack(
    const ib_connection_t * c,
    const buf_head_t * bh)
{
    int ret;
 ...
 */
static void openib_post_sr_rdmaw(
    ib_send_t * sq,
...
static int openib_prepare_cq_block(
    void)
{
    struct openib_device_priv *od = ib_device->priv;
    int ret;
...
static const char *openib_wc_status_string(
    int status)
{
...
static const char *openib_port_state_string(
...
static const char *async_event_type_string(
 ...
 */
static void openib_mem_register(
    memcache_entry_t * c)
{
    struct ibv_mr *mrh;
    struct openib_device_priv *od = ib_device->priv;
...
static void openib_mem_deregister(
    memcache_entry_t * c)
{
    int ret;
    struct ibv_mr *mrh;
...
static struct ibv_device *get_nic_handle(
    void)
{
...
static int openib_check_async_events(
    void)
{
    struct openib_device_priv *od = ib_device->priv;
    int ret;
