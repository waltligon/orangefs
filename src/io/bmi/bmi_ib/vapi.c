 ...
 *
 * $Id: vapi.c,v 1.3.2.1 2006-07-21 17:40:27 kunkel Exp $
 */
#include <stdio.h>
#include <string.h>
#include <stdarg.h>
    ...
    int remote_lid)
{
    ...
    QP_ATTR_MASK_SET(mask,
                     ...
                     | QP_ATTR_PATH_MTU
                     | QP_ATTR_RQ_PSN
                     | QP_ATTR_DEST_QP_NUM | QP_ATTR_MIN_RNR_TIMER);
    ...
    QP_ATTR_MASK_SET(mask,
                     ...
                     | QP_ATTR_TIMEOUT
                     | QP_ATTR_RETRY_COUNT | QP_ATTR_RNR_RETRY);
    ...
    ib_connection_t * c)
{
    ...
    /* transition to drain */
    QP_ATTR_MASK_CLR_ALL(mask);
    QP_ATTR_MASK_SET(mask, QP_ATTR_QP_STATE);
 ...
 * later.
 */
static void vapi_close_connection(
    ib_connection_t * c)
{
    int ret;
    struct vapi_connection_priv *vc = c->priv;
 ...
 */
static void vapi_post_sr_rdmaw(
    ib_send_t * sq,
...
static int vapi_prepare_cq_block(
    void)
{
    struct vapi_device_priv *vd = ib_device->priv;
    int ret;
 ...
 */
static const char *vapi_wc_status_string(
    int status)
{
...
static const char *vapi_port_state_string(
...
static void vapi_mem_deregister(
    memcache_entry_t * c)
{
    struct vapi_device_priv *vd = ib_device->priv;
    VAPI_mr_hndl_t mrh;
...
static void async_event_handler(
    VAPI_hca_hndl_t nic_handle_in __attribute__ ((unused)),
 ...
 * ready.
 */
static void cq_event_handler(
    VAPI_hca_hndl_t hca __attribute__ ((unused)),
 ...
 * them.
 */
static int vapi_check_async_events(
    void)
{
    struct vapi_device_priv *vd = ib_device->priv;
    int ret;
