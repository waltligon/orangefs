/*
 * Experimental InfiniBand BMI method.
 *
 ...
 *   ib_exp://hostname:port/filesystem
...
static struct bmi_method_addr *BMI_ib_method_addr_lookup(const char *id)
{
    ...
    /* parse hostname */
    s = string_key("ib_exp", id);  /* allocs a string */
    if (!s)
    {
        return 0;
    }
                             ...
                             char *options)
{
    ...
    debug(0, "Initializing IB (experimental) module");
    ...
    debug(0, "IB (experimental) module successfully initialized");
    return ret;
}
