...
mx_unexp_handler_action_t bmx_unexpected_recv(void *context,
                                              mx_endpoint_addr_t source,
                                              uint64_t match_value,
                                              uint32_t length,
                                              void *data_if_available);

static int bmx_peer_connect(struct bmx_peer *peer);
static void bmx_create_peername(void);

void *BMI_mx_memalloc(bmi_size_t size,
                      enum bmi_op_type send_recv);

static int bmx_peer_init_state(struct bmx_peer *peer) GCC_UNUSED;
...
static void bmx_get_completion_token(void)
{
    int done = 0;
...
static void bmx_release_completion_token(void)
{
    gen_mutex_lock(&bmi_mx->bmx_completion_lock);
    bmi_mx->bmx_refcount++;
...
static void bmx_ctx_free(struct bmx_ctx *ctx)
{
    if (ctx == NULL) return;
...
static int bmx_ctx_alloc(struct bmx_ctx **ctxp,
                         enum bmx_req_type type)
{
...
static void bmx_ctx_init(struct bmx_ctx *ctx)
{
...
static void bmx_q_ctx(struct bmx_ctx *ctx)
{
    ...
    if (ctx->mxc_type == BMX_REQ_TX)
    {
        queue = &peer->mxp_queued_txs;
    }
    else
    {
        queue = &peer->mxp_queued_rxs;
    }
...
static void bmx_q_pending_ctx(struct bmx_ctx *ctx)
{
    struct bmx_peer *peer = ctx->mxc_peer;
...
static void bmx_deq_pending_ctx(struct bmx_ctx *ctx)
{
    struct bmx_peer *peer = ctx->mxc_peer;
...
static void bmx_deq_unex_rx(struct bmx_ctx **rxp)
{
...
static void bmx_q_completed(struct bmx_ctx *ctx,
                            enum bmx_ctx_state state,
                            mx_status_t status,
...
static void bmx_deq_completed(struct bmx_ctx **ctxp,
...
static struct bmx_ctx *bmx_get_idle_rx(void)
{
    struct bmx_ctx *rx = NULL;
...
static void bmx_put_idle_ctx(struct bmx_ctx *ctx)
{
...
static void bmx_reduce_idle_rxs(int count)
{
    int i = 0;
    struct bmx_ctx  *rx = NULL;
...
static struct bmx_ctx *bmx_get_idle_tx(void)
{
...
static int bmx_verify_hostname(char *host)
{
    int ret = 0;
    int len = 0;
...
static int bmx_verify_num_str(char *num_str)
{
    int ret = 0;
    int len = 0;
...
static int bmx_parse_peername(const char *peername,
                              char **hostname,
                              uint32_t *board,
                              uint32_t *ep_id)
{
    ...
    else
    {
        ...
        if (colon1 == colon2)
        {
            /* colon2_found == 0 */
            debug(BMX_DB_INFO,
                  "parse_peername() MX hostname does not "
    ...
    {
        if (NULL != strchr(colon1, ':') || NULL != strchr(colon2, ':'))
        {
            debug(BMX_DB_INFO,
                  "parse_peername() too many ':' (%s %s)",
                  colon1,
                  colon2);
            free(tmp_peername);
            return -1;
...
static void bmx_peer_free(struct bmx_peer *peer)
{
    ...
    if (!qlist_empty(&peer->mxp_list))
    {
        qlist_del_init(&peer->mxp_list);
    }
...
static void bmx_peer_addref(struct bmx_peer *peer)
{
    gen_mutex_lock(&peer->mxp_lock);
...
static void bmx_peer_decref(struct bmx_peer *peer)
{
    BMX_ENTER;
    gen_mutex_lock(&peer->mxp_lock);
    ...
    {
        debug(BMX_DB_WARN,
              "peer_decref() called for %s when refcount == 0",
    ...
    if (peer->mxp_refcount == 1 && peer->mxp_state == BMX_PEER_DISCONNECT)
    {
        ...
        debug(BMX_DB_PEER,
              "Setting peer %s to BMX_PEER_INIT",
              peer->mxp_mxmap->mxm_peername);
...
static int bmx_peer_alloc(struct bmx_peer **peerp,
                          struct bmx_method_addr *mxmap)
{
    int i = 0;
    int ret = 0;
    ...
    {
        debug((BMX_DB_MX|BMX_DB_WARN),
              "peer_alloc() unable to lookup nic_id "
              "for %s (mx_hostname_to_nic_id() returned %s)",
              name,
              mx_strerror(mxret));
        bmx_peer_free(peer);
...
static int bmx_peer_init_state(struct bmx_peer *peer)
{
    int ret = 0;
...
/* init bmi_mx */
static int bmx_globals_init(int method_id)
{
    int i = 0;
...
static int bmx_open_endpoint(mx_endpoint_t *ep,
                             uint32_t board,
                             uint32_t ep_id)
{
    mx_return_t mxret = MX_SUCCESS;
    ...
    mxret = mx_register_unexp_handler(*ep,
                                    (mx_unexp_handler_t) bmx_unexpected_recv,
                                    NULL);
    if (mxret != MX_SUCCESS)
    {
        debug(BMX_DB_WARN,
              "mx_register_unexp_callback() failed with %s",
              mx_strerror(mxret));
    ...
    {
        debug(BMX_DB_WARN,
              "mx_set_request_timeout() failed with %s",
              mx_strerror(mxret));
        mx_close_endpoint(*ep);
        mx_finalize();
        return -1;
...
static int BMI_mx_initialize(bmi_method_addr_p listen_addr,
                             int method_id,
                             int init_flags)
{
    int i = 0;
    ...
    {
        debug(BMX_DB_ERR,
              "mx_initialize() with illegal parameters. "
...
#if BMX_MEM_ACCT
    debug(BMX_DB_MEM,
          "memory used at end of initialization %lld",
          llu(mem_used));
#endif
    BMX_EXIT;
...
static int BMI_mx_finalize(void)
{
 ...
 */
static void bmx_peer_disconnect(struct bmx_peer *peer,
                                int mx_dis,
                                bmi_error_code_t err)
{
    ...
    debug(BMX_DB_CONN,
          "%s for %s in state %d (%d)",
          __func__,
          peer->mxp_mxmap->mxm_peername,
          peer->mxp_state, mx_dis);
    gen_mutex_lock(&peer->mxp_lock);
    ...
    debug(BMX_DB_PEER,
          "Setting peer %s to BMX_PEER_DISCONNECT",
          peer->mxp_mxmap->mxm_peername);
...
static int BMI_mx_set_info(int option,
    ...
    switch (option)
    {
        case BMI_DROP_ADDR:
                if (inout_parameter != NULL)
                {
                    ...
                    if (mxmap->mxm_peername != NULL)
                    {
                        debug(BMX_DB_PEER,
                              "%s drop %s map 0x%p mxmap 0x%p)",
                              __func__,
                              mxmap->mxm_peername,
                              map,
                              mxmap);
...
static int BMI_mx_get_info(int option,
                           void *inout_parameter)
{
    int ret = 0;
...
static void *bmx_memalloc(bmi_size_t size,
                          int type)
{
    void *buf = NULL;
#if BMX_MEM_TWEAK
...
static int bmx_memfree(void *buffer,
                       bmi_size_t size,
                       int type)
{
#if BMX_MEM_TWEAK
...
static int BMI_mx_memfree(void *buffer,
                          bmi_size_t size,
                          enum bmi_op_type send_recv)
{
    int ret = 0;
...
static int BMI_mx_unexpected_free(void *buf)
{
    int ret = 0;
...
static void bmx_parse_match(uint64_t match,
                            uint8_t *type,
                            uint32_t *id,
                            uint32_t *tag)
{
...
static void bmx_create_match(struct bmx_ctx *ctx)
{
    int connect = 0;
    ...
    {
        debug(BMX_DB_INFO,
              "create_match() for %s called with connect = %d",
              ctx->mxc_type == BMX_REQ_TX ? "TX" : "RX",
              connect);
    }
    ...
    if ((id >> 20) != 0)
    {
        debug(BMX_DB_ERR,
              "invalid %s of %llu\n",
              ctx->mxc_type == BMX_REQ_TX ? "mxp_tx_id" : "mxp_rx_id",
...
static bmi_error_code_t bmx_mx_to_bmi_errno(enum mx_status_code code)
{
    int err = 0;
 ...
 */
static int bmx_post_tx(struct bmx_ctx *tx)
{
    int ret = 0;
    ...
    debug((BMX_DB_FUNC|BMX_DB_CTX),
          "entering %s match= 0x%llx length= %lld peer state= %d op_id= %llu",
          __func__,
          llu(tx->mxc_match),
          lld(tx->mxc_nob),
          peer->mxp_state,
          llu(tx->mxc_mop->op_id));
    ...
    else
    {
        /* peer is not ready */
        debug(BMX_DB_PEER,
              "%s peer is not ready (%d) q_ctx(tx) match= 0x%llx length=%lld",
              __func__,
              peer->mxp_state,
              llu(tx->mxc_match),
              lld(tx->mxc_nob));
...
static int bmx_ensure_connected(struct bmx_method_addr *mxmap)
{
    int ret = 0;
    ...
    if (peer == NULL)
    {
        ...
        if (ret != 0)
        {
            debug((BMX_DB_CONN|BMX_DB_MEM),
                  "%s could not allocate peer for %s",
                  __func__,
                  mxmap->mxm_peername);
            goto out;
        }
    ...
    {
        debug(BMX_DB_CONN,
              "%s calling bmx_peer_connect() for %s",
              __func__,
              mxmap->mxm_peername);
...
static int bmx_post_send_common(bmi_op_id_t *id,
                                ...
                                PVFS_hint hints)
{
    ...
    PINT_EVENT_START(bmi_mx_send_event_id,
                     bmi_mx_pid,
                     NULL,
                     &eid,
    ...
    {
        debug(BMX_DB_INFO,
              "user provided total length %lld does not match "
    ...
    *id = mop->op_id;
    ...
    debug(BMX_DB_CTX,
          "%s tag= %d length= %d %s op_id= %llu context_id= %lld",
          __func__,
          tag,
          (int) total_size,
...
static int BMI_mx_post_send(bmi_op_id_t *id,
                            struct bmi_method_addr *remote_map,
                            const void *buffer,
                            bmi_size_t size,
...
static int BMI_mx_post_send_list(bmi_op_id_t *id,
                                 struct bmi_method_addr *remote_map,
                                 const void *const *buffers,
                                 const bmi_size_t *sizes,
                                 int list_count,
...
static int BMI_mx_post_sendunexpected(bmi_op_id_t *id,
                                    struct bmi_method_addr *remote_map,
                                    const void *buffer,
                                    bmi_size_t size,
                                    enum bmi_buffer_type buffer_flag __unused,
...
static int BMI_mx_post_sendunexpected_list(bmi_op_id_t *id,
                                   struct bmi_method_addr *remote_map,
                                   const void *const *buffers,
                                   const bmi_size_t *sizes,
                                   int list_count,
                                   ...
                                   PVFS_hint hints)
{
        ...
        return bmx_post_send_common(id,
                                    remote_map,
                                    list_count,
                                    buffers,
                                    sizes,
 ...
 */
static int bmx_post_rx(struct bmx_ctx *rx)
{
    int ret = 0;
    ...
    else
    {
        /* peer is not ready */
        debug(BMX_DB_PEER,
              "%s peer is not ready (%d) q_ctx(rx) match= 0x%llx length=%lld",
              __func__,
              peer->mxp_state,
              llu(rx->mxc_match),
              (long long) rx->mxc_nob);
...
static int bmx_post_recv_common(bmi_op_id_t *id,
                                struct bmi_method_addr *remote_map,
                                int numbufs,
                                void *const *buffers,
                                const bmi_size_t *sizes,
                                ...
                                PVFS_hint hints)
{
    ...
    PINT_EVENT_START(bmi_mx_recv_event_id,
                     bmi_mx_pid,
                     NULL,
                     &eid,
                     PINT_HINT_GET_CLIENT_ID(hints),
    ...
    {
        debug(BMX_DB_INFO,
              "user provided total length %d does not match "
              "the buffer list total length %lld",
              (uint32_t) tot_expected_len,
              (long long) rx->mxc_nob);
    }
    ...
    *id = mop->op_id;
    ...
    debug(BMX_DB_CTX,
          "%s tag= %d length= %d op_id= %llu",
          __func__,
          tag,
          (int) tot_expected_len,
          llu(mop->op_id));
...
static int BMI_mx_post_recv(bmi_op_id_t *id,
                            struct bmi_method_addr *remote_map,
                            void *buffer,
...
static int BMI_mx_post_recv_list(bmi_op_id_t *id,
                                 struct bmi_method_addr *remote_map,
                                 void *const *buffers,
                                 const bmi_size_t *sizes,
                                 int list_count,
...
static void bmx_peer_post_queued_rxs(struct bmx_peer *peer)
{
    struct bmx_ctx *rx = NULL;
    list_t *queued_rxs = &peer->mxp_queued_rxs;
...
static void bmx_peer_post_queued_txs(struct bmx_peer *peer)
{
    struct bmx_ctx *tx = NULL;
    list_t *queued_txs = &peer->mxp_queued_txs;
                                    ...
                                    uint32_t length)
{
    ...
    if (rx != NULL)
    {
        ...
        {
            debug(BMX_DB_PEER,
                  "unknown peer sent message 0x%llx length %u",
                  llu(match),
                  length);
        }
        ...
        if (mxret != MX_SUCCESS)
        {
            debug((BMX_DB_MX|BMX_DB_CTX),
                  "mx_irecv() failed with %s for an "
    ...
    switch (type)
    {
        ...
        case BMX_MSG_CONN_ACK:
                ...
                if (peer == NULL)
                {
                    debug((BMX_DB_CONN|BMX_DB_PEER),
                          "receiving CONN_ACK but the endpoint "
                          "context does not have a peer");
                }
                else
                {
                    debug(BMX_DB_CONN,
                          "CONN_ACK from %s id= %d",
                          peer->mxp_mxmap->mxm_peername,
                          id);
                    if (tag == BMX_VERSION)
                    {
                        debug(BMX_DB_CONN,
                              "setting %s's state to READY",
                              peer->mxp_mxmap->mxm_peername);
                        gen_mutex_lock(&peer->mxp_lock);
                        peer->mxp_tx_id = id;
        ...
        case BMX_MSG_UNEXPECTED:
                if (!bmi_mx->bmx_is_server)
                {
                    ...
                    {
                        debug(BMX_DB_ERR,
                              "client receiving unexpected message "
                ...
                ret = bmx_post_unexpected_recv(source,
                                               type,
                                               id,
                                               tag,
                                               match_value,
        ...
        case BMX_MSG_EXPECTED:
                ...
                debug(BMX_DB_CTX,
                      "Early expected message  length %u  tag %u  match "
                      "0x%llx",
                      length,
                      tag,
                      llu(match_value));
        ...
        default:
                debug(BMX_DB_ERR,
                      "received unexpected msg with type %d",
                      type);
                exit(1);
                break;
    }
...
/* This is called before BMI_mx_initialize() on servers, 
 * do not use anything from bmx_data 
 */
static struct bmi_method_addr *bmx_alloc_method_addr(const char *peername,
                                                     const char *hostname,
                                                     uint32_t board,
                                                     uint32_t ep_id)
{
    ...
    if (bmi_mx == NULL)
    {
        map = bmi_alloc_method_addr(tmp_id,
                                    (bmi_size_t) sizeof(*mxmap));
    }
    else
    {
        map = bmi_alloc_method_addr(bmi_mx->bmx_method_id,
                                    (bmi_size_t) sizeof(*mxmap));
    }
    if (map == NULL)
    {
 ...
 *    send CONN_REQ
 */
static void bmx_handle_icon_req(void)
{
    uint32_t result = 0;
    ...
    do {
        ...
        if (result)
        {
            ...
            struct bmx_ctx *tx = NULL;
            struct bmx_peer *peer = NULL;
            struct bmx_method_addr *mxmap f= NULL;

            peer = (struct bmx_peer *) status.context;
            mxmap = peer->mxp_mxmap;
            debug(BMX_DB_CONN,
                  "%s returned for %s with %s",
                  __func__,
                  mxmap->mxm_peername,
            ...
            debug(BMX_DB_CONN,
                  "%s tx match= 0x%llx length= %lld",
                  __func__,
                  llu(tx->mxc_match),
                  lld(tx->mxc_nob));
            mx_isend(bmi_mx->bmx_ep,
                     &tx->mxc_seg,
                     tx->mxc_nseg,
                     peer->mxp_epa,
                     tx->mxc_match,
 ...
 */
static void bmx_handle_conn_req(void)
{
    uint32_t result = 0;
    ...
    do {
        ...
        if (result)
        {
            ...
            uint32_t sid = 0;
            uint32_t version = 0;
            uint64_t nic_id  = 0ULL;    /* unsigned long long */
            uint32_t ep_id = 0;
            ...
            debug(BMX_DB_CONN,
                  "%s returned %s match 0x%llx with %s",
                  __func__,
            ...
            if (version != BMX_VERSION)
            {
                ...
                debug(BMX_DB_WARN,
                      "version mismatch with peer %s (our version 0x%x, "
                      "peer's version 0x%x)",
                      (char *) rx->mxc_buffer,
                      BMX_VERSION, version);
                bmx_peer_decref(rx->mxc_peer);
            ...
            {
                debug(BMX_DB_WARN,
                      "received CONN_REQ on a client.");
                bmx_peer_decref(rx->mxc_peer);
                bmx_put_idle_ctx(rx);
                continue;
            }
            mx_decompose_endpoint_addr2(status.source,
                                        &nic_id,
                                        &ep_id,
                                        &sid);
            {
            ...
            if (peer == NULL)
            {
                ...
                ret = bmx_parse_peername(peername,
                                         &host,
                                         &board,
                                         &ep_id);
                if (ret != 0)
                {
                    debug(BMX_DB_CONN,
                          "parse_peername() failed on %s",
                          (char *) rx->mxc_buffer);
                    bmx_peer_decref(rx->mxc_peer);
                    bmx_put_idle_ctx(rx);
                    continue;
                }
                map = bmx_alloc_method_addr(peername,
                                            host,
                                            board,
                                            ep_id);
                if (map == NULL)
                {
                    debug((BMX_DB_CONN|BMX_DB_MEM),
                          "unable to alloc a method addr for %s",
                          peername);
                    bmx_peer_decref(rx->mxc_peer);
                    bmx_put_idle_ctx(rx);
                    continue;
            ...
            else if (sid != peer->mxp_sid)
            {
                /* reconnecting peer */
                /* cancel queued txs and rxs, pending rxs */
                debug((BMX_DB_CONN|BMX_DB_PEER),
                      "%s peer %s reconnecting",
                      __func__,
                      peer->mxp_mxmap->mxm_peername);
                if (peer->mxp_state == BMX_PEER_READY)
                {
            ...
            {
                debug((BMX_DB_CONN|BMX_DB_PEER),
                      "%s peer %s reconnecting with same sid",
                      __func__,
                      peer->mxp_mxmap->mxm_peername);
                mxmap = peer->mxp_mxmap;
            }
            gen_mutex_lock(&peer->mxp_lock);
            debug(BMX_DB_PEER,
                  "Setting peer %s to BMX_PEER_WAIT",
                  peer->mxp_mxmap->mxm_peername);
 ...
 *    send CONN_ACK
 */
static void bmx_handle_icon_ack(void)
{
    uint32_t result = 0;
    struct bmx_ctx *tx = NULL;
    struct bmx_peer *peer = NULL;
    ...
    do {
        ...
        if (result)
        {
            ...
            peer->mxp_epa = status.source;
            debug(BMX_DB_PEER,
                  "Setting peer %s to BMX_PEER_READY",
                  peer->mxp_mxmap->mxm_peername);
            peer->mxp_state = BMX_PEER_READY;
            ...
            if (tx == NULL)
            {
                ...
                {
                    debug((BMX_DB_CONN|BMX_DB_MEM),
                          "unable to alloc a ctx to send a CONN_ACK to %s",
                          peer->mxp_mxmap->mxm_peername);
                    bmx_peer_disconnect(peer, 1, BMI_ENOMEM);
                    /* drop ref taken before calling mx_iconnect */
                    bmx_peer_decref(peer);
            ...
            {
                debug(BMX_DB_PEER,
                      "calling bmi_method_addr_reg_callback on %s",
                      peer->mxp_mxmap->mxm_peername);
...
static void bmx_handle_conn_ack(void)
{
    uint32_t result = 0;
    struct bmx_ctx *tx = NULL;
    ...
    do {
        ...
        if (result)
        {
            ...
            debug(BMX_DB_CONN,
                  "%s returned tx match 0x%llx with %s",
                  __func__,
                  llu(tx->mxc_match),
                  mx_strstatus(status.code));
            bmx_peer_decref(tx->mxc_peer);
...
static void bmx_connection_handlers(void)
{
    static int count = 0;
...
static void bmx_complete_ctx(struct bmx_ctx *ctx,
                             bmi_op_id_t *outid,
                             bmi_error_code_t *err,
                             bmi_size_t *size,
                             void **user_ptr)
{
    ...
    *size = ctx->mxc_mxstat.xfer_length;
    ...
    if (ctx->mxc_type == BMX_REQ_TX)
    {
        PINT_EVENT_END(bmi_mx_send_event_id,
                       bmi_mx_pid,
                       NULL,
                       ctx->mxc_mop->event_id,
                       *outid,
                       *size);
    }
    else
    {
        PINT_EVENT_END(bmi_mx_recv_event_id,
                       bmi_mx_pid,
                       NULL,
                       ctx->mxc_mop->event_id,
                       *outid,
...
static int BMI_mx_test(bmi_op_id_t id,
                       int *outcount,
                       bmi_error_code_t *err,
                       bmi_size_t *size,
                       void **user_ptr,
...
static int BMI_mx_testcontext(int incount,
                              bmi_op_id_t *outids,
                              int *outcount,
                              ...
                              bmi_context_id context_id)
{
    ...
    for (i = completed; i < incount; i++)
    {
        ...
        {
            mx_test_any(bmi_mx->bmx_ep, match, mask, &status, &result);
            if (!result && wait == 0 && max_idle_time > 0)
            {
                wait = 1;
            }
        }
        ...
        if (result)
        {
            ...
            peer = ctx->mxc_peer;
            debug(BMX_DB_CTX,
                  "%s completing expected %s with match 0x%llx "
    ...
    for (i = completed; i < incount; i++)
    {
        ...
        {
            debug(BMX_DB_CTX,
                  "%s completing unexpected %s with "
            ...
            {
                debug(BMX_DB_CTX,
                      "%s unexpected send completed with error %s",
                      __func__,
                      mx_strstatus(status.code));
                bmx_peer_disconnect(peer, 0, BMI_ENETRESET);
            }
    ...
    {
        debug(BMX_DB_CTX,
              "%s found %d unexpected tx messages",
              __func__,
              completed - old);
    }
...
static int BMI_mx_testunexpected(int incount __unused,
                                 int *outcount,
                                 ...
                                 int max_idle_time __unused)
{
    ...
    {
        int ret = 0;
        ret = bmx_post_unexpected_recv(status.source,
                                       0,
                                       0,
                                       0,
                                       status.match_info,
        ...
        {
            debug(BMX_DB_CTX,
                  "%s mx_iprobe() found rx with match 0x%llx "
                  "length %d but could not receive it",
                  __func__,
    ...
    *outcount = 0;
    ...
    {
        debug(BMX_DB_CTX,
              "%s completing RX with match 0x%llx for %s",
              __func__,
              llu(rx->mxc_match),
              peer->mxp_mxmap->mxm_peername);
...
static int bmx_peer_connect(struct bmx_peer *peer)
{
    ...
    if (bmi_mx->bmx_ep == NULL)
    {
        ...
        if (ret != 0)
        {
            debug((BMX_DB_MX|BMX_DB_CONN),
                  "failed to open endpoint when trying to conenct to %s",
                  mxmap->mxm_peername);
            bmx_peer_decref(peer);
            return ret;
        }
        ...
        mx_decompose_endpoint_addr2(epa,
                                    &nic_id,
                                    &bmi_mx->bmx_ep_id,
                                    &bmi_mx->bmx_sid);
        ...
        {
            debug((BMX_DB_MX|BMX_DB_CONN),
                  "%s mx_nic_id_to_hostname() did not find hostname %s",
                  __func__,
                  host);
            return -1;
        }
...
static struct bmi_method_addr *BMI_mx_method_addr_lookup(const char *id)
{
    int ret = 0;
    int len = 0;
    char *host = NULL;
    ...
    {
        debug(BMX_DB_PEER,
              "%s method_parse_peername() failed on %s",
              __func__,
              id);
        return NULL;
    }
    ...
    if (map == NULL)
    {
        ...
        if (bmi_mx != NULL)
        {
            ...
            if (ret != 0)
            {
                debug((BMX_DB_MEM|BMX_DB_PEER),
                      "%s unable to alloc peer for %s",
                      __func__,
                      mxmap->mxm_peername);
                goto out;
            }
            ...
            if (ret != 0)
            {
                debug((BMX_DB_CONN|BMX_DB_MX|BMX_DB_PEER),
                      "%s peer_connect() failed with %d",
                      __func__,
                      ret);
            }
        }
...
static int BMI_mx_open_context(bmi_context_id context_id __unused)
{
    return 0;
}

static void BMI_mx_close_context(bmi_context_id context_id __unused)
{
    return;
}

/* NOTE There may be a race between this and BMI_mx_testcontext(). */
static int BMI_mx_cancel(bmi_op_id_t id,
                         bmi_context_id context_id)
{
    ...
    debug(BMX_DB_CTX,
          "%s %s op_id %llu mxc_state %d peer state %d",
          __func__,
          ctx->mxc_type == BMX_REQ_TX ? "TX" : "RX",
          llu(ctx->mxc_mop->op_id),
          ctx->mxc_state,
    ...
    switch (ctx->mxc_state)
    {
        ...
        case BMX_CTX_PENDING:
                ...
                if (ctx->mxc_type == BMX_REQ_TX)
                {
                    ...
                    {
                        debug(BMX_DB_CTX, 
                              "%s completed TX op_id %llu mxc_state "
                              "%d peer state %d status.code %s",
                              __func__,
                              llu(ctx->mxc_mop->op_id),
                              ctx->mxc_state,
                              peer->mxp_state,
                              mx_strstatus(ctx->mxc_mxstat.code));
                        bmx_deq_pending_ctx(ctx);
                        bmx_q_completed(ctx,
                                        BMX_CTX_CANCELED,
                                        ctx->mxc_mxstat,
...
static const char *BMI_mx_rev_lookup(struct bmi_method_addr *meth)
{
