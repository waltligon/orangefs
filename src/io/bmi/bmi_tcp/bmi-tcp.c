...
/* function prototypes */
int BMI_tcp_initialize(
    method_addr_p listen_addr,
    int method_id,
    int init_flags);
int BMI_tcp_finalize(
    void);
int BMI_tcp_set_info(
    int option,
    void *inout_parameter);
int BMI_tcp_get_info(
    int option,
    void *inout_parameter);
void *BMI_tcp_memalloc(
    bmi_size_t size,
...
int BMI_tcp_post_sendunexpected(
    bmi_op_id_t * id,
    method_addr_p dest,
    const void *buffer,
    bmi_size_t size,
...
int BMI_tcp_testsome(
    ...
    int max_idle_time_ms,
    bmi_context_id context_id);
int BMI_tcp_testunexpected(
    int incount,
    int *outcount,
    struct method_unexpected_info *info,
    int max_idle_time_ms);
int BMI_tcp_testcontext(
    int incount,
    ...
    int max_idle_time_ms,
    bmi_context_id context_id);
method_addr_p BMI_tcp_method_addr_lookup(
    const char *id_string);
const char *BMI_tcp_addr_rev_lookup_unexpected(
    method_addr_p map);
int BMI_tcp_post_send_list(
    bmi_op_id_t * id,
    method_addr_p dest,
...
int BMI_tcp_post_recv_list(
    ...
    void *user_ptr,
    bmi_context_id context_id);
int BMI_tcp_post_sendunexpected_list(
    bmi_op_id_t * id,
    method_addr_p dest,
    const void *const *buffer_list,
    const bmi_size_t * size_list,
    ...
    void *user_ptr,
    bmi_context_id context_id);
int BMI_tcp_open_context(
    bmi_context_id context_id);
void BMI_tcp_close_context(
    bmi_context_id context_id);
int BMI_tcp_cancel(
    bmi_op_id_t id,
    bmi_context_id context_id);
...
static int enqueue_operation(
    ...
    bmi_size_t expected_size,
    bmi_context_id context_id);
static int tcp_cleanse_addr(
    method_addr_p map,
    int error_code);
static int tcp_shutdown_addr(
    method_addr_p map);
static int tcp_do_work(
    int max_idle_time);
static int tcp_do_work_error(
    method_addr_p map);
static int tcp_do_work_recv(
    method_addr_p map,
    int *stall_flag);
static int tcp_do_work_send(
    method_addr_p map,
    int *stall_flag);
static int work_on_recv_op(
    method_op_p my_method_op,
    int *stall_flag);
static int work_on_send_op(
    method_op_p my_method_op,
    int *blocked_flag,
    int *stall_flag);
static int tcp_accept_init(
    int *socket,
    char **peer);
static method_op_p alloc_tcp_method_op(
    void);
static void dealloc_tcp_method_op(
    method_op_p old_op);
static int handle_new_connection(
    method_addr_p map);
static int BMI_tcp_post_send_generic(
    bmi_op_id_t * id,
    method_addr_p dest,
    const void *const *buffer_list,
    const bmi_size_t * size_list,
    ...
    void *user_ptr,
    bmi_context_id context_id);
static int tcp_post_recv_generic(
    bmi_op_id_t * id,
    method_addr_p src,
    void *const *buffer_list,
    const bmi_size_t * size_list,
...
static int tcp_enable_trusted(
    struct tcp_addr *tcp_addr_data);
#endif
#if defined(USE_TRUSTED) && defined(__PVFS2_SERVER__)
static int tcp_allow_trusted(
    struct sockaddr_in *peer_sockaddr);
#endif

static void bmi_set_sock_buffers(
    int socket);
    ...
    int init_flags)
{
    ...
    if (tcp_method_params.method_flags & BMI_INIT_SERVER)
    {
        ...
        tcp_socket_collection_p =
            BMI_socket_collection_init(tcp_addr_data->socket);
    }
    else
    {
    ...
    void *inout_parameter)
{
    ...
    case BMI_TCP_BUFFER_SEND_SIZE:
        ...
        bmi_set_sock_buffers(((struct tcp_addr *)
                              tcp_method_params.listen_addr->method_data)->
                             socket);
#endif
        break;
    ...
    case BMI_TRUSTED_CONNECTION:
        {
            ...
            else
            {
                ...
                struct server_configuration_s *svc_config =
                    (struct server_configuration_s *) inout_parameter;
                tcp_allowed_connection =
                    (struct tcp_allowed_connection_s *) calloc(1,
                                                               sizeof(struct
                                                                      tcp_allowed_connection_s));
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            inet_aton(tcp_string,
                                      &tcp_allowed_connection->netmask);
                            free(tcp_string);
                        }
                    }
    ...
    method_addr_p map)
{
    ...
    ret =
        getpeername(tcp_addr_data->socket, (struct sockaddr *) &(peer),
                    &peerlen);
    if (ret < 0)
    {
    ...
    int error_code)
{
    ...
    if (tcp_socket_collection_p)
    {
        ...
        BMI_socket_collection_testglobal(tcp_socket_collection_p,
                                         0, &tmp_outcount, &tmp_addr,
                                         &tmp_status, 0, &interface_mutex);
    }
 ...
 */
method_addr_p alloc_tcp_method_addr(
    void)
{
 ...
 */
static method_op_p find_recv_inflight(
    method_addr_p map)
{
    ...
    bmi_context_id context_id)
{
    ...
    *id = new_method_op->op_id;
    ...
    if (tcp_addr_data->addr_error)
    {
        ...
        new_method_op->error_code = tcp_addr_data->addr_error;
        op_list_add(op_list_array[new_method_op->context_id], new_method_op);
        return (tcp_addr_data->addr_error);
    }
#endif
        ...
        else
        {
            ...
            if ((ret == 0) &&
                ...
                (active_method_op->amt_complete <
                 active_method_op->actual_size))
            {
                gossip_debug(GOSSIP_BMI_DEBUG_TCP, "Warning: bmi_tcp unable "
    ...
    struct sockaddr_in *peer_sockaddr)
{
    ...
    if (gtcp_allowed_connection->network_enforce == 1)
    {
        ...
        /* check the masks */
        if ((peer_sockaddr->sin_addr.s_addr & gtcp_allowed_connection->netmask.
             s_addr) != gtcp_allowed_connection->network.s_addr)
        {
            what_failed = 0;
        }
    }
    ...
    bmi_context_id context_id)
{
    ...
    if (query_op)
    {
        ...
        {
            gossip_err
                ("Error: enqueue_operation() or tcp_do_work() returned: %d\n",
                 ret);
        }
        return (ret);
    }
    ...
    ret = payload_progress(tcp_addr_data->socket,
                           ...
                           &cur_index_complete, BMI_SEND, my_header.enc_hdr,
                           &env_amt_complete);
    if (ret < 0)
    {
        PVFS_perror_gossip("Error: payload_progress", ret);
    ...
    bmi_size_t * env_amt_complete)
{
    ...
    if (send_recv == BMI_SEND && *env_amt_complete < TCP_ENC_HDR_SIZE)
    {
        ...
        stat_io_vector[vector_index].iov_len =
            TCP_ENC_HDR_SIZE - *env_amt_complete;
        count++;
        vector_index++;
        header_flag = 1;
    }
    ...
    {
        stat_io_vector[vector_index].iov_len =
            final_size - *current_index_complete;
    }
    else
    {
        stat_io_vector[vector_index].iov_len =
