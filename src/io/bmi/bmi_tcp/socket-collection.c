    ...
    int new_server_socket)
{
    ...
    tmp_scp->pollfd_array = (struct
                             pollfd *) malloc(POLLFD_ARRAY_START *
    ...
    qlist_for_each_safe(iterator, scratch, &scp->add_queue)
    {
        ...
        if (tcp_addr_data->sc_index > -1)
        {
            ...
            gossip_err("HELLO: updating addr: %p, index: %d, ref: %d.\n",
                       scp->addr_array[tcp_addr_data->sc_index],
                       tcp_addr_data->sc_index, tcp_addr_data->write_ref_count);
#endif
        ...
        else
        {
            ...
            if (scp->array_count == scp->array_max)
            {
                ...
                tmp_pollfd_array = (struct pollfd *) malloc((scp->array_max +
                                                             POLLFD_ARRAY_INC) *
                                                            sizeof(struct
                                                                   pollfd));
                /* TODO: handle this */
                assert(tmp_pollfd_array);
                tmp_addr_array = (method_addr_p *) malloc((scp->array_max +
                                                           POLLFD_ARRAY_INC) *
                                                          sizeof
                                                          (method_addr_p));
                /* TODO: handle this */
