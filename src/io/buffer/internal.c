...
static void NCAC_list_add_tail_lock(
    struct list_head *new,
    struct list_head *head,
    NCAC_lock * lock);
static void NCAC_list_del_lock(
    struct list_head *entry,
    NCAC_lock * lock);
static void NCAC_read_request_from_list_lock(
    struct list_head *head,
    NCAC_lock * lock,
    NCAC_req_t ** ncac_req_ptr);

/* This file contains NCAC internal functions. */

static inline struct inode *get_inode(
    PVFS_fs_id,
    PVFS_handle,
    PVFS_context_id);
static inline int NCAC_rwjob_prepare_single(
    NCAC_req_t * ncac_req);
static inline int NCAC_rwjob_prepare_list(
    NCAC_req_t * ncac_req);
...
static inline struct NCAC_req *get_internal_req_lock(
    PVFS_fs_id fsid,
    PVFS_handle hndl)
{
    ...
    if (list_empty(&NCAC_dev.free_req_list))
        return NULL;
    ...
    NCAC_optype optype)
{
    ...
    if (ncac_req == NULL)
    {   /* run out of ncac request resources */
    ...
    /* inode */
    ncac_req->mapping =
        get_inode(desc->coll_id, desc->handle, desc->context_id);
    /* inode aiovec */
    ...
    {   /* copy data into the user's buffer */
        if (optype == NCAC_GEN_READ)
            ...
            ncac_req->optype = NCAC_BUF_WRITE;

        ncac_req->usrbuf = desc->buffer;
        ncac_req->usrlen = desc->len;
    }
    else
    {   /* use cache buffers for communication */
        if (optype == NCAC_GEN_READ)
            ncac_req->optype = NCAC_READ;
        else
            ncac_req->optype = NCAC_WRITE;
    ...
    NCAC_reply_t * reply)
{
    ...
    ret = NCAC_do_a_job(ncac_req, &(NCAC_dev.prepare_list),
                        ...
                        &(NCAC_dev.comp_list), &NCAC_dev.req_list_lock);
    ...
    NCAC_req_t * ncac_req)
{
    ...
    if (ncac_req->reserved_cbufcnt < comcnt)
    {
        ...
        ncac_req->cbufhash = (struct extent **) &ncac_req->cbufsize[comcnt];
    ...
    struct NCAC_req **ncac_req)
{
    ...
    ret =
        NCAC_do_a_job(req, &(NCAC_dev.prepare_list), &(NCAC_dev.bufcomp_list),
                      &(NCAC_dev.comp_list), &NCAC_dev.req_list_lock);
    ...
    int id)
{
    ...
    default:   /* error. leaking here. */
        fprintf(stderr,
                "NCAC_done_request: wrong status of internal request\n");
    ...
    {   /* we want reuse buffer information arrays */
        ncac_req->cbufcnt = 0;
        ncac_req->mapping = 0;
    ...
    NCAC_list_add_tail_lock(&ncac_req->list, &NCAC_dev.free_req_list,
                            &NCAC_dev.req_list_lock);
...
static inline struct inode *search_inode_list(
    PVFS_handle handle)
{
    int inode_index;
    struct inode *cur;
    ...
    PVFS_context_id context_id)
{
    ...
    if (NULL == inode)
    {
        ...
        init_single_radix_tree(&inode->page_tree, NCAC_dev.get_value,
                               NCAC_dev.max_b);
...
static inline void extent_dump(
    struct extent *extent)
{
    ...
    fprintf(stderr, "writes:%d\t reads:%d\t	ioreq:%lld\t\n", extent->writes,
            extent->reads, lld(extent->ioreq));

}

static inline void list_dump(
    struct list_head *head)
{
    struct extent *page;
    ...
    if (!list_empty(head))
    {
        ...
        while (tmp != head)
        {
            page = list_entry(tmp, struct extent,
                              lru);
...
static inline void req_list_dump(
    struct list_head *head)
{
    NCAC_req_t *req;
    struct list_head *tmp;
...
static inline void list_dump_list(
    struct list_head *head)
{
    struct extent *page;
    struct list_head *tmp;
