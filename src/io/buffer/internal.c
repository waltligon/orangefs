...
static void NCAC_list_add_tail_lock(struct qlist_head *new,
        struct qlist_head *head, NCAC_lock *lock);
static void NCAC_list_del_lock(struct qlist_head *entry,
        NCAC_lock *lock);
static void NCAC_read_request_from_list_lock(struct qlist_head *head,
        NCAC_lock *lock, NCAC_req_t ** ncac_req_ptr);
...
static inline struct NCAC_req * get_internal_req_lock(PVFS_fs_id fsid,
        PVFS_handle hndl)
{

    NCAC_req_t *req=NULL;
    struct qlist_head *new;

    list_lock(&NCAC_dev.req_list_lock); 

    if ( qlist_empty(&NCAC_dev.free_req_list) ) return NULL;
    ...
    req = qlist_entry(new->prev, NCAC_req_t, list);
...
static void NCAC_list_del_lock(struct qlist_head *entry, NCAC_lock *lock)
{
    list_lock(lock);
        ...
        NCAC_lock *lock, NCAC_req_t ** ncac_req_ptr)
{
    ...
    for (pos = head->next; pos != head; pos = pos->next) {
            req = qlist_entry( pos, NCAC_req_t, list);
        if ( !req->read_out ) {
            req->read_out = 1;
            break;
        }
...
int NCAC_do_jobs(struct qlist_head *prep_list, struct qlist_head *bufcomp_list,
...
int NCAC_do_a_job(NCAC_req_t *ncac_req, struct qlist_head *prep_list, 
...
int NCAC_check_request( int id, struct NCAC_req **ncac_req )
{
    ...
    ret = NCAC_do_a_job(req, &(NCAC_dev.prepare_list),
        &(NCAC_dev.bufcomp_list), &(NCAC_dev.comp_list),
        &NCAC_dev.req_list_lock);
                ...
                PVFS_handle handle, PVFS_context_id context_id)
{
    ...
    if ( NULL == inode ){
        ...
        INIT_QLIST_HEAD(&(inode->clean_pages));
        INIT_QLIST_HEAD(&(inode->dirty_pages));
...
static inline void list_dump(struct qlist_head *head)
{
    struct extent *page;
    ...
    if (!qlist_empty(head)) {
        tmp = head->next;
        while (tmp!=head) {
            page = qlist_entry(tmp, struct extent, lru);
            fprintf(stderr, "extent: %p\t", (void *)page);
            extent_dump(page);
            tmp = tmp->next; 
        } 
    }
...
static inline void req_list_dump(struct qlist_head *head)
{
    NCAC_req_t *req;
    struct qlist_head *tmp;
    ...
    if (!qlist_empty(head)) {
        tmp = head->next;
        while (tmp!=head) {
            req = qlist_entry(tmp, NCAC_req_t, list);
            fprintf(stderr, "req: %p\t", (void *)req); 
            //req_dump(req);
            tmp = tmp->next; 
        } 
    }
...
static inline void list_dump_list(struct qlist_head *head)
{
    struct extent *page;
    struct qlist_head *tmp;
    ...
    if (!qlist_empty(head)) {
        tmp = head->next;
        while (tmp!=head) {
            page = qlist_entry(tmp, struct extent, list);
            fprintf(stderr, "extent: %p\t", (void *)page); 
            extent_dump(page);
