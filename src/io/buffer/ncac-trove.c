...
static inline void offset_shorten(
    int s_cnt,
    ...
    int *ioreq)
{
    ...
    offset_shorten(s_cnt,
                   stream_offset_array,
                   stream_size_array,
                   m_cnt, mem_offset_array, mem_size_array, &s_cnt, &m_cnt);
    ...
    ret = trove_bstream_write_list(coll_id, handle, mem_offset_array, mem_size_array, m_cnt, stream_offset_array, stream_size_array, s_cnt, &output_size, 0,    /* flags */
                                   NULL,        /* vtag */
                                   user_ptr_array, context, &op_id);
    ...
    int *ioreq)
{
    ...
    DPRINT("do_read_for_rmw; pos=%lld, buf=%p, size=%lld\n", pos, buf,
           inout_size);
    ret =
        trove_bstream_read_at(coll_id, handle, buf, &inout_size, 0, 0, NULL,
                              NULL, context, &op_id);
    ...
    struct extent *extent)
{
    ...
    ret =
        trove_dspace_test(coll_id, op_id, context_id, &count, NULL, NULL,
    ...
    TROVE_op_id * ioreq)
{
    ...
    ret = trove_bstream_read_at(coll_id, handle, buf, &size, foffset, 0,        /* flags */
                                NULL,   /* vtag */
                                user_ptr_array, context, ioreq);
