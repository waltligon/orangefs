    ...
    void *item)
{
    ...
    if ((item2 = p->a[j]))
    {
        ...
        {
            fprintf(stderr,
                    "rst_insert: error, key2=key=%ld (item=%p, exist=%p)\n",
                    key, item, item2);
    ...
    unsigned long index)
{
    ...
    if (!p->a[j] || t->get_value(p->a[j]) != index)
        return NULL;
 ...
 * NULL if no item was found.
 */
static void *rst_find_min(
    rst_t * t) __attribute__ ((unused));
static void *rst_find_min(
    rst_t * t)
{
    unsigned int j;
    ...
    unsigned long key)
{
     ...
     * item was found.
     */
    if (!p->a[j] || t->get_value(p->a[j]) != key)
        return NULL;
    return_item = p->a[j];
    ...
    {
        if (tos == 0)
        {       /* Special case: deleteing a child of the root node. */
            p->a[j] = NULL;
     ...
     */
    if (p->child_links & (!j + 1))
    {   /* p->a[!j] points to a node. */
        p->a[j] = NULL;
    }
    else
    {   /* p->a[!j] points to an item. */
