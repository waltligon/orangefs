...
int NCAC_extent_done_access(NCAC_req_t *ncac_req)
{
    int  i;
    struct inode *inode;
    struct qlist_head *dirty_list = NULL;
    ...
    if (ncac_req->optype == NCAC_WRITE)
    {
        ...
        for (i = 0; i < ncac_req->cbufcnt; i++)
        {
            if (ncac_req->cbufflag[i] && ncac_req->cbufhash[i])
            {
                ...
                /* add dirty pages */
                qlist_add_tail (&ncac_req->cbufhash[i]->list, dirty_list);
                nr_dirty++;
...
int data_sync_inode(struct inode *inode)
{
    ...
    tail = list->next; 
    while ( tail != list ){
        extent = qlist_entry(tail, struct extent, list);
