...
static int NCAC_move_inactive_to_active(
 ...
 */
int NCAC_extent_first_read_access(
    NCAC_req_t * ncac_req,
    struct extent *page)
{
 ...
 * put it into cache.
 */
int NCAC_extent_first_write_access(
    NCAC_req_t * ncac_req,
    struct extent *page)
{
    ClearPageBlank(page);
...
int NCAC_extent_read_comm_done(
    struct extent *page)
{
    ClearPageRcomm(page);
    return 0;
...
/* read commnication is done */
int NCAC_extent_write_comm_done(
    struct extent *page)
{
    ClearPageWcomm(page);
    ClearPageClean(page);
...
int NCAC_extent_read_access_recheck(
    NCAC_req_t * req,
    struct extent *page,
    unsigned int offset,
    unsigned int size)
...
int NCAC_extent_write_access_recheck(
    NCAC_req_t * ncac_req,
    struct extent *page,
    unsigned int offset,
    unsigned int size)
...
#if 0
static void balance_dirty_extents(
    struct cache_stack *cache_stack)
{
    ...
    struct inode *inode)
{
    ...
    offset =
        (PVFS_offset *) malloc(nr_dirty *
                               (sizeof(PVFS_offset) + sizeof(char *) +
                                2 * sizeof(PVFS_size)));
    ...
    while (tail != list)
    {
        extent = list_entry(tail, struct extent,
                            list);
