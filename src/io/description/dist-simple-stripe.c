...
static PVFS_size physical_to_logical_size(
    void *params,
    PINT_request_file_data * fd,
    PVFS_size physical_size)
{
    ...
    return (strips_div * dparam->strip_size * server_ct) +
        (dparam->strip_size * server_nr) + strips_mod;
}
    ...
    PVFS_offset logical_offset)
{
    ...
    else if (diff >= dparam->strip_size)
