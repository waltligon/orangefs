 ...
 *  if none exists
 */
static PINT_dist_param_offset *PINT_get_param_offset(
...
int PINT_dist_default_get_num_dfiles(
    void *params,
...
int PINT_dist_default_set_param(
    const char *dist_name,
    void *params,
    const char *param_name,
    void *value)
...
int PINT_dist_register_param_offset(
    const char *dist_name,
    const char *param_name,
    size_t offset,
    size_t field_size)
        ...
        if (0 != buf)
        {
            if (PINT_dist_param_table_size)
            {
                memcpy(buf, PINT_dist_param_table,
                       PINT_dist_param_table_size *
                       sizeof(PINT_dist_param_offset));
            }
