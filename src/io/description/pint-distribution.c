 ...
 */
int PINT_register_distribution(
    PINT_dist * d_p)
{
 ...
 */
int PINT_unregister_distribution(
    char *dist_name)
{
    int d;
    if (!dist_name)
    ...
    if (PINT_dist_lookup(&old_dist) == 0)
    {
        ...
        if (new_dist)
        {
            *new_dist = old_dist;
            ...
            memcpy(new_dist->dist_name, old_dist.dist_name, old_dist.name_size);
...
PINT_dist *PINT_dist_copy(
    const PINT_dist * dist)
{
    int dist_size;
    PINT_dist *new_dist;
    ...
    if (new_dist)
    {
        ...
        new_dist->dist_name = (char *) new_dist + roundup8(sizeof(*new_dist));
        new_dist->params
            = (void *) (new_dist->dist_name + roundup8(new_dist->name_size));
