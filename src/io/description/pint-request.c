...
static PVFS_offset PINT_request_disp(
    PINT_Request * request);
...
/* in the request */
int PINT_process_request(
    PINT_Request_state * req,
    ...
    int mode)
{
    ...
    if (!PINT_IS_MEMREQ(mode))
    {
        ...
        gossip_debug(GOSSIP_REQUEST_DEBUG, "\ttiling %lld copies\n",
                     lld(count + 1));
    }
    ...
    while (!retval)
    {
        if (req->cur[req->lvl].rq)
        {
            ...
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         "\tDo seq of %lld ne %d st %lld nb %d "
                         "ub %lld lb %lld as %lld co %llu\n",
                         lld(req->cur[req->lvl].rq->offset),
                         req->cur[req->lvl].rq->num_ereqs,
                         lld(req->cur[req->lvl].rq->stride),
                         req->cur[req->lvl].rq->num_blocks,
                         lld(req->cur[req->lvl].rq->ub),
                         lld(req->cur[req->lvl].rq->lb),
                         lld(req->cur[req->lvl].rq->aggregate_size),
                         lld(req->cur[req->lvl].chunk_offset));
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         "\t\tlvl %d el %lld blk %d by %lld\n", req->lvl,
                         lld(req->cur[req->lvl].el), req->cur[req->lvl].blk,
                         lld(req->bytes));
                 ...
                 req->cur[req->lvl].rq->ereq->num_contig_chunks == 1)
        {
            ...
            contig_offset = req->cur[req->lvl].chunk_offset +
                ...
                req->cur[req->lvl].rq->offset + (req->cur[req->lvl].rq->stride *
                                                 req->cur[req->lvl].blk) +
                req->bytes + PINT_request_disp(req->cur[req->lvl].rq);
            contig_size =
                (req->cur[req->lvl].rq->ereq->aggregate_size *
        ...
        else
        {
            ...
            {
                gossip_lerr
                    ("PINT_process_request exceeded request depth - possibly corrupted request or request state\n");
                return -PVFS_EINVAL;
            }
            ...
            req->cur[req->lvl + 1].chunk_offset =
        ...
        gossip_debug(GOSSIP_REQUEST_DEBUG,
                     ...
                     lld(contig_offset), lld(contig_size), lvl_flag);
      ...
      return_from_level:
        ...
        if (req->cur[req->lvl].blk >= req->cur[req->lvl].rq->num_blocks)
        {
            ...
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         "\tgoing to next item in sequence chain\n");
    ...
    gossip_debug(GOSSIP_REQUEST_DEBUG,
                 "\tdone sg %d sm %d by %lld bm %lld ta %lld to %lld fo %lld eof %d\n",
                 result->segs, result->segmax, lld(result->bytes),
                 lld(result->bytemax), lld(req->target_offset),
                 lld(req->type_offset), lld(req->final_offset), req->eof_flag);
...
struct PINT_Request_state *PINT_new_request_state(
    PINT_Request * request)
{
    ...
    if (!
        (req =
         (struct PINT_Request_state *)
         malloc(sizeof(struct PINT_Request_state))))
    {
    ...
    int mode)
{
    ...
    gossip_debug(GOSSIP_REQUEST_DEBUG,
                 ...
                 "fsz %lld exfl %d\n",
                 lld(offset), lld(size), result->segs, result->segmax,
                 lld(result->bytes),
                 lld(result->bytemax), lld(rfdata->fsize), rfdata->extend_flag);
    orig_offset = offset;
    orig_size = size;
    *eof_flag = 0;
        ...
        !rfdata->dist->params)
    {
        if (!rfdata)
            ...
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         "rfdata->dist->methods is NULL\n");
        else if (!rfdata->dist->params)
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         "rfdata->dist->params is NULL\n");
    ...
    while ((diff = loff - offset) < size)
    {
        ...
        fraglen = (*rfdata->dist->methods->contiguous_length)
            (rfdata->dist->params, rfdata, poff);
        ...
        if (poff + sz > rfdata->fsize)
        {
            ...
            else
            {
                ...
                gossip_debug(GOSSIP_REQUEST_DEBUG,
                             ...
                             lld(poff), lld(sz), lld(rfdata->fsize));
                *eof_flag = 1;
        ...
        if (PINT_IS_CLIENT(mode))
        {
            ...
            gossip_debug(GOSSIP_REQUEST_DEBUG,
                         ...
                         lld(result->offset_array[result->segs]), lld(diff),
                         lld(poff));
        }
        ...
        loff = (*rfdata->dist->methods->next_mapped_offset)
            (rfdata->dist->params, rfdata, offset);
        assert(-1 != loff);
    ...
    gossip_debug(GOSSIP_REQUEST_DEBUG,
                 ...
                 lld(offset), lld(size), result->segs, result->segmax,
                 lld(result->bytes), lld(result->bytemax));
    ...
    else
    {
        gossip_debug(GOSSIP_REQUEST_DEBUG, "\t\t\treturn value %lld%s\n",
                     lld(offset - orig_offset), *eof_flag ? " (EOF)" : "");
...
PINT_Request *PINT_do_request_commit(
    PINT_Request * region,
    PINT_Request * node,
    int32_t * index,
    int32_t depth)
