		...
		PVFS_size oldext, PVFS_Request *newreq)
{
	...
	if (oldreq->aggregate_size == oldext && oldreq->num_contig_chunks == 1)
		...
		(*newreq)->num_contig_chunks = oldreq->num_contig_chunks * bsize * count;
#if 0
	if (oldreq->aggregate_size != oldext)
		...
		(*newreq)->num_contig_chunks = count;
	else if (oldreq->aggregate_size == 0)
		(*newreq)->num_contig_chunks = 0;
	else
		(*newreq)->num_contig_chunks = 1;
#endif
...
int PVFS_Request_indexed_block(int32_t count, int32_t blocklength,
		      PVFS_size *displacements, PVFS_Request oldreq, PVFS_Request *newreq)
{
	int i;
