    ...
    int flags)
{
    ...
    debug_mask = PVFS_kmod_eventlog_to_mask(debug_string);
    mask_info.mask_value = debug_mask.mask1;
    ...
    if (!upstream_kmod)
    {
        ...
        mask_info.mask_value = gossip_debug_mask.mask1; /* loses info here */
        ret = ioctl(pdev_fd, PVFS_DEV_DEBUG, &mask_info);
        if (ret < 0)
        {
    ...
    {
        mask2_info.mask1_value = gossip_debug_mask.mask1;
        ...
        for (i = 0; i < num_keyword_mask_map; i++)
        {
            bytes = snprintf(client_debug_array_string + offset,
                             ...
                             (unsigned long long)s_keyword_mask_map[i].mask.mask1,
                             (unsigned long long)s_keyword_mask_map[i].mask.mask2);
