	...
	PVFS_bitfield flags)
{
	...
	active_flowproto_table = (struct flowproto_ops**)malloc(
	...
	flow_descriptor* flow_d)
{
	...
	flow_d->request_state =
		PINT_New_request_state(flow_d->request);
...
static int do_one_work_cycle(int* num_completed)
{
	...
	*num_completed = 0;
	...
	{
		ret = active_flowproto_table[tmp_flow->flowproto_id]->flowproto_service(
			tmp_flow);
		if(ret < 0)
		{
			/* no good way to clean this up */
	...
	dest_endpoint_id)
{
	...
	for(i=0; i<active_flowproto_count; i++)
	{
		ret = active_flowproto_table[i]->flowproto_getinfo(NULL,
			FLOWPROTO_SUPPORT_QUERY, &type_query);
		if(ret >= 0)
		{
			/* found a match; add it to the cache and return */
			flow_ref_add(flow_mapping, src_endpoint_id, dest_endpoint_id,
				ret);
			return(ret);
		}
	}
