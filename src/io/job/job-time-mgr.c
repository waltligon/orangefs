    ...
    void)
{
    ...
    {
        tmp_bucket = qlist_entry(iterator, struct time_bucket,
                                 bucket_link);
        assert(tmp_bucket);
        ...
        {
            jd = qlist_entry(iterator2, struct job_desc,
                             job_time_link);
    ...
    int timeout_sec)
{
    ...
    {
        tmp_bucket = qlist_entry(tmp_link, struct time_bucket,
                                 bucket_link);
        assert(tmp_bucket);
    ...
    void)
{
    ...
    qlist_for_each_safe(iterator, scratch, &bucket_queue)
    {
        ...
        qlist_for_each_safe(iterator2, scratch2, &tmp_bucket->jd_queue)
        {
            ...
            case JOB_BMI:
                gossip_err
                    ("Job time out: cancelling bmi operation, job_id: %llu.\n",
                     llu(jd->job_id));
            ...
            case JOB_FLOW:
                ...
                else
                {
                    ...
                    gossip_err
                        ("Job time out: cancelling flow operation, job_id: %llu.\n",
                         llu(jd->job_id));
            ...
            case JOB_TROVE:
                gossip_err
                    ("Job time out: cancelling trove operation, job_id: %llu.\n",
                     llu(jd->job_id));
                ret =
                    job_trove_dspace_cancel(jd->u.trove.fsid, jd->job_id,
                                            jd->context_id);
                break;
            default:
                ret = 0;
