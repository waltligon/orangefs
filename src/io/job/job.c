...
static int do_one_work_cycle_bmi(int* num_completed, int wait_flag);
static int do_one_work_cycle_bmi_unexp(int* num_completed, int
	wait_flag);
	...
	int flags)
{
...
#ifdef __PVFS2_JOB_THREADED__
	/* startup threads */
	ret = pthread_create(&bmi_thread_id, NULL, bmi_thread_function,
		NULL);
	if(ret != 0)
	{
		teardown_queues();
	...
	ret = pthread_create(&flow_thread_id, NULL, flow_thread_function,
		NULL);
	if(ret != 0)
	{
	...
	ret = pthread_create(&trove_thread_id, NULL, trove_thread_function,
		NULL);
	if(ret != 0)
	{
		pthread_cancel(bmi_thread_id);
	...
	job_id_t* id)
{
	...
	else
	{
		ret = BMI_post_sendunexpected_list(&(jd->u.bmi.id), addr, 
			buffer_list, size_list, list_count, total_size, buffer_flag, 
			tag, jd);
	}
	...
	job_id_t* id)
{
	...
	ret = BMI_post_recv_list(&(jd->u.bmi.id), addr, buffer_list, 
		size_list, list_count, total_expected_size, &(jd->u.bmi.actual_size), 
		buffer_flag, tag, jd);
 ...
 * failure
 */
int job_flow(flow_descriptor* flow_d, void* user_ptr, job_status_s*
	out_status_p, job_id_t* id)
{
	struct job_desc* jd = NULL;
	int ret = -1;
	...
	job_id_t* id)
{
	...
	ret = trove_collection_create(collname, new_coll_id, jd,
		&(jd->u.trove.id)); 
	...
	job_status_s* out_status_p)
{
	...
	/* should be the same whether we have a thread or not; just do a
	 * quick search of completion queue - no work
	 */
 ...
 * returns 0 on success, -errno on failure
 */
static int do_one_work_cycle_bmi_unexp(int* num_completed, int wait_flag)
{
	int incount, outcount;
...
static int do_one_work_cycle_flow(int* num_completed)
{
	...
	for(i=0; i<outcount; i++)
	{
		...
		tmp_desc = (struct
			job_desc*)stat_flow_array[stat_flow_index_array[i]]->user_ptr;
		gen_mutex_lock(&flow_mutex);
			job_desc_q_remove(tmp_desc);
		gen_mutex_unlock(&flow_mutex);
...
static int do_one_work_cycle_trove(int* num_completed)
{
	...
	ret = trove_dspace_testsome(HACK_global_fsid, stat_trove_id_array,
		&count, stat_trove_index_array, stat_trove_vtag_array,
		stat_trove_user_ptr_array, stat_trove_ds_state_array);
	...
	job_status_s* status)
{
	...
	switch(jd->type)
	{
		...
		case JOB_FLOW:
			...
			status->actual_size =
				jd->u.flow.flow_d->total_transfered;
			break;
		case JOB_REQ_SCHED:
