...
static gen_mutex_t bmi_thread_running_mutex = GEN_MUTEX_INITIALIZER;
...
static void *bmi_thread_function(void *ptr)
{
    ...
    {/*start block*/
        ...
        if (bmi_unexp_count)
        {
            ...
            ret = BMI_testunexpected(incount,
                                     &outcount,
                                     stat_bmi_unexp_array,
                                     0);
            if (ret < 0)
            {
...
static void *dev_thread_function(void *ptr)
{
...
#endif
    {
        ...
        ret = PINT_dev_test_unexpected(incount,
                                       &outcount,
                                       stat_dev_unexp_array,
                                       timeout);
...
#endif
        if((stat_bmi_id_array[i] == id) &&
           (stat_bmi_user_ptr_array[i] == user_ptr))
        {
#if 0
...
#endif
        if((stat_trove_id_array[i] == id) &&
           (stat_trove_user_ptr_array[i] == user_ptr))
        {
#if 0
