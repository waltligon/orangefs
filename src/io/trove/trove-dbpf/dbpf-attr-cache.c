...
static int s_max_num_cache_elems = DBPF_ATTR_CACHE_DEFAULT_MAX_NUM_CACHE_ELEMS;
...
int dbpf_attr_cache_set_keywords(
    char *keywords)
{
    assert(keywords);
    ...
    s_cacheable_keyword_array_size =
        PINT_split_string_list(&s_cacheable_keyword_array, keywords);
    return (s_cacheable_keyword_array ? 0 : -1);
}


int dbpf_attr_cache_set_size(
    int cache_size)
{
...
int dbpf_attr_cache_set_max_num_elems(
    int max_num_elems)
{
...
/*
  ...
  are specified
*/
int dbpf_attr_cache_do_initialize(
    void)
{
    int ret = -1;
    gossip_debug(GOSSIP_DBPF_ATTRCACHE_DEBUG, "There are %d cacheable "
                 "keywords registered\n", s_cacheable_keyword_array_size);
    ret = dbpf_attr_cache_initialize(s_cache_size, s_max_num_cache_elems,
                                     s_cacheable_keyword_array,
                                     s_cacheable_keyword_array_size);
    ...
    int num_cacheable_keywords)
{
    ...
    if (s_key_to_attr_table == NULL)
    {
        if (cacheable_keywords)
        {
            ...
            {
                gossip_err("****************************************\n");
                gossip_err("Warning: There are too many specified cacheable "
                ...
                gossip_err("No specified Attribute Keywords will be cached\n");
                gossip_err("****************************************\n");
                goto return_error;
            }
        ...
        s_key_to_attr_table =
            qhash_init(hash_key_compare, hash_key, table_size);
    ...
    void)
{
    ...
    if (DBPF_ATTR_CACHE_INITIALIZED())
    {
        for (i = 0; i < s_key_to_attr_table->table_size; i++)
        {
            ...
            {
                hash_link =
                    qhash_search_and_remove_at_index(s_key_to_attr_table, i);
                if (hash_link)
                {
                    cache_elem =
                        qhash_entry(hash_link, dbpf_attr_cache_elem_t,
                                    hash_link);
                    free(cache_elem);
    ...
    TROVE_object_ref key)
{
    ...
    if (DBPF_ATTR_CACHE_INITIALIZED())
    {
        ...
        {
            cache_elem =
                qhash_entry(hash_link, dbpf_attr_cache_elem_t, hash_link);
            assert(cache_elem);
            gossip_debug(GOSSIP_DBPF_ATTRCACHE_DEBUG,
    ...
    TROVE_ds_attributes * src_ds_attr)
{
    ...
    if (cache_elem && src_ds_attr)
    {
        if (cache_elem && src_ds_attr)
        {
            memcpy(&cache_elem->attr, src_ds_attr, sizeof(TROVE_ds_attributes));
    ...
    char *key)
{
        ...
        (cache_elem && key && cache_elem->num_keyval_pairs))
    {
        for (i = 0; i < cache_elem->num_keyval_pairs; i++)
        {
            ...
            {
                gossip_debug(GOSSIP_DBPF_ATTRCACHE_DEBUG, "Returning data %p "
    ...
    if (DBPF_ATTR_CACHE_INITIALIZED())
    {
        if ((s_current_num_cache_elems + 1) > s_max_num_cache_elems)
        {
          ...
          hashtable_linear_scan:
            ...
            gossip_debug(GOSSIP_DBPF_ATTRCACHE_DEBUG, "*** Cache is full -- "
        ...
        if (cache_elem)
        {
            ...
            if (s_cacheable_keyword_array)
            {
                ...
                cache_elem->num_keyval_pairs = s_cacheable_keyword_array_size;
            }
            ...
            if (!already_exists)
            {
                qhash_add(s_key_to_attr_table, &(key),
    ...
    TROVE_object_ref key)
{
    ...
    if (DBPF_ATTR_CACHE_INITIALIZED())
    {
        ...
        if (hash_link)
        {
            ...
            gossip_debug(GOSSIP_DBPF_ATTRCACHE_DEBUG, "dbpf_attr_cache_remove: "
                         "removing %llu\n", llu(key.handle));
