...
/* dbpf-collection.c
 *
 * This file implements two functions designed to maintain a list of
...
void dbpf_collection_register(struct dbpf_collection *coll_p)
{
	...
	while (ptr->next_p != NULL) ptr = ptr->next_p;
	ptr->next_p = coll_p;
    }
...
struct dbpf_collection *dbpf_collection_find_registered(TROVE_coll_id coll_id)
{
    ...
    while (ptr != NULL && ptr->coll_id != coll_id) ptr = ptr->next_p;
...
{
    int ret;
    struct dbpf_collection *ptr = root_coll_p, *free_ptr;
