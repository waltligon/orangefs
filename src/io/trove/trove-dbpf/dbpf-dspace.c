...
static int dbpf_dspace_iterate_handles_op_svc(
    struct dbpf_op *op_p);
static int dbpf_dspace_create_op_svc(
    struct dbpf_op *op_p);
static int dbpf_dspace_remove_op_svc(
    struct dbpf_op *op_p);
static int dbpf_dspace_verify_op_svc(
    struct dbpf_op *op_p);
static int dbpf_dspace_setattr_op_svc(
    struct dbpf_op *op_p);
static int dbpf_dspace_getattr_op_svc(
    struct dbpf_op *op_p);

static int dbpf_dspace_create(
    ...
    TROVE_op_id * out_op_id_p)
{
    ...
    ret = dbpf_op_init_queued_or_immediate(&op,
                                           ...
                                           (handle_p ? *handle_p :
                                            TROVE_HANDLE_NULL),
                                           dbpf_dspace_create_op_svc, flags,
    ...
    op_p->u.d_create.extent_array.extent_count = extent_array->extent_count;
    ...
    struct dbpf_op *op_p)
{
    ...
    else
    {
         ...
         */
        new_handle =
            trove_handle_alloc_from_range(op_p->coll_p->coll_id,
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG, "[%d extents] -- new_handle is %llu "
                 ...
                 llu(new_handle), llu(cur_extent.first), llu(cur_extent.last));
    /*
    ...
    struct dbpf_op *op_p)
{
    ...
    case DB_NOTFOUND:
        ...
        goto return_error;
    default:
        op_p->coll_p->ds_db->err(op_p->coll_p->ds_db, ret,
                                 "dbpf_dspace_remove");
        ret = -dbpf_db_error_to_trove_error(ret);
        goto return_error;
    case 0:
...
static int dbpf_dspace_iterate_handles(
    TROVE_coll_id coll_id,
    ...
    struct dbpf_op *op_p)
{
    ...
    if (*op_p->u.d_iterate_handles.position_p != TROVE_ITERATE_START)
    {
         ...
         */
        dummy_handle = (TROVE_handle) (*op_p->u.d_iterate_handles.position_p);
        memset(&key, 0, sizeof(key));
...
static int dbpf_dspace_getattr(
    TROVE_coll_id coll_id,
    TROVE_handle handle,
    ...
    TROVE_op_id * out_op_id_p)
{
    ...
    {
#if 0
        gossip_debug(GOSSIP_TROVE_DEBUG, "ATTRIB: retrieved "
...
static int dbpf_dspace_setattr(
    TROVE_coll_id coll_id,
    TROVE_handle handle,
    TROVE_ds_attributes_s * ds_attr_p,
    TROVE_ds_flags flags,
    ...
    struct dbpf_op *op_p)
{
    ...
    if (ret != 0)
    {
        op_p->coll_p->ds_db->err(op_p->coll_p->ds_db, ret, "DB->put setattr");
        ret = -dbpf_db_error_to_trove_error(ret);
        goto return_error;
    }
    ...
    gen_mutex_lock(&dbpf_attr_cache_mutex);
    dbpf_attr_cache_ds_attr_update_cached_data(ref, op_p->u.d_setattr.attr_p);
    gen_mutex_unlock(&dbpf_attr_cache_mutex);
    ...
    struct dbpf_op *op_p)
{
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG, "ATTRIB: retrieved attributes "
                 ...
                 llu(op_p->handle), (int) s_attr.uid, (int) s_attr.mode,
                 (int) s_attr.type, (int) s_attr.dfile_count,
                 (int) s_attr.dist_size, llu(b_size));
    ...
    case OP_QUEUED:
        {
            ...
            {
                gossip_debug(GOSSIP_TROVE_DEBUG,
                             "op %p could not be canceled, is drawn"
                             " from pending queue\n", cur_op);
            }
    ...
    TROVE_context_id context_id)
{
    ...
    *inout_count_p = 0;
    ...
    {
        cur_op =
            dbpf_op_pop_front_nolock(dbpf_completion_queue_array[context_id]);
        assert(cur_op);
    ...
    int max_idle_time_ms)
{
  ...
  scan_for_completed_ops:
    ...
    for (i = 0; i < *inout_count_p; i++)
    {
        ...
        ret = (((state == OP_COMPLETED) || (state == OP_CANCELED)) ? 1 : 0);
        if (ret != 0)
        {
