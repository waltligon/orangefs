                             ...
                             struct dbpf_collection* coll_p)
{
    ...
    {
        gossip_debug(GOSSIP_TROVE_DEBUG, "removed dataspace with handle %s\n",
...
static int dbpf_dspace_iterate_handles_op_svc(struct dbpf_op *op_p)
{
    ...
    if (op_p->u.d_iterate_handles.position_p == TROVE_HANDLE_END)
    {
    ...
    if (op_p->u.d_iterate_handles.position_p != TROVE_HANDLE_START)
    {
        ...
        {
            ret = -ret;
            gossip_err("failed to set cursor position: %s\n",
                       PVFS_OID_str(op_p->u.d_iterate_handles.position_p));
            goto return_error;
        }
    }
    else
    ...
    memcpy(op_p->u.d_iterate_handles.position_p, &dummy_handle, sizeof *op_p->u.d_iterate_handles.position_p);

return_ok:
    if (ret == TROVE_ENOENT)
    {
        /* if off the end of the database, return TROVE_HANDLE_END */
        op_p->u.d_iterate_handles.position_p = TROVE_HANDLE_END;
    }
