...
/* return values for dbpf_keyval_read_value_query */
#define DBPF_VALUE_MATCH_NO_ADD    -2
#define DBPF_VALUE_STOP_ITER       -1
#define DBPF_VALUE_MATCH        0
#define DBPF_VALUE_KEEP_ITER        1
#define DBPF_VALUE_MATCH_ADD        2
...
/* add handle to array p if b should be added based on query and a 
 * called from dbpf_keyval_read_value_query */
static int dbpf_value_add_unique_match( char *a, char *b, uint32_t query,
                                        PVFS_handle *p, PVFS_handle h, 
                                        uint32_t *count );
...
static int dbpf_keyval_read_value_query_op_svc(struct dbpf_op *op_p)
{
    ...
    uint32_t cursor_flags = 0, get_flags = 0, unique_held = 0, unique_max = 0;
    struct dbpf_keyval_db_entry key_entry;
    ...
    unique_max = *op_p->u.v_query.count;
    if( *op_p->u.v_query.position_p != TROVE_ITERATE_START && 
        *op_p->u.v_query.position_p != TROVE_ITERATE_END )
    {
        unique_max += *op_p->u.v_query.position_p;
    }
    if( (unique_p = calloc( unique_max, sizeof(PVFS_handle))) == 0 )
    {
        gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, "[DBPF KEYVAL]: malloc for "
                     "unique_p failed.\n");
        free(lookup_key);
    ...
    {   /* if request came with position other than start, whip through them */
        local_p = 0;
        while( (ret == 0) && (local_p < (*op_p->u.v_query.position_p)-1) )
        {
            key.ulen = (2 * DBPF_MAX_KEY_LENGTH);
            key.size = lookup_key_sz - 1;
            ...
            else /* if following dups don't cleanup key */
            {
                key.size = lookup_key_sz - 1;
            }
            ...
            else if( ret != 0 ) /* Unknown failure, bail */
            {
                gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, 
                             "[DBPF KEYVAL]: dbpf_keyval_read_value: BDB "
                             "error iterating to requested position (%llu): "
                             "%s\n", *op_p->u.v_query.position_p, 
                             db_strerror(ret));
            ...
            {
                /* check if we added this entry previously so we can add
                 * it to the unique handle list. this doesn't add it to the
                 * result set. handle is added to unique_p so duplicate handles
                 * across calls are not returned.
                 *
                 * if throw away the return value because we just want the 
                 * handle in the list. if we iterate past matching keys
                 * it'll handle it below (but that shouldn't happe).
                 *
                 * if a value was subsequently stripped by remove_result 
                 * back in the caller, we don't care because any subsequent
                 * results for that handle will fail the same check */
                dbpf_value_add_unique_match( original_key, key.data, 
                                             op_p->u.v_query.query_type,
                                             unique_p, key_entry.handle,
                                             &(unique_held) );
            }
            local_p++;
        }
    ...
    * pointers have buffers set to max allowable size. */
    ...
    {
            ret = dbpf_value_add_unique_match( original_key, key.data, 
                                               op_p->u.v_query.query_type,
                                               unique_p, key_entry.handle,
                                               &(unique_held) );

            if( ret == DBPF_VALUE_MATCH_ADD ) /* should add record to set */
            {
                ...
                gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, 
                     ...
                     llu(op_p->u.v_query.handle_p[
                         (*op_p->u.v_query.count)]));
                (*op_p->u.v_query.count)++;
            }
            else if( ret == DBPF_VALUE_KEEP_ITER )
            {
                gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG,
                             "[DBPF KEYVAL]: dbpf_keyval_read_value: key (%s) "
                             "doesn't match but we'll keep going past (%llu)\n",
                             (char *)key.data, key_entry.handle);
            }
            else if( ret == DBPF_VALUE_MATCH_NO_ADD )
            {
                gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG,
                             "[DBPF KEYVAL]: dbpf_keyval_read_value: key (%s) "
                             "matches but is not unique (%llu)\n",
                             (char *)key.data, key_entry.handle);
            }
            else if( ret == DBPF_VALUE_STOP_ITER ) /* end of what to add */
            { 
                gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, 
            ...
            assert(ret != DBPF_VALUE_MATCH); /*no other possible return values*/
...
static int dbpf_value_add_unique_match( char *a, char *b, uint32_t query,
                                        ...
                                        uint32_t *count )
{
    int max_len = (strlen(a)>strlen(b)?strlen(a):strlen(b)), ret = 0;
    uint32_t i = 0;

    gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, "[DBPF KEYVAL]: "
                 "dbpf_value_add_unique_match: enter: key: (%s)(%s)\n", a, b);
    ...
    gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, "[DBPF KEYVAL]: "
                 "dbpf_value_add_unique_match: ret: (%d)\n", ret);

    /* if key shouldn't be added, return */
    if( ret != DBPF_VALUE_MATCH )
    {
        return ret;
    }

    /* add key to unique list */
    for(i=0; i < *count; i++)
    {
        if( p[i] == h )
        {
        gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, "[DBPF KEYVAL]: "
                     "dbpf_value_add_unique_match: handle (%llu) already "
                     "added\n", llu(h));
            return DBPF_VALUE_MATCH_NO_ADD;
        }
    }

    p[(*count)++] = h;
    gossip_debug(GOSSIP_DBPF_KEYVAL_DEBUG, "[DBPF KEYVAL]: "
                 "dbpf_value_add_unique_match: handle (%llu) matches and "
                 "adding\n", llu(h));
    return DBPF_VALUE_MATCH_ADD;
}
