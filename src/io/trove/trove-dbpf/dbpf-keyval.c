...
static int dbpf_keyval_do_remove(dbpf_db *db_p,
                                 TROVE_handle handle,
                                 char type,
                                 TROVE_keyval_s *key,
                                 TROVE_keyval_s *val);
                            ...
                            PVFS_hint  hints)
{
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove KeyVal Read of %s coll_id %d\n",
                 (char *)key_p->buffer, coll_id);
    ...
    if (cache_elem && (!(flags & TROVE_BINARY_KEY)))
    {
        ...
        if (keyval_pair)
        {
             ...
             */
            gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove %s: checking keyval cache\n",
                         __func__);
            ret = dbpf_attr_cache_keyval_pair_fetch_cached_data(cache_elem,
                                                               keyval_pair,
                                                               val_p->buffer,
                                                               &val_p->read_sz);
            ...
            if(ret < 0)
            {
                gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove %s: keyval cache error: %d\n",
                              __func__, ret);
                return ret;
            }
            //return 1;
    ...
    if (coll_p == NULL)
    {
        gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove %s: bad collection_id\n)", __func__);
        return -TROVE_EINVAL;
    }

    gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove %s: reading handle: %s\n",
    ...
    if(ret < 0)
    {
        gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove %s: keyval read error: %d\n",
                     __func__, ret);
        return ret;
    }
...
static int dbpf_keyval_read_op_svc(struct dbpf_op *op_p)
{
    ...
    assert(op_p->u.k_read.key->buffer_sz <= DBPF_MAX_KEY_LENGTH);
    ...
    {
        gen_mutex_lock(&dbpf_attr_cache_mutex);
        if (dbpf_attr_cache_elem_set_data_based_on_key(ref,
                                                       key_entry.key,
                             ...
                             PVFS_hint  hints)
{
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG, "trove: dbpf_keyval_write handle: %s\n)",
                 PVFS_OID_str(&handle));
    gossip_debug(GOSSIP_TROVE_DEBUG, "*** Trove Keyval Write of %s\n",
                 (char *)key_p->buffer);
...
static int dbpf_keyval_write_list_op_svc(struct dbpf_op *op_p)
{
    ...
    /* This loop goes through all of the handle/key data and reads
     * each combination to make sure it doesn't already exist.
     * after this loop another will write the records to the db.
     */
    ...
    /* >>>> */
        ...
        gossip_debug(GOSSIP_TROVE_DEBUG,
                     "%s: key_entry.handle:%s key_entry.type:%d key_entry.key:%s\n",
...
ret = -TROVE_ENOENT;
    ...
    /* this is the loop where the records are actually written
     */
    gossip_debug(GOSSIP_TROVE_DEBUG,
                 "%s: Writing records\n", __func__);
    /* write reccords */
    for (k = 0; k < op_p->u.k_write_list.count; k++)
    {
        ...
        {
            gossip_debug(GOSSIP_TROVE_DEBUG, "%s: Writing NULL data->data\n", __func__);
            data.data = NULL;
            data.len = 0;
        }
        else
        {
            gossip_debug(GOSSIP_TROVE_DEBUG, "%s: Writing NON-NULL data->data\n", __func__);
        ...
        {
            gossip_debug(GOSSIP_TROVE_DEBUG,
                         "%s: keyval_db->put(handle= %s, key= %s (%d)) ksize=%zu vsize=%zu\n",
                         __func__,
                         PVFS_OID_str(&key_entry.handle), 
                         key_entry.key,
                         op_p->u.k_write_list.key_array[k].buffer_sz,
                         (size_t)key.len, (size_t)data.len);
        }

        gossip_debug(GOSSIP_TROVE_DEBUG, "%s: Writing key/data\n", __func__);
        ret = dbpf_db_put(op_p->coll_p->keyval_db, &key, &data);
        if (ret != TROVE_SUCCESS)
        {
        ...
        {
            gossip_debug(GOSSIP_TROVE_DEBUG,
                         "%s: *** Incrementing Keyval handle count\n", __func__);
            ret = dbpf_keyval_handle_info_ops(
                                     op_p,
                                     DBPF_KEYVAL_HANDLE_COUNT_INCREMENT);
            if(ret != 0)
                             ...
                             PINT_dbpf_keyval_iterate_callback callback)
{
    ...
    for(; i < *count || get_key_count; ++i)
    {
        ...
        gossip_debug(GOSSIP_TROVE_DEBUG, "iterate key: %*s, val: %llu\n", 
                                 ...
                                 TROVE_keyval_s *val)
{
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG,
                 "keyval_db->del(handle= %s, type = %c, key= %s (%d)) size=%zu\n",
                 PVFS_OID_str(&key_entry.handle),
                 key_entry.type,
                 key_entry.key,
                 key->buffer_sz,
...
static int dbpf_keyval_iterate_get_first_entry(TROVE_handle handle,
                                               char type,
                                               dbpf_cursor *dbc)
{
    int ret = 0;
...
static int dbpf_keyval_iterate_skip_to_position(TROVE_handle handle,
                                                char type,
                                                TROVE_ds_position pos,
                                                PINT_dbpf_keyval_pcache *pcache,
                                                dbpf_cursor *dbc)
{
    ...
    ret = PINT_dbpf_keyval_pcache_lookup(pcache,
                                         handle,
                                         pos, 
                                         (const void **)&key.buffer,
                                         &key.buffer_sz);
    if(ret == -PVFS_ENOENT)
    {
...
static int dbpf_keyval_iterate_step_to_position(TROVE_handle handle,
                                                char type,
                                                TROVE_ds_position pos,
                                                dbpf_cursor *dbc)
{
 ...
 */
static int dbpf_keyval_iterate_cursor_get(TROVE_handle handle,
                                          char type,
                                          dbpf_cursor *dbc,
                                          TROVE_keyval_s * key,
                                          TROVE_keyval_s * data,
                                          uint32_t db_flags)
{
    ...
    if (ret != 0)
    {
        gossip_lerr("Failed to perform cursor get:"
                    "\n\thandle: %s\n\ttype: %d\n\tdb error: %d\n",
                    PVFS_OID_str(&key_entry.handle), db_flags, ret);
        return -ret;
    }
...
static int dbpf_keyval_get_handle_info(TROVE_coll_id coll_id,
                                       TROVE_handle handle,
                                       TROVE_ds_flags flags,
                                       TROVE_keyval_handle_info *info,
                                       void * user_ptr,
...
static int dbpf_keyval_get_handle_info_op_svc(struct dbpf_op * op_p)
{
    ...
    if(ret != 0)
    {
        if(ret != TROVE_ENOENT)
        {
            gossip_err("%s: keyval dbpf_db_get (handle info)\n", __func__);
        }
    ...
    gossip_debug(GOSSIP_TROVE_DEBUG,
                 "%s: handle_info get: handle: %s, count: %d\n", __func__,
                 PVFS_OID_str(&op_p->handle),
