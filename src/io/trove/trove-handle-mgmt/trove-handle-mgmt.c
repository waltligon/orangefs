 ...
 */
static int trove_check_handle_ranges(
    TROVE_coll_id coll_id,
        ...
        while (count > 0)
        {
            ...
            if (count > 0)
            {
                for (i = 0; i != count; i++)
                {
                    ...
                    if (!PINT_handle_in_extent_list(extent_list, handles[i]))
                    {
                        gossip_debug(GOSSIP_TROVE_DEBUG,
                                     "handle %llu is invalid "
                                     "(out of bounds)\n", llu(handles[i]));
                        return -1;
                    }
                    ...
                    if (ret != 0)
                    {
                        gossip_debug(GOSSIP_TROVE_DEBUG, "could not remove "
                                     "handle %llu\n", llu(handles[i]));
                        break;
                    }
                }
...
static int trove_map_handle_ranges(
    PINT_llist * extent_list,
    struct handle_ledger *ledger)
{
    int ret = -1;
...
static handle_ledger_t *get_or_add_handle_ledger(
    TROVE_coll_id coll_id)
{
    ...
    )
{
    ...
    {
        s_fsid_to_ledger_table = qhash_init(hash_fsid_compare, hash_fsid, 67);
    ...
    if (handle_range_str)
    {
        ...
        if (extent_list)
        {
            ...
            if (ledger)
            {
                ...
                ret = trove_map_handle_ranges(extent_list, ledger->ledger);
                if (ret != 0)
                {
                    gen_mutex_unlock(&trove_handle_mutex);
                    return ret;
                ...
                ret =
                    trove_check_handle_ranges(coll_id, context_id, extent_list,
                                              ledger->ledger);
                if (ret != 0)
                {
                    gen_mutex_unlock(&trove_handle_mutex);
    ...
    TROVE_handle_extent_array * extent_array)
{
    ...
    if (hash_link)
    {
        ...
        if (ledger && (ledger->have_valid_ranges == 1))
        {
            ...
            {
                handle =
                    trove_ledger_handle_alloc_from_range(ledger->ledger,
                                                         &(extent_array->
                                                           extent_array[i]));
    ...
    int *returned_handle_count)
{
    ...
    if (hash_link)
    {
        ...
        if (ledger && (ledger->have_valid_ranges == 1))
        {
            ret = trove_ledger_peek_handles(ledger->ledger, out_handle_array,
                                            max_num_handles,
    ...
    int *returned_handle_count)
{
    ...
    if (hash_link)
    {
        ...
        if (ledger && (ledger->have_valid_ranges == 1))
        {
            ...
            {
                ret =
                    trove_ledger_peek_handles_from_extent(ledger->ledger,
                                                          &(extent_array->
                                                            extent_array[i]),
                                                          out_handle_array,
                                                          max_num_handles,
    ...
    uint64_t * free_count)
{
    ...
    if (hash_link)
    {
        ...
        if (ledger)
        {
            trove_handle_ledger_get_statistics(ledger->ledger, free_count);
            gen_mutex_unlock(&trove_handle_mutex);
            return (0);
        }
        else
