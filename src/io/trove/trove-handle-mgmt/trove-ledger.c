    ...
    char *admin_name)
{
    ...
    if (extentlist_init(&(ledger->free_list)))
        return NULL;
    if (extentlist_init(&(ledger->recently_freed_list)))
        return NULL;
    if (extentlist_init(&(ledger->overflow_list)))
        return NULL;
...
TROVE_handle trove_ledger_handle_alloc_from_range(
    ...
    TROVE_extent * extent)
{
    return (hl ? extentlist_get_from_extent(&(hl->free_list), extent) :
            TROVE_HANDLE_NULL);
}
...
{
    return (hl ?
            extentlist_peek_handles(&(hl->free_list), out_handle_array,
                                    max_num_handles,
...
{
    return (hl ?
            extentlist_peek_handles_from_extent(&(hl->free_list), extent,
                                                out_handle_array,
                                                max_num_handles,
...
static int handle_store_extentlist_exists(
    TROVE_coll_id coll_id,
    TROVE_handle bstream_handle)
{
    ...
    while (ret == 0)
        trove_dspace_test(coll_id, op_id, &count, NULL, NULL, &state);
    if (ret < 0)
    {
 ...
 *  nonzero		trouble
 */
static int handle_store_extentlist_create(
    TROVE_coll_id coll_id,
    TROVE_handle bstream_handle,
    struct TROVE_handle_extentlist *elist)
{
    ...
    extent_array.extent_array =
        malloc(elist->num_extents * sizeof(TROVE_extent));
    ...
    ret = trove_dspace_create(coll_id, &extent_array, &bstream_handle, TROVE_PLAIN_FILE,        /* not sure exactly what to
                                                                                                   call this, but
                                                                                                   TROVE_TEST_BSTREAM is
                                                                                                   defintely wrong */
                              NULL, TROVE_SYNC, NULL, &op_id);
    while (ret == 0)
        trove_dspace_test(coll_id, op_id, &count, NULL, NULL, &state);
    ...
    struct handle_ledger *ledger)
{
    ...
    {
        ret =
            handle_store_extentlist_read(admin_id, free_list_handle,
                                         &(ledger->free_list));
        if (ret < 0)
        {
    ...
    {
        ret =
            handle_store_extentlist_create(admin_id, free_list_handle,
                                           &(ledger->free_list));
        if (ret < 0)
        {
    ...
    {
        ret =
            handle_store_extentlist_read(admin_id, recently_freed_list_handle,
    ...
    {
        ret =
            handle_store_extentlist_create(admin_id, recently_freed_list_handle,
                                           &(ledger->recently_freed_list));
        if (ret < 0)
        {
            gossip_lerr("creation of backing store failed\n");
    ...
    {
        ret =
            handle_store_extentlist_read(admin_id, overflow_list_handle,
 ...
 *    nonzero if not
 */
inline int trove_handle_ledger_addextent(
    struct handle_ledger *hl,
    TROVE_extent * extent)
{
    return extentlist_addextent(&(hl->free_list), extent->first, extent->last);
}
...
inline int trove_handle_remove(
    struct handle_ledger *hl,
    TROVE_handle handle)
{
 ...
 */
inline void trove_handle_ledger_set_threshold(
    struct handle_ledger *hl,
    uint64_t nhandles)
{
