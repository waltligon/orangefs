...
extern int pvfs2_open(struct inode *inode, struct file *file);
extern int pvfs2_release(struct inode *inode, struct file *file);
...
*/
static int pvfs2_readdir(struct file *file, void *dirent, filldir_t filldir)
{
    int pos = 0, ret = 0;
    ino_t ino = 0;
        ...
        default:
            ...
            */
            new_op->upcall.req.readdir.token = 
                (pos == 2 ? PVFS2_READDIR_START : (PVFS_ds_position)
                 (pos-2));
            ...
            if (new_op->downcall.status == 0)
            {
                ...
                pvfs2_inode->readdir_token =
                    new_op->downcall.resp.readdir.token;
