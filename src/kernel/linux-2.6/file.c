...
static int pvfs2_precheck_file_write(
    struct file *file,
    struct inode *inode,
    size_t * count,
    loff_t * ppos);
    ...
    struct rw_options *rw)
{
    ...
    while (total_count < count)
    {
        ...
        if (rw->type == IO_WRITE)
        {
            ...
            ret =
                pvfs_bufmap_copy_from_user(buffer_index, current_buf,
                                           each_count);
            if (ret < 0)
            {
        ...
        ret = service_operation(new_op, fnstr, get_interruptible_flag(inode));
        ...
        if (ret < 0)
        {
            ...
            {
                pvfs2_error("%s: error writing to handle %llu, "
                            "-- returning %ld\n",
                            fnstr,
                            llu(pvfs2_ino_to_handle(inode->i_ino)), (long) ret);
            }
            goto out;
        }
        if (rw->type == IO_READ)
        {
            ...
            if (new_op->downcall.resp.io.amt_complete)
            {
                if (copy_to_user)
                {
                    ret = pvfs_bufmap_copy_to_user(current_buf, buffer_index,
                                                   new_op->downcall.resp.io.
                                                   amt_complete);
                }
                else
                {
                    ret = pvfs_bufmap_copy_to_kernel(current_buf, buffer_index,
                                                     new_op->downcall.resp.io.
                                                     amt_complete);
                }
                if (ret)
...
static long estimate_max_iovecs(
    ...
    loff_t * offset)
{
    ...
    while (total_count < count)
    {
        ...
        if (type == IO_WRITEV)
        {
            ...
            ret =
                pvfs_bufmap_copy_iovec_from_user(buffer_index, ptr,
                                                 seg_array[seg], each_count);
            if (ret < 0)
            {
        ...
        if (ret < 0)
        {
            ...
            {
                pvfs2_error("%s: error on handle %llu, "
                            "FILE: %s\n  -- returning %zd\n",
                            fnstr, llu(pvfs2_ino_to_handle(inode->i_ino)),
                            (file && file->f_dentry
                             && file->f_dentry->d_name.name ? (char *) file->
                             f_dentry->d_name.name : "UNKNOWN"), ret);
            }
            goto out;
        }
        ...
        if (type == IO_READV)
        {
            ...
            {
                ret =
                    pvfs_bufmap_copy_to_user_iovec(buffer_index, ptr,
                                                   seg_array[seg],
                                                   new_op->downcall.resp.io.
                                                   amt_complete);
                if (ret < 0)
 ...
 * that's a big relief.
 */
static ssize_t pvfs2_aio_retry(
    struct kiocb *iocb)
{
    pvfs2_kiocb *x = NULL;
    ...
    {
        pvfs2_error("pvfs2_aio_retry: could not " " retrieve pvfs2_kiocb!\n");
        return -EINVAL;
    }
    /* highly unlikely, but somehow paranoid need for checking */
    if (((op = x->op) == NULL) || x->kiocb != iocb || x->buffer_index < 0)
    {
        /*
        ...
        do
        {
            ...
            while (op->io_completed == 0)
            {
                ...
                if (!signal_pending(current))
                {
                    ...
                    {
                        pvfs2_print
                            ("Timed out on I/O cancellation - aborting\n");
...
static inline void fill_default_kiocb(
    pvfs2_kiocb * x,
 ...
 */
static ssize_t pvfs2_file_aio_read(
    struct kiocb *iocb,
    char __user * buffer,
    size_t count,
    loff_t offset)
{
    ...
    filp = iocb->ki_filp;
    error = -EINVAL;
    if (filp && filp->f_mapping && (inode = filp->f_mapping->host))
    {
        ssize_t ret = 0;
        pvfs2_kiocb *x = NULL;
        ...
        if ((x = (pvfs2_kiocb *) iocb->private) == NULL)
        {
            ...
            new_op->upcall.req.io.async_vfs_io =
                is_sync_kiocb(iocb) ? PVFS_VFS_SYNC_IO : PVFS_VFS_ASYNC_IO;
            new_op->upcall.req.io.readahead_size = 0;
            ...
            if (is_sync_kiocb(iocb))
            {
                 ...
                 * Stage the operation!
                 */
                ret = service_operation(new_op, "pvfs2_file_aio_read",
                ...
                if (ret < 0)
                {
                    ...
                    {
                        pvfs2_error("pvfs2_file_aio_read: error reading from "
                                    " handle %llu, "
                                    "\n  -- returning %d\n",
 ...
 * context (i.e. the client daemon). 
 */
static ssize_t pvfs2_file_aio_write(
    struct kiocb *iocb,
    ...
    loff_t offset)
{
    ...
    if (filp && inode)
    {
        ...
        if ((x = (pvfs2_kiocb *) iocb->private) == NULL)
        {
             ...
             */
            error =
                pvfs_bufmap_copy_from_user(buffer_index, current_buf, count);
            if (error < 0)
            {
                pvfs2_print("Failed to copy user buffer %d\n", (int) ret);
                /* drop the buffer index */
            ...
            if (is_sync_kiocb(iocb))
            {
                 ...
                 * Stage the operation!
                 */
                ret = service_operation(new_op, "pvfs2_file_aio_write",
                                        get_interruptible_flag(inode));
                if (ret < 0)
                {
                    handle_sync_aio_error();
                    ...
                    {
                        pvfs2_print
                            ("pvfs2_file_aio_write: returning error %d\n",
                             (int) ret);
                    }
                    else
                    {
                        pvfs2_error("pvfs2_file_aio_write: error writing to "
                                    " handle %llu, "
                                    "FILE: %s\n  -- "
                                    "returning %d\n",
                                    llu(pvfs2_ino_to_handle(inode->i_ino)),
                                    (filp && filp->f_dentry
                                     ...
                                     (char *) filp->f_dentry->d_name.
                                     name : "UNKNOWN"), (int) ret);
                    }
                    error = ret;
                    goto out_error;
                }
...
static void do_bypass_page_cache_read(
    struct file *filp,
    loff_t * ppos,
    ...
    for (;;)
    {
        ...
        if (prev_index != begin_index)
        {
            ...
            pvfs2_print("begin_index = %lu offset = %lu file_offset = %ld\n",
                        (unsigned long) begin_index, (unsigned long) offset,
                        (unsigned long) file_offset);

            error =
                pvfs2_inode_read(inode, (void *) kaddr, PAGE_CACHE_SIZE,
                                 &file_offset, 0, 0);
        ...
        pvfs2_print("actor with offset %lu nr %lu return %lu desc->count %lu\n",
                    ...
                    (unsigned long) ret, (unsigned long) desc->count);
    ...
    void *target)
{
     ...
     */
    if ((error =
         pvfs2_inode_getattr(filp->f_mapping->host, PVFS_ATTR_SYS_SIZE)) < 0)
    {
        return error;
    }
