...
extern struct inode *pvfs2_get_custom_inode(struct super_block *sb,
                                            int mode, dev_t dev);
    ...
    struct inode *inode, PVFS_sys_attr *attrs)
{
    ...
    if (inode && attrs)
    {
        ...
        if (attrs->perms & PVFS_O_EXECUTE) perm_mode |= S_IXOTH;
        if (attrs->perms & PVFS_O_WRITE) perm_mode |= S_IWOTH;
        if (attrs->perms & PVFS_O_READ) perm_mode |= S_IROTH;

        if (attrs->perms & PVFS_G_EXECUTE) perm_mode |= S_IXGRP;
        if (attrs->perms & PVFS_G_WRITE) perm_mode |= S_IWGRP;
        if (attrs->perms & PVFS_G_READ) perm_mode |= S_IRGRP;

        if (attrs->perms & PVFS_U_EXECUTE) perm_mode |= S_IXUSR;
        if (attrs->perms & PVFS_U_WRITE) perm_mode |= S_IWUSR;
        if (attrs->perms & PVFS_U_READ) perm_mode |= S_IRUSR;
    ...
    int mode)
{
    ...
    pvfs2_inode_t *pvfs2_inode = NULL;
    struct inode *inode = pvfs2_get_custom_inode(
        dir->i_sb, (S_IFREG | mode), 0);
...
/*         case PVFS2_VFS_OP_SYMLINK: */
            default:
                printk("pvfs2_create_entry got a bad "
                       "op_type (%d)\n",op_type);
        }
    }
    return NULL;
