...
do {                                                        \
    gossip_if (mask)                                        \
    {                                                       \
        PVFS_debug_afield((resp)->caller_handle_index, uint32_t);\
        PVFS_debug_afield((resp)->handle_count, uint32_t);  \
        PVFS_debug_afield((resp)->dentcnt, PVFS_size);      \
        PVFS_debug_afield((resp)->error, PVFS_error);       \
    }                                                       \
    gossip_end;                                             \
} while (0)
