    ...
    {                
        switch (s_op->req->op)
        {
            ...
            case PVFS_SERV_IO:
                ...
                if (PVFS_OID_EQ(&handle, &PVFS_HANDLE_NULL))
                {
        ...
        if (PVFS_OID_NE(&handle, &PVFS_HANDLE_NULL))
        {
            ...
            {
                if (PVFS_OID_EQ(&cap->handle_array[i], &handle))
                {
                    break;
                }
            }
            ...
            {
                 gossip_err("%s: attempted to perform a operation on target "
...
/* Returns true if all the eattrs are dir hints
 * (see mkdir_seteattr_setup_msgpair in sys-mkdir.sm).
 */
static int check_seteattr_dir_hint(struct PVFS_servreq_seteattr *seteattr)
{
    int i;
