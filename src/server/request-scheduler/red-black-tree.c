...
static int inorderWalkthrough(int (*callback)(RBData* data, void* funcData),tree_node* node,void* funcData){
    int ret;
    
	if(node->left != NULL){
        ret = inorderWalkthrough(callback,node->left,funcData);
		if ( ret != 0 )
        {
            return ret;
        }
	}
    ret = (*callback)(node->data,funcData);
	if ( ret != 0 )
    {
        return ret;
    }
    if(node->right != NULL){
    		return inorderWalkthrough(callback,node->right,funcData);
    }
    return 0;	
}
...
static void rebuildRBTree(red_black_tree* tree, tree_node* node){
	...
	if( parent->color == RED ){ /* condition not held ! */
		...
		if(parent == grandparent->left){ 
            /* randparent->color must be BLACK because parent is RED */
