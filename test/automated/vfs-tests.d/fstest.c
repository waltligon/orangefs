    ...
    int fnum)
{
    ...
    fd = open(fname, O_RDWR | O_CREAT | O_TRUNC | (use_sync ? O_SYNC : 0),
              0644);
    if (fd == -1)
    {
        perror(fname);
    ...
    char *argv[])
{
    ...
    printf
        ("num_children=%d file_size=%d num_files=%d loop_count=%d block_size=%d\nmmap=%d sync=%d\n",
         num_children, file_size, num_files, loop_count, block_size, use_mmap,
         use_sync);
