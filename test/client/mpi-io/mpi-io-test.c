    ...
    char **argv)
{
    ...
    if (!opt_rdonly)
    {
        ...
        for (j = 0; j < opt_iter; j++)
        {
            ...
            {
                for (i = 0, v = mynod + j, check = buf; i < opt_block;
                     i++, v++, check++)
                    *check = (char) v;
            }
        ...
        for (j = 0; j < opt_iter; j++)
        {
            ...
            if (err < 0)
            {
                fprintf(stderr, "node %d, read error, loc = %lld: %s\n",
                        mynod, (long long) mynod * opt_block,
                        strerror(myerrno));
            }
            ...
            if (opt_correct)
            {
                ...
                for (i = 0, v = mynod + j, check = buf;
                     i < opt_block && badct < 10; i++, v++, check++)
                {
                    if (*check != (char) v)
                    {
                        my_correct = 0;
    ...
    void)
{
    ...
    printf
        (" -S       all process write to same Single region of file [default: off]\n");
