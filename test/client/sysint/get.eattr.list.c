...
int pvfs2_geteattr_list (int nkey, PVFS_ds_keyval *key_p,
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < user_opts->target_count; i++) {
    ret = pvfs2_geteattr_list(user_opts->nkey, user_opts->key, user_opts->val,
...
int pvfs2_geteattr_list (int nkey, PVFS_ds_keyval *key_p,
  ...
  PVFS_sysresp_geteattr_list resp_geteattr_list;
  ...
  resp_geteattr_list.val_array = val_p;
  ret = PVFS_sys_geteattr_list(resp_lookup.ref,
          &credentials, nkey, key_p, &resp_geteattr_list);
  if (ret < 0)
  {
      PVFS_perror("PVFS_sys_geteattr_list failed with errcode", ret);
      return(-1);
  }
