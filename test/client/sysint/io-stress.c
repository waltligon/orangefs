...
static PVFS_error post_generic_write(
    io_request * req);
static PVFS_error post_generic_read(
    io_request * req);
#define NUM_IO 5
    ...
    io_request * vfs_request)
{
    ...
    if (vfs_request)
    {
        ...
        if (hash_link)
        {
            tmp_vfs_request = qhash_entry(hash_link, io_request, hash_link);
    ...
    io_request * vfs_request)
{
    ...
    if (vfs_request)
    {
        qhash_add(s_ops_in_progress_table,
                  (void *) (&vfs_request->tag), &vfs_request->hash_link);
        ret = 0;
    }
    return ret;
}
    ...
    int num_io)
{
    ...
    while (1)
    {
        ...
        ret = PVFS_sys_testsome(op_id_array, &op_count, (void **) all_request,
                                error_code_array, 10);
        for (i = 0; i < op_count; i++)
        {
            ...
            {
                PVFS_perror_gossip
                    ("Failed to remove op in progress from table\n", ret);
            }
            count--;
            if (count > 0)
                post_op(req);
    ...
    io_request * req)
{
    ...
    else
    {
        ...
        ret = PVFS_isys_io(req->src->u.pvfs2.ref, req->file_req, req->offset,
                           req->buffer, req->mem_req, &req->credentials,
                           &req->resp_io, PVFS_IO_READ, &req->op_id, req);
        if (ret != 0)
        {
    ...
    io_request * req)
{
    ...
    else
    {
        ...
        ret = PVFS_isys_io(req->src->u.pvfs2.ref, req->file_req, req->offset,
                           req->buffer, req->mem_req, &req->credentials,
                           &req->resp_io, PVFS_IO_WRITE, &req->op_id, req);
        if (ret != 0)
        {
            PVFS_perror("PVFS_isys_io", ret);
            PVFS_Request_free(&req->mem_req);
