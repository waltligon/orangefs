...
#ifndef NUM_KEYS_PER_REQUEST
#define NUM_KEYS_PER_REQUEST 10
#endif
...
int pvfs2_listeattr (int nkey, PVFS_ds_keyval *key_p,
        char *destfile);
...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < user_opts->target_count; i++) {
    ret = pvfs2_listeattr(user_opts->nkey, user_opts->key, user_opts->destfiles[i]);
    if (ret < 0) {
        printf("listeattr returned error code - exiting\n");
        break;
    }
    printf("listeattr returned %d keys\n", ret);
    for (k = 0; k < (user_opts->nkey ? ret : 0); k++) {
        printf("key[%d]:%s\n",k,
...
/* pvfs2_listeattr()
 *
 * Retrieves the list of keys for a particular object!
 * returns # of keys read successfully on success and negative one on failure
 */
int pvfs2_listeattr(int nkey, PVFS_ds_keyval *key_p,
        char *destfile) 
{
...
static void usage(int argc, char** argv)
{
    fprintf(stderr,"Usage: %s [-v] -n <# of keys> filename(s)\n",argv[0]);
