    ...
    char **argv)
{
    ...
    ret = PINT_ncache_insert("testfile001", PVFS2_LOOKUP_LINK_NO_FOLLOW,
                             test_ref, root_ref);

    test_ref.handle = 1001;
    ret = PINT_ncache_insert("testfile001", PVFS2_LOOKUP_LINK_FOLLOW,
                             test_ref, root_ref);

    ret = PINT_ncache_lookup("testfile001", PVFS2_LOOKUP_LINK_NO_FOLLOW,
                             root_ref, &test_ref);
    ...
    ret = PINT_ncache_lookup("testfile001", PVFS2_LOOKUP_LINK_FOLLOW,
                             root_ref, &test_ref);
    ...
    for (i = 0; i < ENTRIES_TO_ADD; i++)
    {
        ...
        test_ref.handle = i;
        test_ref.fs_id = 0;
        ret = PINT_ncache_insert(new_filename[i], PVFS2_LOOKUP_LINK_NO_FOLLOW,
                                 test_ref, root_ref);
        if (ret < 0)
        {
    ...
    for (i = 0; i < ENTRIES_TO_ADD; i++)
    {
        ret = PINT_ncache_lookup(new_filename[i], PVFS2_LOOKUP_LINK_NO_FOLLOW,
                                 root_ref, &test_ref);
    ...
    gossip_debug(GOSSIP_NCACHE_DEBUG, "Waiting for all entries to expire\n");
    sleep(5);
    ...
    for (i = 0; i < ENTRIES_TO_ADD; i++)
    {
        ...
        {
            ret =
                PINT_ncache_lookup(new_filename[i], PVFS2_LOOKUP_LINK_NO_FOLLOW,
                                   root_ref, &test_ref);
    ...
    for (i = 0; i < ENTRIES_TO_ADD; i++)
    {
        ret = PINT_ncache_remove(new_filename[i], PVFS2_LOOKUP_LINK_NO_FOLLOW,
                                 root_ref, &found_flag);
        if (ret < 0)
        {
