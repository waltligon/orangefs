    ...
    cm_frame_t * frame)
{
    ...
    frame->cf_valid_start =
        (cm_offset_t *) calloc(sizeof(cm_offset_t), frame->cf_valid_count);
    frame->cf_valid_size =
        (cm_size_t *) calloc(sizeof(cm_size_t), frame->cf_valid_count);
    ...
    for (c = 0; c < frame->cf_valid_count; c++)
    {
        ...
        assert(frame->cf_valid_start[c] + frame->cf_valid_size[c] <=
               frame->buf_size);

        printf("(%d): valid_start: %Ld, valid_size: %d\n", c,
               frame->cf_valid_start[c], frame->cf_valid_size[c]);
    ...
    char **argv)
{
    ...
    ret = PVFS_util_resolve(fname, &cur_fs, pvfs_path, PVFS_NAME_MAX);
    if (ret < 0)
    {
    ...
    if (strcmp(pvfs_path, "/") == 0)
    {
        ...
        while (!PINT_string_next_segment(fname, &segp, &segstate))
        {
            prev_segp = segp;
        }
    ...
    else
    {
        ...
        ret =
            PINT_lookup_parent(pvfs_path, cur_fs, &credentials,
                               &parent_ref.handle);
    ...
    if (ret == 0)
    {
        fprintf(stderr, "Target file %s already exists! Deleting.\n",
                entry_name);
        if (PVFS_sys_remove(entry_name, parent_ref, &credentials) < 0)
        {
    ...
    while (c > 0)
    {
        ...
        ret = PVFS_Request_hindexed(tmp_valid_count,
                                    &frame.cf_valid_size[count],
                                    &frame.cf_valid_start[count], PVFS_BYTE,
                                    &mem_req);
        if (ret < 0)
        {
    ...
    printf("Bandwidth: %f MB/second\n",
           (((double) total_written) / ((double) (1024 * 1024)) /
            (time2 - time1)));
    printf("********************************************************\n\n");

    ret = PVFS_sys_getattr(ref, PVFS_ATTR_SYS_ALL, &credentials, &resp_getattr);
    if (ret < 0)
    {
    ...
    printf("Elapsed time: %f seconds\n", (time2 - time1));
    printf("Bandwidth: %f MB/second\n",
           (((double) total_read) / ((double) (1024 * 1024)) /
            (time2 - time1)));
    printf("********************************************************\n");
    ...
    for (c = 0; c < frame.cf_valid_count; c++)
    {
        ...
        {
            fprintf(stderr,
                    "(%d) -> Read buffer did not match with write buffer from [%Ld upto %d bytes]\n",
