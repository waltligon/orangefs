...
{
    int ret = 0, mode = 0;
    char filename[PATH_MAX] = "", testDir[PATH_MAX] = "",       /* Directory where all the test files will reside */
        pvfsPath[PVFS_NAME_MAX] = "",   /* fs relative path */
        cmd[PATH_MAX] = "", pvfs2_tab_file[PATH_MAX] = "";      /* used to store pvfs2 tab file */
    ...
    ret = create_directory(testDir,
                           mode, commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    ret =
        remove_directory(testDir, commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    if (commonOpts.use_pvfs2_lib)
    {
        ...
        else
        {
            ...
            {
                fprintf(stdout,
                        "\t PASSED. [%s] created with parent mode set and mode [%o]\n",
                        filename, mode);
            }
        }
    ...
    if (commonOpts.use_pvfs2_lib)
    {
        ...
        else
        {
            ...
            {
                fprintf(stdout,
                        "\t PASSED. created two directories via command line\n");
            }
        }
