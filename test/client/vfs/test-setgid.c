...
static int destroy_setgid(
    const char *directory,
    struct common_options *commonOpts);
static int determine_group(
    char **groupName,
    gid_t * group_id);
static int determine_user(
    char **userName,
    uid_t * user_id);
static void cleanup(
    ...
    char **argv)
{
    ...
    {
        print_error
            ("\tERROR: Group [%s]:[%d] must be different than user [%s]:[%d]\n",
             opts.groupName, opts.group_id, opts.userName, opts.user_id);
        exit(1);
    }
    ...
    if (commonOpts.use_pvfs2_lib)
    {
        ...
        ret = is_pvfs2(commonOpts.directory,
                       ...
                       PVFSPath,
                       sizeof(PVFSPath),
                       commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    sprintf(testDir, "%s/%s-%s", commonOpts.directory, opts.userName,
            "setgid-test");
     ...
     */
    ret = stat_file(testDir, &stStats, 0,       /* Don't follow link */
                    commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    ret = open_file(fileName, O_RDONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, commonOpts.use_pvfs2_lib, commonOpts.verbose, 1,       /* Follow Link */
                    &stFileRef);
    ...
    ret = stat_file(fileName, &stStats, 0,      /* Don't follow link */
                    commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    ret = create_directory(fileName, S_IRWXO | S_IRWXG | S_IRWXU,       /* 0777 */
                           commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    ret =
        remove_directory(fileName, commonOpts.use_pvfs2_lib,
                         commonOpts.verbose);
...
{
    int num_avail_users = 0, counter = 0;
    struct passwd *user = NULL;
...
{
    int num_avail_groups = 0, counter = 0;
    struct group *group = NULL;
