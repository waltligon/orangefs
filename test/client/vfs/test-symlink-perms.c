    ...
    char **argv)
{
     ...
     */
    ret = stat_file(linkName, &stStats, 0,      /* Don't follow link */
                    commonOpts.use_pvfs2_lib, commonOpts.verbose);
    ...
    else
    {
        ...
        {
            print_error
                ("\tFAILED. Permissions are incorrect (before chmod) on [%s]\n",
                 linkName);
        }
    }
    ...
    ret =
        change_mode(linkName, 0700, &error_code, commonOpts.use_pvfs2_lib,
                    commonOpts.verbose);
    ...
    else
    {
        ...
        {
            print_error
                ("\tFAILED. Permissions are incorrect (after chmod) on [%s]\n",
                 linkName);
        }
    }
    ...
    struct common_options *opts)  /**< Options structure */
{
    ...
    /* Create the file */
    ret = create_file(file_name, mode, opts->use_pvfs2_lib, opts->verbose);
    ...
    ret = create_symlink(link_name,
                         file_name, opts->use_pvfs2_lib, opts->verbose);
