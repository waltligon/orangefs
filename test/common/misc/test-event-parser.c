    ...
    char **argv)
{
    ...
    data_array =
        (struct data_point *) malloc(array_size * sizeof(struct data_point));
    if (!data_array)
    {
        perror("malloc");
        return (-1);
    ...
    printf
        ("# (measurement #) (server) (api_op) (size/value) (start) (end) (length)\n");
    ...
    while (fgets(tmp_buf, 512, infile))
    {
        ...
        ret = sscanf(tmp_buf, "%d %d %d %lld %lld %d %ld %ld\n",
                     ...
                     &data_array[cur_index].sec, &data_array[cur_index].usec);
        if (ret != 8)
        {
    ...
    for (cur_index = 0; cur_index < array_size; cur_index++)
    {
        ...
        {
            if (!data_array[run_index].processed &&
                ...
                data_array[run_index].id == data_array[cur_index].id)
            {
                ...
                printf("%d\t%d_%d\t", data_array[cur_index].server,
                       data_array[cur_index].api, data_array[cur_index].op);
