    ...
    config * myconfig)
{
    ...
    while (myconfig->testqueue[index].test_func != NULL)
    {
        ...
        {
            params =
                run_param(myconfig->testqueue[index].test_param_init,
                          myconfig->testqueue[index].test_params);
            if (params == NULL)
            {
        ...
        {
            fprintf(stderr,
                    "PTS: test %s not set up properly, no function to run\n",
                    myconfig->testqueue[index].test_name);
            rc = 1;
        }
        ...
        MPI_Gather(&errcode, 1, MPI_INT, allerrcodes, 1, MPI_INT, 0,
                   MPI_COMM_WORLD);
    ...
    config * myconfig)
{
    ...
    while ((c = getopt_long(argc, argv, "d:c:", long_opts, &index)) != -1)
    {
        switch (c)
        {
        case 'd':
        ...
        if (linebuf[0] != '#')
        {
            ...
            if (strindex != NULL)
            {
                *strindex = '\0';
                ...
                {
                    fprintf(stderr,
                            "parse error, test '%s' not found internally\n",
                            linebuf);
                }
                else
                {
