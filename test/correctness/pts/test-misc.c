...
static int test_permissions(int testcase){
    ...
    ret = PVFS_sys_read(resp_lk.pinode_refn, req_io, io_buffer, 100,
                          credentials, &resp_io);
    PVFS_sys_finalize();
    return ret;
...
static int test_size_after_write(int testcase){
    ...
    ret = PVFS_sys_write(resp_lk.pinode_refn, req_io, io_buffer, 100,
                           credentials, &resp_io);
    if(ret < 0){
...
static int test_read_beyond(int testcase){
    ...
    ret = PVFS_sys_read(resp_lk.pinode_refn, req_io, io_buffer, resp.attr.u.data.size+ 100, credentials, &resp_io);
    if(ret < 0){
	debug_printf("write failed on %s\n", filename);
    }
...
static int test_write_beyond(int testcase){
    ...
    ret = PVFS_sys_write(resp_lk.pinode_refn, req_io, io_buffer, oldsize,
                           credentials, &resp_io);
    if(ret < 0){
	debug_printf("write failed on %s\n", filename);
    }
...
static int test_io_on_dir(int testcase)
{
    ...
    {
	case 0:
	    ret = PVFS_sys_read(resp_lookup.pinode_refn, req_io, io_buffer, 100, credentials, &resp_io);
	    break;
	case 1:
	    for(i = 0; i < 100; i++)
	    {
	    ...
	    ret = PVFS_sys_write(resp_lookup.pinode_refn, req_io, io_buffer, 100, credentials, &resp_io);
	    break;
