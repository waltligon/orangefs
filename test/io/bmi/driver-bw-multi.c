...
static int mpi_server_postall(struct bench_options* opts, struct 
	mem_buffers* mpi_buf_array, int num_clients, int* addr_array, 
	double* wtime, int world_rank);
...
int main( int argc, char *argv[])
{
	...
	if(im_a_server)
	{
		...
		bmi_buf_array = (struct mem_buffers*)malloc(num_clients * 
			sizeof(struct mem_buffers));
	...
	else
	{
		...
		mpi_buf_array = (struct mem_buffers*)malloc(opts.num_servers * 
			sizeof(struct mem_buffers));
		bmi_buf_array = (struct mem_buffers*)malloc(opts.num_servers * 
			sizeof(struct mem_buffers));
		if(!mpi_buf_array || !bmi_buf_array)
		{
			fprintf(stderr, "malloc failure.\n");
	 ...
	 * times and max times
	 */
	MPI_Allreduce(&mpi_time, &max_mpi_time, 1, MPI_DOUBLE, MPI_MAX,
		comm);
	MPI_Allreduce(&mpi_time, &min_mpi_time, 1, MPI_DOUBLE, MPI_MIN,
		comm);
	MPI_Allreduce(&mpi_time, &sum_mpi_time, 1, MPI_DOUBLE, MPI_SUM,
		comm);
	MPI_Allreduce(&bmi_time, &max_bmi_time, 1, MPI_DOUBLE, MPI_MAX,
		comm);
	MPI_Allreduce(&bmi_time, &min_bmi_time, 1, MPI_DOUBLE, MPI_MIN,
		comm);
	MPI_Allreduce(&bmi_time, &sum_bmi_time, 1, MPI_DOUBLE, MPI_SUM,
		comm);
	sq_bmi_time = bmi_time * bmi_time;
	sq_mpi_time = mpi_time * mpi_time;
	MPI_Allreduce(&sq_bmi_time, &sumsq_bmi_time, 1, MPI_DOUBLE, MPI_SUM,
		comm);
	MPI_Allreduce(&sq_mpi_time, &sumsq_mpi_time, 1, MPI_DOUBLE, MPI_SUM,
		comm);
	...
	if(world_rank == 0)
	{
		...
		printf("%d %d %f %f %f %f %f (msg_len,servers,min,max,ave,stddev,agg_MB/s) bmi server\n",
			opts.message_len,opts.num_servers, min_bmi_time, max_bmi_time, ave_bmi_time, 
			stddev_bmi_time, agg_bmi_bw/(1024*1024));
		printf("%d %d %f %f %f %f %f (msg_len,servers,min,max,ave,stddev,agg_MB/s) mpi server\n",
			opts.message_len,opts.num_servers, min_mpi_time, max_mpi_time, ave_mpi_time, 
			stddev_mpi_time, agg_mpi_bw/(1024*1024));
	}
	...
	if(world_rank == opts.num_servers)
	{
		...
		printf("%d %d %f %f %f %f %f (msg_len,clients,min,max,ave,stddev,agg_MB/s) bmi client\n",
			opts.message_len,num_clients, min_bmi_time, max_bmi_time, ave_bmi_time, 
			stddev_bmi_time, agg_bmi_bw/(1024*1024));
		printf("%d %d %f %f %f %f %f (msg_len,clients,min,max,ave,stddev,agg_MB/s) mpi client\n",
			opts.message_len,num_clients, min_mpi_time, max_mpi_time, ave_mpi_time, 
			stddev_mpi_time, agg_mpi_bw/(1024*1024));
	}
	...
	bmi_flag_t buffer_flag, double* wtime, int world_rank)
{
	...
	actual_size_array = (bmi_size_t*)malloc(num_clients * 
		sizeof(bmi_size_t));
	...
	while(done_clients < num_clients)
	{
		...
		for(i=0; i<outcount; i++)
		{
			...
			else
			{
				...
				{
					id_array[index_array[i]] = ids[index_array[i]][done_index[index_array[i]]];
				}
			}
		}
	}
	...
	bmi_flag_t buffer_flag, double* wtime, int world_rank)
{
	...
	actual_size_array = (bmi_size_t*)malloc(num_servers * 
		sizeof(bmi_size_t));
	...
	for(i=0; i<num_buffers; i++)
	{
		for(j=0; j<num_clients; j++)
		{
			...
			ret = MPI_Irecv(recv_buffer, mpi_buf_array[0].size, 
				MPI_BYTE, addr_array[j], 0, MPI_COMM_WORLD, &requests[j][i]);
			if(ret != MPI_SUCCESS)
			{
