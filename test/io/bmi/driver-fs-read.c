...
int svr_handle_next(
    struct svr_xfer_state *state,
    bmi_context_id context);
int svr_handle_next_mpi(
    struct svr_xfer_state *state);
int client_handle_next_mpi(
    struct svr_xfer_state *state);
int client_handle_next(
    struct svr_xfer_state *state,
    bmi_context_id context);
int prepare_states(
    struct svr_xfer_state *state_array,
    PVFS_BMI_addr_t * addr_array,
    struct bench_options *opts,
    int count,
    int svr_flag);
int prepare_states_mpi(
    struct svr_xfer_state *state_array,
    int *addr_array,
    struct bench_options *opts,
    int count,
    ...
    char *argv[])
{
    ...
    state_array = (struct
                   svr_xfer_state *) malloc(state_size *
                                            sizeof(struct svr_xfer_state));
    assert(state_array);
    ...
    if (im_a_server)
    {
        while (num_done < num_clients)
        {
            ...
            while (indexer < outcount)
            {
                ...
                state_array[num_requested].unexp_buffer =
                    info_array[indexer].buffer;
                state_array[num_requested].unexp_size =
                    info_array[indexer].size;
            ...
            ret = BMI_testcontext(TESTCOUNT, id_array, &outcount,
                                  error_array, size_array, user_ptr_array, 0,
                                  context);
            assert(ret >= 0);
            indexer = 0;
            while (indexer < outcount)
    ...
    {
        request_array =
            (MPI_Request *) malloc(((num_messages + 1) * num_clients) *
                                   sizeof(MPI_Request));
        index_array =
            (int *) malloc(((num_messages + 1) * num_clients) * sizeof(int));
        status_array =
            (MPI_Status *) malloc(((num_messages + 1) * num_clients) *
                                  sizeof(MPI_Status));
        stupid_array =
            (void **) malloc(((num_messages + 1) * num_clients) *
                             sizeof(void *));
    ...
    {
        request_array =
            (MPI_Request *) malloc(((num_messages + 2) * num_clients) *
                                   sizeof(MPI_Request));
        index_array =
            (int *) malloc(((num_messages + 2) * num_clients) * sizeof(int));
        status_array =
            (MPI_Status *) malloc(((num_messages + 2) * num_clients) *
                                  sizeof(MPI_Status));
        stupid_array =
            (void **) malloc(((num_messages + 2) * num_clients) *
                             sizeof(void *));
        assert(request_array);
        assert(index_array);
        assert(status_array);
    ...
    if (im_a_server)
    {
        while (num_done < num_clients)
        {
            ...
            ret = MPI_Iprobe(MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &flag, &status);
            ...
            {
                ret =
                    svr_handle_next_mpi((struct svr_xfer_state *)
                                        stupid_array[index_array[indexer]]);
                assert(ret == 0);
                indexer++;
            }
        }
    ...
    else
    {
        ...
        while (num_done < opts.num_servers)
        {
            ...
            {
                ret =
                    client_handle_next_mpi((struct svr_xfer_state *)
                                           stupid_array[index_array[indexer]]);
                assert(ret == 0);
                indexer++;
            }
    ...
    bmi_context_id context)
{
    ...
    case 0:
        ...
        ret = BMI_post_recv(&tmp_id, state->addr, state->resp,
                            sizeof(struct response), &actual_size,
                            BMI_PRE_ALLOC, state->tag, state, context);
        if (ret < 0)
        {
        ...
        ret = BMI_post_sendunexpected(&tmp_id, state->addr, state->req,
                                      sizeof(struct request), BMI_PRE_ALLOC,
    ...
    default:
        ...
        ret = BMI_post_recv_list(&tmp_id, state->addr,
                                 state->buffer_list, state->size_list,
                                 state->list_factor, MSG_SIZE, &actual_size,
                                 BMI_PRE_ALLOC, state->tag, state, context);
        assert(ret >= 0);
        if (ret == 1)
            return (client_handle_next(state, context));
    ...
    if (svr_flag == 0)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            state_array[i].buffer_array =
                (void **) malloc(state_array[i].buffer_array_size *
                                 sizeof(void *));
            assert(state_array[i].buffer_array);
            for (j = 0; j < state_array[i].buffer_array_size; j++)
            {
                state_array[i].buffer_array[j] = malloc(MSG_SIZE);
            ...
            state_array[i].size_list =
                (bmi_size_t *) malloc(opts->list_io_factor *
                                      sizeof(bmi_size_t));
    ...
    struct svr_xfer_state *state)
{
    ...
    case 0:
        ...
        ret = MPI_Recv(state->mpi_unexp_buffer, state->mpi_unexp_size,
                       MPI_BYTE, state->mpi_addr, state->mpi_tag,
    ...
    case 1:
        ...
        for (i = 0; i < state->buffer_array_size; i++)
        {
            ...
            ret = MPI_Isend(state->buffer_array[i],
                            MSG_SIZE, MPI_BYTE, state->mpi_addr, 0,
                            MPI_COMM_WORLD, &request_array[request_count]);
            request_count++;
            assert(ret == MPI_SUCCESS);
        }
    ...
    int svr_flag)
{
    ...
    if (svr_flag == 0)
    {
        ...
        for (i = 0; i < count; i++)
        {
            ...
            {
                state_array[i].displace_array[j] =
                    state_array[i].size_list[j - 1];
            }
            ret = MPI_Type_hindexed(opts->list_io_factor,
                                    state_array[i].blocklen_array,
                                    state_array[i].displace_array, MPI_BYTE,
                                    &state_array[i].dtype);
            assert(ret == MPI_SUCCESS);
