...
extern void cache_dump_active_list(
    void);
extern void cache_dump_inactive_list(
    void);
    ...
    char *argv[])
{
    ...
    if (pthread_create(&thread1,
                       NULL, (void *) do_io_write, (void *) &ret1) != 0)
        perror("pthread_create"), exit(1);

    if (pthread_create(&thread2,
                       NULL, (void *) do_io_write, (void *) &ret2) != 0)
        perror("pthread_create"), exit(1);

    if (pthread_create(&thread3,
                       NULL, (void *) do_io_write, (void *) &ret3) != 0)
        perror("pthread_create"), exit(1);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                {
                    fprintf(stderr,
                            "cache_req_test ok: flag=%d, status: %d, cbufcnt=%d\n",
