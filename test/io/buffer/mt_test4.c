    ...
    char *argv[])
{
    ...
    if (pthread_create(&thread1, NULL, (void *) do_io, (void *) &ret1) != 0)
        perror("pthread_create"), exit(1);

    if (pthread_create(&thread2, NULL, (void *) do_io, (void *) &ret2) != 0)
        perror("pthread_create"), exit(1);
    ...
    int *id)
{
    ...
    while (comp < req_cnt)
    {
        for (i = start; i < end; i++)
        {
            if (request[i].status == NCAC_COMPLETE)
                continue;
    ...
    for (i = start; i < end; i++)
    {
        offarr[0] = i * 65536;
        ret =
            cache_read_post((cache_read_desc_t *) & desc, &request[i],
                            &reply[i], NULL);
        if (ret < 0)
        {
