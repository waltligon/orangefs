    ...
    char **argv)
{
    ...
    ret = trove_dspace_create(coll_id,
			      ...
			      &file_handle,
			      TROVE_TEST_BSTREAM,
			      NULL, TROVE_SYNC, NULL, trove_context, &t_op_id);
    while (ret == 0)
	ret =
	    trove_dspace_test(coll_id, t_op_id, trove_context, &count, NULL,
			      NULL, &state, TROVE_DEFAULT_TEST_TIMEOUT);
    if (ret < 0)
    {
	fprintf(stderr,
		"HEY: dspace create failed, I hope because it already existed\n");
    }
    ...
    ret = trove_bstream_write_at(coll_id, file_handle, mybuffer,
				 &tmp_size, 0, 0, NULL, NULL, trove_context,
				 &t_op_id);
    while (ret == 0)
	ret =
	    trove_dspace_test(coll_id, t_op_id, trove_context, &count, NULL,
    ...
    TROVE_handle * out_handle_p)
{
    ...
    while (ret == 0)
	trove_dspace_test(coll_id, op_id, trove_context,
