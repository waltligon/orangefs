    ...
    char **argv)
{
	...
	PINT_REQUEST_PACK_SIZE(file_req) + PINT_DIST_PACK_SIZE(io_dist);
    ...
    encode_io_dist =
	(PVFS_Dist *) ((char *) encode_file_req + req->file_req_size);
    ...
    ack = BMI_memalloc(server_addr, sizeof(struct wire_harness_ack), BMI_RECV);
    if (!ack)
    {
    ...
    ret = run_io_operation(server_addr, req, total_req_size, ack,
			   memory_buffer, io_size, file_req, io_dist, context,
			   flow_context);
    if (ret < 0)
    {
	fprintf(stderr,
		"Error: failed to successfully complete I/O exchange.\n");
	return (-1);
    }
    ...
    BMI_memfree(server_addr, ack, sizeof(struct wire_harness_ack), BMI_RECV);
    ...
    FLOW_context_id flow_context)
{
    ...
    printf
	("** sending req: fsid (ignored) = %d, handle = %d, op = %d, io_r_sz = %d, dist_sz = %d\n",
	 (int) req->fs_id, (int) req->handle, req->op, (int) req->file_req_size,
	 (int) req->dist_size);
    ...
    ret = BMI_post_recv(&op, server_addr, ack, sizeof(struct
						      wire_harness_ack),
			&actual_size, BMI_PRE_ALLOC, 0, NULL, context);
    if (ret < 0)
    {
