    ...
    char **argv)
{
    ...
    {
        fprintf(stderr,
                "flow_initialize() succeeded when it shouldn't have.\n");
        return (-1);
    }
    ...
    ret = job_trove_dspace_create(coll_id,
                                  ...
                                  NULL,
                                  TROVE_SYNC,
                                  NULL, 0, &job_stat, &foo_id, context);
    if (ret < 0)
    {
    ...
    ret = job_trove_keyval_write(coll_id, parent_handle, &key,
                                 &val, 0, NULL, NULL, 0, &job_stat, &foo_id,
                                 context);
    if (ret < 0)
    {
    ...
    ret = job_trove_bstream_write_at(coll_id, file_handle, 0,
                                     buffer1, BUF_SIZE, 0, NULL, NULL, 0,
    ...
    if (ret == 0)
    {
        ...
        {
            fprintf(stderr,
                    "job_trove_bstream_write_at() failed (at job_test()).\n");
            return (-1);
        }
    }
    if (job_stat.error_code)
    ...
    ret = job_trove_bstream_read_at(coll_id, file_handle, 0,
                                    buffer2, BUF_SIZE, 0, NULL, NULL, 0,
                                    &job_stat, &foo_id, context);
    if (ret < 0)
    {
    ...
    if (ret == 0)
    {
        ...
        {
            fprintf(stderr,
                    "job_trove_bstream_read_at() failed (at job_test()).\n");
            return (-1);
        }
    }
    if (job_stat.error_code)
