    ...
    while (!ret)
    {
        ...
        if (ret == 0)
        {
            ...
            fprintf(stderr, "lio_state: mct=%d,sct=%d,"
                    "cur_mem_size: %lld, cur_mem_off:%p, "
                    "cur_stream_size: %lld, cur_stream_off:%llu\n",
                    lio_state.mem_ct, lio_state.stream_ct,
                    lld(lio_state.cur_mem_size), lio_state.cur_mem_off,
                    lld(lio_state.cur_stream_size),
                    llu(lio_state.cur_stream_off));
        }
    }
