    ...
    TROVE_handle * out_handle_p)
{
    ...
    ret =
        trove_collection_geteattr(coll_id, &key, &val, 0, NULL, context_id,
                                  &op_id);
    while (ret == 0)
        ret =
    ...
    for (;;)
    {
        ...
        val.buffer = &handle;
        val.buffer_sz = sizeof(handle);
        ret = trove_keyval_read(coll_id, parent_handle, &key, &val, 0,
                                NULL, NULL, context_id, &op_id);
        while (ret == 0)
            ret =
                trove_dspace_test(coll_id, op_id, context_id, &count, NULL,
        ...
        ret =
            trove_dspace_getattr(coll_id, handle, &s_attr, 0, NULL, context_id,
                                 &op_id);
        while (ret == 0)
            ret =
                trove_dspace_test(coll_id, op_id, context_id, &count, NULL,
    ...
    char **argv)
{
    ...
    while ((c = getopt(argc, argv, "s:c:f:")) != EOF)
    {
        ...
        default:
            fprintf(stderr,
                    "%s: [-s storage space] [-c collection] [-f file_path ]\n",
                    argv[0]);
            return -1;
        }
    }
