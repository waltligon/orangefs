    ...
    int *server_count)
{
    ...
    ret = PVFS_mgmt_count_servers(cur_fs, &creds, flags, server_count);
    if (ret < 0)
    {
    ...
    perf_matrix =
        (struct PVFS_mgmt_perf_stat **) malloc(count *
    ...
    end_time_ms_array = (uint64_t *) malloc(count * sizeof(uint64_t));
    ...
    ret = PVFS_mgmt_get_server_array(cur_fs, &creds, flags, addr_array, &count);
    if (ret < 0)
    {
    ...
    for (i = 0; i < count; i++)
    {
        for (j = 0; j < HISTORY; ++j)
        {
            ...
            printf("%d\t%llu\t%lld\t%lld\n",
                   ...
                   lld(perf_matrix[i][j].write), lld(perf_matrix[i][j].read));
        }
    }
    ...
    for (i = 0; i < count; i++)
    {
        for (j = 0; j < HISTORY; ++j)
        {
            ...
            printf("%d\t%llu\t%u\t%u\t%u\n",
                   ...
                   perf_matrix[i][j].keyval_queue, perf_matrix[i][j].reqsched);
        }
    }
...
void test_util_print_avg_and_dev(
    void)
{
    int i = 0;
    float avg, dev;
