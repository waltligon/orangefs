 ...
 */
unsigned int 
crc32c_le(unsigned int crc, unsigned char const *p, size_t len)
{
	int i;
	while (len--) {
		crc ^= *p++;
...
static unsigned int crc32c_table[256] = {
...
unsigned int 
crc32c_le(unsigned int seed, unsigned char const *data, size_t length)
{
	unsigned int crc = __cpu_to_le32(seed);
...
#if CRC_BE_BITS == 1
unsigned int 
crc32c_be(unsigned int crc, unsigned char const *p, size_t len)
{
	int i;
	while (len--) {
		crc ^= *p++ << 24;
