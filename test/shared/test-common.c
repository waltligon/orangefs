...
static void display_common_usage(
    char *exeName);
 ...
 */
int set_util_path(
    const char *utility_path)                /**< NULL terminated path. MUST end with "/" */
{
    int retval = 0;
    ...
    retval =
        snprintf(pvfsEXELocation, sizeof(pvfsEXELocation), "%s", utility_path);
    ...
    const char *fs_file) /**< fs_file as defined in fstab (mount point) */
{
    ...
    retval = snprintf(fs_spec, sizeof(fs_spec), "%s://%s:%d/%s",
                      networkProto, hostname, port, fsname);
 ...
 * \retval TEST_COMMON_FAIL Failure 
 */
int destroy_pvfs2tab_file(
    const char *pvfs2tab_name)                        /**< name of file to delete */
{
    if (unlink(pvfs2tab_name))
    {
        print_error("Unable to unlink [%s].\n"
                    "\tError Number = [%d]\n"
                    "\tError Desc   = [%s]\n",
                    pvfs2tab_name, errno, strerror(errno));
        return (TEST_COMMON_FAIL);
    }
    ...
    const int verbose)             /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (verbose)
    {
        printf("Checking to see if [%s] is on a pvfs2 file system\n", fileName);
    }

    if (use_pvfs2_lib)
    {
        ret = PVFS_util_resolve(fileName,
                                cur_fs, relativeName, relativeNameSize);
        if (ret < 0)
        {
 ...
 */
int finalize(
    int use_pvfs2_lib)          /**< determines use of pvfs2 library */
{
    int ret;
        ...
        if (ret != 0)
        {
            ...
            if (verbose)
            {
                print_error("\tError closing file descriptor [%d].\n"
                            "\t\tError Number = [%d]\n"
                            "\t\tError Desc   = [%s]\n",
                            stFileRef->fd, errno, strerror(errno));
            }
        }
        stFileRef->fd = 0;
    }
    ...
    const int verbose)           /**< Turns on verbose prints if set to non-zero value */
{
    ...
    {
        ret = PVFS_util_resolve(fileName,
                                &fs_id, szPvfsPath, sizeof(szPvfsPath));

        if (ret < 0)
        {
            print_error
                ("Error: could not find file system for [%s] in pvfstab\n",
                 fileName);
            return (TEST_COMMON_FAIL);
        }
    ...
    struct file_ref *pstFileRef)   /**< File descriptor (or handle) for an open file */
{
    ...
    if (use_pvfs2_lib)
    {
        ret = pvfs2_open(fileName,
                         accessFlags, mode, verbose, followLink, pstFileRef);
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (verbose)
    {
        printf("\tCreating [%s] using mode [%o]\n", fileName, mode);
    }
    ...
    if (use_pvfs2_lib)
    {
        ...
        ret = pvfs2_create_file(szPvfsPath,
                                fs_id, &credentials, mode, verbose, &stFileRef);
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (use_pvfs2_lib)
    {
        if (verbose)
        {
            snprintf(cmd, sizeof(cmd), "%spvfs2-mkdir -m %o %s",
                     pvfsEXELocation, mode, directory);
        }
        else
        {
            /* Make sure nothing prints to STDOUT/STDERR if verbose mode is off */
            snprintf(cmd, sizeof(cmd), "%spvfs2-mkdir -m %o %s >/dev/null 2>&1",
                     pvfsEXELocation, mode, directory);
        }
        ret = system(cmd);
        if (ret != 0)
        ...
        if (ret != 0)
        {
            ...
            if (verbose)
            {
                print_error("\tUnable to create [%s] using mode [%o]\n"
                            "\t\tError Number = [%d]\n"
                            "\t\tError Desc   = [%s]\n",
                            directory, mode, errno, strerror(errno));
            }
        }
    }
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (use_pvfs2_lib)
    {
        if (verbose)
        {
            snprintf(cmd, sizeof(cmd), "%spvfs2-rm %s", pvfsEXELocation,
                     directory);
        }
        else
        {
            snprintf(cmd, sizeof(cmd), "%spvfs2-rm %s  >/dev/null 2>&1",
                     pvfsEXELocation, directory);
        }
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (verbose)
    {
        printf("\tChanging mode on [%s] to [%o]\n", fileName, mode);
    }
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value */
{
    ...
    if (use_pvfs2_lib)
    {
        ...
        if (geteuid() != owner_id || geteuid() != group_id)
        {
            if (verbose)
            {
            ...
            else
            {
                snprintf(cmd, sizeof(cmd),
                         "sudo %spvfs2-chown %s %s %s >/dev/null 2>&1",
                         pvfsEXELocation, ownerName, groupName, fileName);
            }
        }
        else
        {
            ...
            {
                snprintf(cmd, sizeof(cmd),
                         "%spvfs2-chown %s %s %s >/dev/null 2>&1",
                         pvfsEXELocation, ownerName, groupName, fileName);
            }
        }
    }
    else
    {
        ...
        if (geteuid() != owner_id && geteuid() != group_id)
        {
            ...
            {
                snprintf(cmd, sizeof(cmd),
                         "sudo chown %s:%s %s >/dev/null 2>&1", ownerName,
                         groupName, fileName);
            }
        }
        else
        ...
        if (use_pvfs2_lib)
        {
            if (verbose)
            {
                print_error("\tUnable to change owner of [%s] to [%s].[%s]\n"
                            ...
                            "\t\tError Desc   = [%s]\n",
                            fileName,
                            ownerName, groupName, errno, strerror(errno));
            }
        }
        else
        {
            if (verbose)
            {
                print_error("\tUnable to change owner of [%s] to [%d]\n"
                            "\t\tError Number = [%d]\n"
                            "\t\tError Desc   = [%s]\n",
                            fileName, owner_id, errno, strerror(errno));
            }
        }
        return (TEST_COMMON_FAIL);
    }
    ...
    const int verbose)      /**< Turns on verbose prints if set to non-zero value*/
{
    ...
    if (verbose)
    {
        printf("\tChanging group on [%s] to [%o]\n", fileName, group_id);
    }
    ...
    if (use_pvfs2_lib)
    {
        ...
        {
            printf
                ("\tchange_group not appropriate for PVFS2 API. call change_owner\n");
        }
        return (-ENODATA);
    }
    else
        ...
        if (ret == -1)
        {
            ...
            if (verbose)
            {
                print_error("\tUnable to change group of [%s] to [%d]\n"
                            "\t\tError Number = [%d]\n"
                            "\t\tError Desc   = [%s]\n",
                            fileName, group_id, errno, strerror(errno));
            }
        }
    }
    ...
    struct file_ref *pstFileRef)   /**< File descriptor (or handle) for an open file */
{
    ...
    ret = PVFS_util_resolve(fileName, &fs_id, szPvfsPath, sizeof(szPvfsPath));
    ...
    if ((ret == -PVFS_ENOENT) && (accessFlags & O_CREAT))
    {
        ret = pvfs2_create_file(szPvfsPath,
                                fs_id, &credentials, mode, verbose, pstFileRef);
    ...
    struct file_ref *pstFileRef)                            /**< File descriptor (or handle) for an open file*/
{
    ...
    ret = PVFS_sys_create(baseName, parent_ref, /* handle & fs_id of parent  */
                          attr, (PVFS_credentials *) credentials, NULL, /* Accept default distribution for fs */
                          &resp_create);
    ...
    int verbose)                                  /**< Turns on verbose prints if set to non-zero value */
{
    ...
    ret = PVFS_sys_lookup(fs_id,
                          szSegment,
                          credentials, &resp_look, PVFS2_LOOKUP_LINK_FOLLOW);
    if (ret < 0)
    {
        if (verbose)
        {
...
{
    int ret = TEST_COMMON_SUCCESS, len = 0;
...
{
    int len = 0;
    char *start = NULL, *end = NULL, *endRef = NULL;
        ...
        if (ret != 0)
        {
            ...
            if (verbose)
            {
                print_error("\tUnable to create symlink [%s] to [%s]\n"
                            "\t\tError Number = [%d]\n"
                            "\t\tError Desc   = [%s]\n",
                            linkName, linkTarget, errno, strerror(errno));
            }
        }
    }
    return ret;
    ...
    struct common_options *opts) /**< structure to hold the common arguments */
{
    ...
    while ((ret =
            getopt_long_only(argc, argv, "", long_opts, &option_index)) != -1)
    {
        switch (ret)
        {
        case 0:
